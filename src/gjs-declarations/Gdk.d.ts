...
/**
 * Generated with https://github.com/Gr3q/GIR2TS - If possible do not
 * modify.
 */
declare module "gi://Gdk" {
  namespace Gdk {
    ...
    interface IAppLaunchContext {
       ...
       * when using this context when running under a window manager
       * that supports multiple workspaces, as described in the
       ...
       * as the Icon field in desktop files. See also
       * {@link Gdk.AppLaunchContext.set_icon}.
       *
       * If both #icon and #icon_name are set, the #icon_name takes
       * priority. If neither #icon or #icon_name is set, the icon is
       * taken from either the file that is passed to launched
       ...
       * priority. If neither #screen or #display are set, the default
       ...
       * typing in another window. This is also known as 'focus
       * stealing prevention'.
       *
     ...
     * that handles launching an application in a graphical context.
     * It provides startup notification and allows to launch
     ...
     * context, &error)) g_warning ("Launching failed: %s\n",
     * error->message);
     *
    ...
    interface ICursor {
       ...
       * system and on the cursor, GDK may not be able to obtain the
       * image data. In this case, %NULL is returned.
       *
       * @returns A {@link Pixbuf} representing #cursor, or %NULL
       */
      get_image(): GdkPixbuf.Pixbuf | null;
      /**
       * Returns a cairo image surface with the image used to display
       * the cursor.
       *
       * Note that depending on the capabilities of the windowing
       * system and on the cursor, GDK may not be able to obtain the
       ...
       *
       *   Removes a reference from #cursor, deallocating the cursor if
    ...
    class Cursor {
       ...
       *
       *   Creates a new cursor from the set of builtin cursors for the
       *   default display. See {@link Gdk.Cursor.new_for_display}.
       *
       ...
       *
       * @param display The {@link Display} for which the cursor will
       *   be created
       * @param cursor_type Cursor to create
       ...
       * different platforms can be found in the CSS specification:
       *
       * - "none"
       ...
       * @returns A new {@link Cursor}, or %NULL if there is no cursor
       *   with the given name
       */
      public static new_from_name(
        display: Display,
        name: string
      ): Cursor | null;
      /**
       ...
       * supported, a monochrome approximation will be displayed. The
       * functions {@link Gdk.Display.supports_cursor_alpha} and
       ...
       * “x_hot” and “y_hot”, resp., containing integer values between
       * `0` and the width resp. height of the pixbuf. (Since: 3.0)
       *
       ...
       * @param display The {@link Display} for which the cursor will
       *   be created
       * @param surface The cairo image surface containing the cursor
       *   pixel data
    ...
    interface IDevice {
       ...
       * keyboard/pointer pairs. Other device types will have a %NULL
       * associated device.
       */
       ...
       * Devices of type %GDK_DEVICE_TYPE_MASTER will have %TRUE
       * here.
       */
      has_cursor: boolean;
       ...
       * Will be 0 if the device is not a touch device or if the
       * number of touches is unknown.
       */
      num_touches: number;
      /**
       ...
       *
       * If #device is of type %GDK_DEVICE_TYPE_FLOATING, %NULL will
       * be returned, as there is no associated device.
       *
       ...
       * device, and locates the value in the array for a given axis
       * use.
       *
       ...
       * Interprets an array of double as axis values for a given
       * device, and locates the value in the array for a given axis
       * label, as returned by {@link Gdk.Device.list_axes}
       *
       * @param axes Pointer to an array of axes
       ...
       *
       * @returns A {@link Display}. This memory is owned by GTK+, and
       *   must not be freed or unreffed.
       */
      get_display(): Display;
      /**
       * Determines whether the pointer follows device motion. This is
       * not meaningful for keyboard devices, which don't have a
       * pointer.
       *
      ...
      /**
       * Gets information about which window the given pointer device
       * is in, based on events that have been received so far from
       * the display server. If another application has a pointer
       * grab, or this application has a grab with owner_events =
       * %FALSE, %NULL may be returned even if the pointer is
       ...
       * unless there is an ongoing grab on them, see
       * {@link Gdk.Device.grab}.
       *
       * @returns Location to store the {@link Screen} the #device is
       *   on, or %NULL.
       *
      ...
      /**
       * Gets the current location of #device in double precision. As
       * a slave device's coordinates are those of its master pointer,
       * this function may not be called on devices of type
       ...
       * information couldn't be obtained. This ID is retrieved from
       * the device, and is thus constant for it. See
       ...
       *
       * @returns A {@link Seat}. This memory is owned by GTK+ and must
       *   not be freed.
       */
      get_seat(): Seat;
      /**
       ...
       * master pointer, this function may not be called on devices of
       * type %GDK_DEVICE_TYPE_SLAVE, unless there is an ongoing grab
       * on them. See {@link Gdk.Device.grab}.
       *
       ...
       * information couldn't be obtained. This ID is retrieved from
       * the device, and is thus constant for it.
       *
       * This function, together with
       * {@link Gdk.Device.get_product_id}, can be used to eg. compose
       * #GSettings paths to store settings for this device.
       *
       * |[<!-- language="C" -->
       *
       * Static GSettings * get_device_settings (GdkDevice *device) {
       * const gchar *vendor, *product; GSettings *settings; GdkDevice
       * *device; gchar *path;
       *
      ...
      /**
       * Obtains the window underneath #device, returning the location
       * of the device in #win_x and #win_y. Returns %NULL if the
       * window tree under #device is not known to GDK (for example,
      ...
      /**
       * Obtains the window underneath #device, returning the location
       * of the device in #win_x and #win_y in double precision.
       * Returns %NULL if the window tree under #device is not known
       * to GDK (for example, belongs to another application).
       *
       * As a slave device coordinates are those of its master
       * pointer, This function may not be called on devices of type
       * %GDK_DEVICE_TYPE_SLAVE, unless there is an ongoing grab on
       ...
       *
       *   Grabs the device so that all events coming from this device
       *   are passed to this application until the device is
       *   ungrabbed with {@link Gdk.Device.ungrab}, or the window
       *   becomes unviewable. This overrides any previous grab on the
       ...
       *   handle the {@link EventGrabBroken} events that are emitted
       ...
       *   #window and only if selected by #event_mask. In either
       ...
       * @param cursor The cursor to display while the grab is active
       *   if the device is a pointer. If this is %NULL then the
       *   normal cursors are used for #window and its descendants,
       ...
       * return the list of slave devices attached to it, otherwise it
       * will return %NULL
       *
       * @returns The list of slave devices, or %NULL. The list must
       *   be freed with {@link G.list_free}, the contents of the list
      ...
      /**
       * Specifies the X key event to generate when a macro button of
       * a device is pressed.
       *
       ...
       * device is active and whether the device’s range is mapped to
       * the entire screen or to a single window.
       *
       ...
       * #screen, unless the device is confined to a window by a grab,
       * in which case it will be moved as far as allowed by the grab.
       * Warping the pointer creates events as if the user had moved
       * the mouse instantaneously to the destination.
       *
       * Note that the pointer should normally be under the control of
       * the user. This function was added to cover some rare use
       * cases like keyboard navigation support for the color picker
       * in the #GtkColorSelectionDialog.
       *
       ...
       * has changed the number of either axes or keys. For example In
       * X this will normally happen when the slave device routing
    ...
    class Device {
      ...
      /**
       * @deprecated The symbol was never meant to be used outside of
       *   GTK+
       *
       *   Determines information about the current keyboard grab. This
       *   is not public API and must not be used by applications.
       * @param display The display for which to get the grab
       *   information
       * @param device Device to get the grab information from
       * @returns %TRUE if this application currently has the keyboard
       *   grabbed.
       *
    ...
    interface IDeviceManager {
       ...
       *
       *   Returns the client pointer, that is, the master pointer that
       *   acts as the core pointer for this application. In X11,
       *   window managers may change this depending on the
       ...
       * @returns The client pointer. This memory is owned by GDK and
       ...
       *   gdk_seat_get_keyboard() and gdk_seat_get_slaves() instead.
       *
       *   Returns the list of devices of type #type currently attached
       *   to #device_manager.
       * @param type Device type to get.
       * @returns A list of {@link Devices}. The returned list must be
      ...
      /**
       * The ::device-changed signal is emitted whenever a device has
       * changed in the hierarchy, either slave devices being
       * disconnected from their master device or connected to another
       * one, or master devices being added or removed a slave
       * device.
       *
    ...
    /**
     * In addition to a single pointer and keyboard for user interface
     * input, GDK contains support for a variety of input devices,
     * including graphics tablets, touchscreens and multiple
     * pointers/keyboards interacting simultaneously with the user
     * interface. Such input devices often have additional features,
     * such as sub-pixel positioning information and additional
     * device-dependent information.
     *
     * In order to query the device hierarchy and be aware of changes
     * in the device hierarchy (such as virtual devices being created
     * or removed, or physical devices being plugged or unplugged),
     * GDK provides {@link DeviceManager}.
     *
     ...
     * {@link Gdk.disable.multidevice} before gdk_display_open(). There
     * should rarely be a need to do that though, since GDK defaults
     * to a compatibility mode in which it will emit just one
     * enter/leave event pair for all devices on a window. To enable
     * per-device enter/leave events and other multi-pointer
     * interaction features, gdk_window_set_support_multidevice() must
     * be called on #GdkWindows (or
     * gtk_widget_set_support_multidevice() on widgets). window. See
     * the gdk_window_set_support_multidevice() documentation for more
     * information.
     *
     ...
     * source. Otherwise either the core or XInput 1 implementations
     * will be used.
     *
     * For simple applications that don’t have any special interest in
     * input devices, the so-called “client pointer” provides a
     ...
     * pointer cursors and keyboard foci that are seen on the screen.
     * Physical devices (or slave devices) represent the hardware that
     * is controlling the virtual devices, and thus have no visible
     * cursor on the screen.
     *
     * Virtual devices are always paired, so there is a keyboard
     * device for every pointer device. Associations between devices
     * may be inspected through gdk_device_get_associated_device().
     *
     * There may be several virtual devices, and several physical
     * devices could be controlling each of these virtual devices.
     * Physical devices may also be “floating”, which means they are
     * not attached to any virtual device.
     *
     ...
     * [master pointer (3)] ⎜ ↳ Virtual core XTEST pointer id=4 [slave
     * pointer (2)] ⎜ ↳ Wacom ISDv4 E6 Pen stylus id=10 [slave pointer
     * (2)] ⎜ ↳ Wacom ISDv4 E6 Finger touch id=11 [slave pointer (2)]
     * ⎜ ↳ SynPS/2 Synaptics TouchPad id=13 [slave pointer (2)] ⎜ ↳
     * TPPS/2 IBM TrackPoint id=14 [slave pointer (2)] ⎜ ↳ Wacom ISDv4
     * E6 Pen eraser id=16 [slave pointer (2)] ⎣ Virtual core keyboard
     * id=3 [master keyboard (2)] ↳ Virtual core XTEST keyboard id=5
     * [slave keyboard (3)] ↳ Power Button id=6 [slave keyboard (3)] ↳
     * Video Bus id=7 [slave keyboard (3)] ↳ Sleep Button id=8 [slave
     * keyboard (3)] ↳ Integrated Camera id=9 [slave keyboard (3)] ↳
     * AT Translated Set 2 keyboard id=12 [slave keyboard (3)] ↳
     * ThinkPad Extra Buttons id=15 [slave keyboard (3)] ]|
     *
     * By default, GDK will automatically listen for events coming
     * from all master devices, setting the #GdkDevice for all events
     * coming from input devices. Events containing device information
     * are #GDK_MOTION_NOTIFY, #GDK_BUTTON_PRESS, #GDK_2BUTTON_PRESS,
     * #GDK_3BUTTON_PRESS, #GDK_BUTTON_RELEASE, #GDK_SCROLL,
     * #GDK_KEY_PRESS, #GDK_KEY_RELEASE, #GDK_ENTER_NOTIFY,
     * #GDK_LEAVE_NOTIFY, #GDK_FOCUS_CHANGE, #GDK_PROXIMITY_IN,
     * #GDK_PROXIMITY_OUT, #GDK_DRAG_ENTER, #GDK_DRAG_LEAVE,
     * #GDK_DRAG_MOTION, #GDK_DRAG_STATUS, #GDK_DROP_START,
     * #GDK_DROP_FINISHED and #GDK_GRAB_BROKEN. When dealing with an
     * event on a master device, it is possible to get the source
     * (slave) device that the event originated from via
     * gdk_event_get_source_device().
     *
     * On a standard session, all physical devices are connected by
     * default to the "Virtual Core Pointer/Keyboard" master devices,
     * hence routing all events through these. This behavior is only
     * modified by device grabs, where the slave device is temporarily
     * detached for as long as the grab is held, and more permanently
     * by user modifications to the device hierarchy.
     *
     * On certain application specific setups, it may make sense to
     * detach a physical device from its master pointer, and mapping
     * it to an specific window. This can be achieved by the
     * combination of gdk_device_grab() and gdk_device_set_mode().
     *
     * In order to listen for events coming from devices other than a
     * virtual device, gdk_window_set_device_events() must be called.
     * Generally, this function can be used to modify the event mask
     * for any given device.
     *
     * Input devices may also provide additional information besides
     * X/Y. For example, graphics tablets may also provide pressure
     * and X/Y tilt information. This information is device-dependent,
     * and may be queried through gdk_device_get_axis(). In
     * multidevice mode, virtual devices will change axes in order to
     * always represent the physical device that is routing events
     * through it. Whenever the physical device changes, the
     * #GdkDevice:n-axes property will be notified, and
     * gdk_device_list_axes() will return the new device axes.
     *
     * Devices may also have associated “keys” or macro buttons. Such
     * keys can be globally set to map into normal X keyboard events.
    ...
    interface IDeviceTool {
       ...
       * When non-zero, the identificator is unique for the given tool
       * model, meaning that two identical tools will share the same
       * #hardware_id, but will have different serial numbers (see
       ...
       * multiple devices with the same #GdkDeviceToolType, but having
       ...
       *   figure out what sort of pen is being used, such as an
    ...
    interface IDisplay {
      ...
      beep(): void;
      /**
       * Closes the connection to the windowing system for the given
       ...
       * happens automatically when the main loop blocks waiting for
       * new events, but if your application is drawing without
       * returning control to the main loop, you may need to call this
       * function explicitly. A common case where this function needs
       * to be called is when an application is executing drawing
       * commands from a thread other than the thread where the main
       * loop is running.
       *
       ...
       * @returns The {@link Monitor}, or %NULL if #monitor_num is not
       *   a valid monitor number
       */
      get_monitor(monitor_num: number): Monitor | null;
      /**
       * Gets the monitor in which the point (#x, #y) is located, or a
       ...
       * {@link Display.monitor_added} or #GdkDisplay::monitor-removed
       * signal.
       *
       ...
       *
       * @returns A string representing the display name. This string
       *   is owned by GDK and should not be modified or freed.
       */
      get_name(): string;
      /**
       ...
       * @returns Location to store the screen that the cursor is on,
       *   or %NULL.
       *
       ...
       *
       * The primary monitor is considered the monitor where the “main
       * desktop” lives. While normal application windows typically
       * allow the window manager to place the windows, specialized
       * desktop applications such as panels should place themselves
       * on the primary monitor.
       *
       * @returns The primary monitor, or %NULL if no primary monitor
       ...
       *   #win_y for #screen. Returns %NULL if the window under the
       *   mouse pointer is not known to GDK (for example, belongs to
       *   another application).
       ...
       *   #display. The list is statically allocated and should not
       *   be freed.
      ...
      /**
       * Gets a copy of the first {@link Event} in the #display’s event
       * queue, without removing the event from the queue. (Note that
       * this function will not get more events from the windowing
       * system. It only checks the events that have already been
       * moved to the GDK event queue.)
       *
       * @returns A copy of the first {@link Event} on the event queue,
       *   or %NULL if no events are in the queue. The returned
       *   #GdkEvent should be freed with {@link Gdk.Event.free}.
       */
      peek_event(): Event | null;
      /**
       * @deprecated Use {@link Gdk.Display.device_is_grabbed} instead.
       *
       ...
       * @returns Whether {@link EventOwnerChange} events will be sent.
       */
       ...
       * {@link Gdk.Display.set_double_click_time}. Applications should
       ...
       * #GDK_2BUTTON_PRESS event). Applications should not set this,
       * it is a global user-configured setting.
       *
       * @param msec Double click time in milliseconds (thousandths of
       *   a second)
       */
       ...
       * according to the [FreeDesktop Clipboard
       ...
       * @param targets An array of targets that should be saved, or
       *   %NULL if all available targets should be saved.
       ...
       * data after an application has quit. On X11 this checks if a
      ...
      /**
       * @deprecated Compositing is an outdated technology that only
       *   ever worked on X11.
       *
       ...
       * #display. Otherwise, cursors are restricted to bilevel alpha
       * (i.e. a mask).
       *
      ...
      /**
       * Returns %TRUE if {@link Gdk.Window.input_shape_combine_mask}
       * can be used to modify the input shape of windows on
       * #display.
       *
      ...
      /**
       * Returns %TRUE if {@link Gdk.Window.shape_combine_mask} can be
       ...
       *   screen #screen, unless the pointer is confined to a window
       *   by a grab, in which case it will be moved as far as allowed
       *   by the grab. Warping the pointer creates events as if the
     ...
     * which can be described as a workstation consisting of a
     * keyboard, a pointing device (such as a mouse) and one or more
     * screens. It is used to open and keep track of various GdkScreen
     * objects currently instantiated by the application. It is also
     * used to access the keyboard(s) and mouse pointer(s) of the
     * display.
     *
    ...
    class Display {
       ...
       * @returns A {@link Display}, or %NULL if the display could not
       *   be opened
       */
      public static open(display_name: string): Display | null;
      /**
       * @deprecated This symbol was never meant to be used outside of
       *   GTK+
       *
       *   Opens the default display specified by command line arguments
       *   or environment variables, sets it as the default display,
       *   and returns it. {@link Gdk.parse.args} must have been called
       *   first. If the default display has previously been set,
       *   simply returns that. An internal function that should not
    ...
    interface IDisplayManager {
       ...
       *
       * @returns A newly allocated #GSList of {@link Display} objects.
       *   Free with {@link G.slist_free} when you are done with it.
       */
    ...
    /**
     * The purpose of the {@link DisplayManager} singleton object is to
     ...
     * with multiple GDK backends, you have to consider both compile
     * time and runtime. At compile time, use the #GDK_WINDOWING_X11,
     * #GDK_WINDOWING_WIN32 macros, etc. to find out which backends
     * are present in the GDK library you are building your
    ...
    class DisplayManager {
       ...
       * with multiple backends). Applications can use
       * {@link Gdk.set.allowed_backends} to limit what backends can be
       * used.
       *
       * @returns The global {@link DisplayManager} singleton;
       *   {@link Gdk.parse.args}, gdk_init(), or gdk_init_check() must
    ...
    interface IDragContext {
       ...
       * {@link Gdk.DragContext.get_suggested_action} returns
       * %GDK_ACTION_ASK.
       *
       ...
       * the window in accordance with the ongoing drag operation. The
       * window is owned by #context and will be destroyed when the
       ...
       * #context. When a drag and drop operation becomes managed, the
       * {@link DragContext} will internally handle all input and
       * source-side #GdkEventDND events as required by the windowing
       * system.
       *
       * Once the drag and drop operation is managed, the drag context
       * will emit the following signals:
       *
       * - The #GdkDragContext::action-changed signal whenever the final
       *   action to be performed by the drag and drop operation
       *   changes.
       * - The #GdkDragContext::drop-performed signal after the user
       *   performs the drag and drop gesture (typically by releasing
       *   the mouse button).
       * - The #GdkDragContext::dnd-finished signal after the drag and
       *   drop operation concludes (after all #GdkSelection transfers
       *   happen).
      ...
      /**
       * Associates a {@link Device} to #context, so all Drag and Drop
       * events for #context are emitted as if they came from this
       * device.
       *
       * @param device A {@link Device}
       */
      ...
      /**
       * Sets the position of the drag window that will be kept under
       * the cursor hotspot. Initially, the hotspot is at the top left
    ...
    export interface DragContextInitOptions
      extends DragContextInitOptionsMixin {}
    ...
    interface IDrawingContext {
       ...
       *
       * @returns A Cairo context to be used to draw the contents of
       *   the {@link Window}. The context is owned by the
    ...
    /**
     * {@link DrawingContext} is an object that represents the current
     * drawing state of a #GdkWindow.
     *
    ...
    interface IFrameClock {
       ...
       * {@link Gdk.FrameClock.end_updating} is made, the frame clock
       * will continually request a new frame with the
       * %GDK_FRAME_CLOCK_PHASE_UPDATE phase. This function may be
       * called multiple times and frames will be requested until
       * gdk_frame_clock_end_updating() is called the same number of
       * times.
       */
      begin_updating(): void;
      /**
       ...
       *   being processed, or even no frame is being processed, for
       *   the previous frame. Before any frames have been processed,
       *   returns %NULL.
       */
       ...
       *   counter for the current frame. Outside of frame processing,
       *   the frame counter for the last frame.
       */
      get_frame_counter(): number;
      /**
       * Gets the time that should currently be used for animations.
       * Inside the processing of a frame, it’s the time used to
       * compute the animation position of everything in a frame.
       * Outside of a frame, it's the time of the conceptual “previous
       * frame,” which may be either the actual previous frame time,
       * or if that’s too old, an updated time.
       *
       * @returns A timestamp in microseconds, in the timescale of of
       ...
       * retrieved with {@link Gdk.FrameClock.get_timings}. The set of
       * stored frames is the set from the counter values given by
       * gdk_frame_clock_get_history_start() and
       * gdk_frame_clock_get_frame_counter(), inclusive.
       *
       * @returns The frame counter value for the oldest frame that is
       *   available in the internal frame history of the
       *   {@link FrameClock}.
       */
      get_history_start(): number;
      /**
       * Using the frame history stored in the frame clock, finds the
       * last known presentation time and refresh interval, and
       * assuming that presentation times are separated by the refresh
       * interval, predicts a presentation time that is a multiple of
       * the refresh interval after the last presentation time, and
       * later than #base_time.
       *
       * @param base_time Base time for determining a presentaton time
       * @returns A location to store the determined refresh interval,
       *   or %NULL. A default refresh interval of 1/60th of a second
       *   will be stored if no history is present.
       *
       *   A location to store the next candidate presentation time
       *   after the given base time. 0 will be will be stored if no
       *   history is present.
       */
      get_refresh_info(
        base_time: number
       ...
       *
       * @param frame_counter The frame counter value identifying the
       ...
       * %GDK_FRAME_CLOCK_PHASE_UPDATE phase for a period of time, you
       * should use gdk_frame_clock_begin_updating() instead, since
      ...
      /**
       * This signal is used to flush pending motion events that are
       * being batched up and compressed together. Applications should
       ...
       * application processing of the frame. Any work to update sizes
       * and positions of application elements should be performed.
       * GTK+ normally handles this internally.
       *
       * @param signal
       * @param callback Callback function
       *
       ...
       * application processing of the frame. The frame is repainted.
       * GDK normally handles this internally and produces expose
       * events, which are turned into GTK+ #GtkWidget::draw signals.
       *
       * @param signal
       * @param callback Callback function
       *
       ...
       * event processing. Applications should not handle this
       * signal.
       *
       * @param signal
       * @param callback Callback function
     ...
     * uses a simple timer rather than a hardware-based vertical sync,
     * the frame clock helps because it ensures everything paints at
     * the same time (reducing the total number of frames). The frame
     * clock can also automatically stop painting when it knows the
     * frames will not be visible, or scale back animation
     * framerates.
     *
     ...
     * {@link Gdk.FrameClock.request_phase}. At some later point that
     * makes sense for the synchronization being implemented, the
     * clock will process a frame and emit signals for each phase that
     * has been requested. (See the signals of the #GdkFrameClock
     ...
     *
     * The frame time is reported in microseconds and generally in the
     * same timescale as g_get_monotonic_time(), however, it is not
     * the same as g_get_monotonic_time(). The frame time does not
     * advance during the time a frame is being painted, and outside
     * of a frame, an attempt is made so that all calls to
     * gdk_frame_clock_get_frame_time() that are called at a “similar”
     * time get the same value. This means that if different
     * animations are timed by looking at the difference in time
     * between an initial value from gdk_frame_clock_get_frame_time()
     * and the value inside the #GdkFrameClock::update signal of the
     * clock, they will stay exactly synchronized.
     */
    ...
    interface IGLContext {
      ...
      display: Display;
      /**
       * The {@link GLContext} that this context is sharing data with,
       * or %NULL
       */
      ...
      /**
       * Retrieves the {@link GLContext} that this #context share data
       * with.
       *
      ...
      /**
       * Checks whether the #context is using an OpenGL or OpenGL ES
       * profile.
       *
       * @returns %TRUE if the {@link GLContext} is using an OpenGL ES
       *   profile
       */
      get_use_es(): boolean;
      /**
       ...
       * function.
       *
       * When realizing a GL context, GDK will try to use the OpenGL
       * 3.2 core profile; this profile removes all the OpenGL API
       * that was deprecated prior to the 3.2 version of the
       * specification. If the realization is successful, this
       * function will return %FALSE.
       *
       * If the underlying OpenGL implementation does not support core
       * profiles, GDK will fall back to a pre-3.2 compatibility
       * profile, and this function will return %TRUE.
       *
       * You can use the value returned by this function to decide
       * which kind of OpenGL API to use, or whether to do extension
       * discovery, or what kind of shader programs to load.
       *
       ...
       *
       * The #GdkGLContext must not be realized or made current prior
       ...
       * other hand, must support both deprecated and non deprecated
       * functionality.
       *
       * The #GdkGLContext must not be realized or made current prior
       * to calling this function.
       ...
       *
       * By default, GDK will attempt to automatically detect whether
       * the underlying GL implementation is OpenGL or OpenGL ES once
       * the #context is realized.
       *
       * You should check the return value of
       * {@link Gdk.gl_context_get_use_es} after calling
       * gdk_gl_context_realize() to decide whether to use the OpenGL
       * or OpenGL ES API, extensions, or shaders.
       *
     ...
     * back buffer. The GDK repaint system is in full control of the
     * painting to that. Instead, you can create render buffers or
     * textures and use gdk_cairo_draw_from_gl() in the draw function
     * of your widget to draw them. Then GDK will handle the
     ...
     * #GdkWindow, which you typically get during the realize call of
     * a widget.
     *
     * A #GdkGLContext is not realized until either
     * gdk_gl_context_make_current(), or until it is realized using
     * gdk_gl_context_realize(). It is possible to specify details of
     * the GL context like the OpenGL version to be used, or whether
     * the GL context should have extra state validation enabled after
     * calling gdk_window_create_gl_context() by calling
     * gdk_gl_context_realize(). If the realization fails you have the
     * option to change the settings of the #GdkGLContext and try
     * again.
     *
     * ## Using a GdkGLContext
     *
     * You will need to make the #GdkGLContext the current context
     * before issuing OpenGL calls; the system sends OpenGL commands
     * to whichever context is current. It is possible to have
     * multiple contexts, so you always need to ensure that the one
     ...
     * gdk_gl_context_get_current(); you can also unset any
    ...
    interface IKeymap {
       ...
       * are set in #state to the virtual modifiers (i.e. Super, Hyper
       * and Meta) and set the corresponding bits in #state.
       *
       * GDK already does this before delivering key events, but for
       * compatibility reasons, it only sets the first virtual
       ...
       *
       * @returns %PANGO_DIRECTION_LTR or %PANGO_DIRECTION_RTL if it
       *   can determine the direction. %PANGO_DIRECTION_NEUTRAL
       *   otherwise.
       */
       ...
       * #keyvals. Free the returned arrays with {@link G.free}. When a
       * keycode is pressed by the user, the keyval from this list of
       * entries is selected by considering the effective keyboard
       * group and level. See gdk_keymap_translate_keyboard_state().
       *
       ...
       * mode; in general, the level determines whether the top or
       * bottom symbol on a key is used, and the group determines
       * whether the left or right symbol is used. On US keyboards,
       * the shift key changes the keyboard level, and there are no
       * groups. A group switch key might convert a keyboard between
       * Hebrew to English modes, for example. {@link EventKey}
       * contains a %group field that indicates the active keyboard
       * group. The level is computed from the modifier mask. The
       * returned array should be freed with {@link G.free}.
       *
       ...
       * #GDK_MOD1_MASK rather than #GDK_META_MASK if the backend maps
       * MOD1 to META), so there are use cases where the return value
       * of this function has to be transformed by
       * {@link Gdk.Keymap.add_virtual_modifiers} in order to contain
       * the expected result.
       *
       ...
       * If no keyval is bound to #key, returns 0. For normal user
       * input, you want to use
       * {@link Gdk.Keymap.translate_keyboard_state} instead of this
       * function, since the effective group/level may not be the same
       * as the current keyboard state.
       *
       * @param key A {@link KeymapKey} with keycode, group, and level
       *   initialized
       ...
       * are set in #state to their non-virtual counterparts (i.e.
       ...
       *   same non-virtual modifier. Note that %FALSE is also
       *   returned if a virtual modifier is mapped to a non-virtual
       *   modifier that was already set in #state.
       */
      map_virtual_modifiers(): boolean;
      /**
       * Translates the contents of a {@link EventKey} into a keyval,
       * effective group, and level. Modifiers that affected the
       * translation and are thus unavailable for application use are
       * returned in #consumed_modifiers. See
       * [Groups][key-group-explanation] for an explanation of groups
       * and levels. The #effective_group is the group that was
       * actually used for the translation; some keys such as Enter
       * are not affected by the active keyboard group. The #level is
       * derived from #state. For convenience, #GdkEventKey already
       * contains the translated keyval, so this function isn’t as
       ...
       * For instance, on a US keyboard, the `plus` symbol is shifted,
       * so when comparing a key press to a `<Control>plus`
       ...
       * &keyval, NULL, NULL, &consumed); if (keyval == GDK_PLUS &&
       * (event->state & ~consumed & ALL_ACCELS_MASK) ==
       * GDK_CONTROL_MASK) // Control was pressed ]|
       *
       * An older interpretation #consumed_modifiers was that it
       * contained all modifiers that might affect the translation of
       * the key; this allowed accelerators to be stored with
       * irrelevant consumed modifiers, by doing:
       *
       * |[<!-- language="C" -->
       *
       * // XXX Don’t do this XXX if (keyval == accel_keyval &&
       * (event->state & ~consumed & ALL_ACCELS_MASK) == (accel_mods &
       ...
       * would be masked out even if only `<Control><Alt>` was used in
       * the keymap. To support this usage as well as well as
       * possible, all single modifier combinations that could affect
       * the key for any combination of modifiers will be returned in
       * #consumed_modifiers; multi-modifier combinations are returned
       * only when actually found in #state. When you store
       * accelerators, you should always store them with consumed
       * modifiers removed. Store `<Control>plus`, not
       * `<Control><Shift>plus`,
       *
       * @param hardware_keycode A keycode
       ...
       *
       *   Return location for modifiers that were used to determine the
       ...
       * {@link Gdk.Keymap.get_caps_lock_state}.
       *
       * @param signal
       * @param callback Callback function
       *
    ...
    interface IMonitor {
       ...
       * within the display coordinate space. The returned geometry is
       ...
       * systems this is 1, but on very high density outputs this can
       * be a higher value (often 2).
       *
       * This can be used if you want to create pixel based data for a
       * particular monitor, but most of the time you’re drawing to a
       * window where it is better to use
       ...
       * geometry is in ”application pixels”, not in ”device pixels”
       * (see {@link Gdk.Monitor.get_scale_factor}).
       *
       * The work area should be considered when positioning menus and
       * similar popups, to avoid placing them below panels, docks or
       ...
       * {@link Gdk.Display.get_primary_monitor}).
       *
     ...
     * enumerate monitors with {@link Gdk.Display.get_n_monitors} and
     * gdk_display_get_monitor(), and to find particular monitors with
     * gdk_display_get_primary_monitor() or
    ...
    interface IScreen {
       ...
       * there is no currently currently active window, or the window
       * manager does not support the _NET_ACTIVE_WINDOW hint, this
       * function returns %NULL.
       *
       * On other platforms, this function may return %NULL, depending
       ...
       *   has multiple monitors of different resolution. It is
       *   recommended to use the monitor dimensions instead.
       ...
       * @returns The monitor number in which the point (#x,#y) lies,
       ...
       *   position of the individual monitor within the entire screen
       *   area. The returned geometry is in ”application pixels”, not
       *   in ”device pixels” (see
       *   {@link Gdk.Screen.get_monitor_scale_factor}).
       *
       *   Monitor numbers start at 0. To obtain the number of monitors
       *   of #screen, use gdk_screen_get_n_monitors().
       *
       *   Note that the size of the entire screen area can be retrieved
       *   via gdk_screen_get_width() and gdk_screen_get_height().
       ...
       *   something like VGA, DVI, or TV, not the actual product name
      ...
      /**
       * @deprecated Use {@link Gdk.Monitor.get_scale_factor} instead
       *
       ...
       *
       *   This can be used if you want to create pixel based data for a
       *   particular monitor, but most of the time you’re drawing to
       *   a window where it is better to use
       *   {@link Gdk.Window.get_scale_factor} instead.
       * @param monitor_num Number of the monitor, between 0 and
       *   gdk_screen_get_n_monitors (screen)
       * @returns The scale factor
       ...
       *
       *   Retrieves the {@link Rectangle} representing the size and
       *   position of the “work area” on a monitor within the entire
       *   screen area. The returned geometry is in ”application
       *   pixels”, not in ”device pixels” (see
       *   {@link Gdk.Screen.get_monitor_scale_factor}).
       *
       *   The work area should be considered when positioning menus and
       *   similar popups, to avoid placing them below panels, docks
      ...
      get_n_monitors(): number;
      /**
       * Gets the index of #screen among the screens in the display to
       * which it belongs. (See {@link Gdk.Screen.get_display})
       *
       * @returns The index
       */
      get_number(): number;
       ...
       *   instead
       *
       *   Gets the primary monitor for #screen. The primary monitor is
       *   considered the monitor where the “main desktop” lives.
       *   While normal application windows typically allow the window
       *   manager to place the windows, specialized desktop
       *   applications such as panels should place themselves on the
       *   primary monitor.
       *
       *   If no primary monitor is configured by the user, the return
       *   value will be 0, defaulting to the first monitor.
       ...
       *
       * @returns The current resolution, or -1 if no resolution has
       *   been set.
       */
       ...
       * that the window’s alpha channel won’t be honored when
       * displaying the window on the screen: in particular, for X an
       * appropriate windowing manager and compositing manager must be
       * running to provide appropriate display.
       *
       ...
       *
       * @returns A visual to use for windows with an alpha channel or
       *   %NULL if the capability is not available.
       */
       ...
       * visual for the root window of the display. The return value
       * should not be freed.
       *
       ...
       *
       * The returned list should be freed with g_list_free(), but its
       * elements need not be freed.
       *
       ...
       * Hints](http://www.freedesktop.org/Standards/wm-spec). If the
       ...
       * on whether it is implementable on that platform.
       *
       * The returned list is newly allocated and owns references to
       * the windows it contains, so it should be freed using
       ...
       *   expected to have their alpha channels drawn correctly on
       *   the screen.
       */
      is_composited(): boolean;
      /**
       * Lists the available visuals for the specified #screen. A
       * visual describes a hardware image data format. For example, a
       * visual might support 24-bit color, or 8-bit color, and might
      ...
      /**
       * Sets the default font options for the screen. These options
       * will be set on any #PangoContext’s newly created with
       * {@link Gdk.pango.context_get_for_screen}. Changing the default
       * set of font options does not affect contexts that have
      ...
      /**
       * Sets the resolution for font handling on the screen. This is
       * a scale factor between points specified in a
       * #PangoFontDescription and cairo units. The default value is
       * 96, meaning that a 10 point font will be 13 units high. (10 *
       * 96. / 72. = 13.3).
       *
    ...
    /**
     * {@link Screen} objects are the GDK representation of the screen
     * on which windows can be displayed and on which the pointer
     * moves. X originally identified screens with physical screens,
     * but nowadays it is more common to have a single #GdkScreen
     * which combines several physical monitors (see
     ...
     * used to query the screen specification and default settings
     * such as the default visual (gdk_screen_get_system_visual()),
     * the dimensions of the physical monitors
    ...
    class Screen {
       ...
       *   size is in ”application pixels”, not in ”device pixels”
       *   (see {@link Gdk.Screen.get_monitor_scale_factor}).
       ...
       * @deprecated Use per-monitor information
       *
       *   Returns the height of the default screen in millimeters. Note
       ...
       * @deprecated Use per-monitor information
       *
       *   Returns the width of the default screen in millimeters. Note
       *   that on many X servers this value will not be correct.
    ...
    interface ISeat {
       ...
       *
       * @returns A {@link Display}. This object is owned by GTK+ and
       *   must not be freed.
       */
      get_display(): Display;
      /**
       ...
       *
       * @returns A master {@link Device} with keyboard capabilities.
       ...
       *   with {@link G.list_free}, the elements are owned by GDK and
       *   must not be freed.
       */
      get_slaves(capabilities: SeatCapabilities): GLib.List;
      /**
       * Grabs the seat so that all events corresponding to the given
       * #capabilities are passed to this application until the seat
       * is ungrabbed with {@link Gdk.Seat.ungrab}, or the window
       * becomes hidden. This overrides any previous grab on the seat
       * by this client.
       *
       ...
       * over the events corresponding to the given capabilities. For
       * example in GTK+ this is used for Drag and Drop operations,
       * popup menus and such.
       *
       * Note that if the event mask of a {@link Window} has selected
       * both button press and button release events, or touch begin
       * and touch end, then a press event will cause an automatic
       * grab until the button is released, equivalent to a grab on
       * the window with #owner_events set to %TRUE. This is done
       * because most applications expect to receive paired press and
       * release events.
       *
       * If you set up anything at the time you take the grab that
       * needs to be cleaned up when the grab ends, you should handle
       * the #GdkEventGrabBroken events that are emitted when the grab
       * ends unvoluntarily.
       *
       ...
       *   used elsewhere.
       * @param event The event that is triggering the grab, or %NULL
       *   if none is available.
       * @param prepare_func Function to prepare the window to be
       *   grabbed, it can be %NULL if #window is visible before this
       *   call.
      ...
      ungrab(): void;
      /**
       * The ::device-added signal is emitted when a new input device
       * is related to this seat.
       *
       * @param signal
       * @param callback Callback function
       *
      ...
      /**
       * The ::device-removed signal is emitted when an input device
       * is removed (e.g. unplugged).
       *
       * @param signal
       * @param callback Callback function
       *
       ...
       * made known to the seat. The tool may later be assigned to a
       * device (i.e. on proximity with a tablet). The device will
       * emit the {@link Device.tool_changed} signal accordingly.
       *
      ...
      /**
       * This signal is emitted whenever a tool is no longer known to
       * this #seat.
       *
       * @param signal
       * @param callback Callback function
    ...
    interface IVisual {
      /**
       * @deprecated Use {@link Gdk.Visual.get_red_pixel_details} and
       *   its variants to learn about the pixel layout of TrueColor
       ...
       *   function.
       * @returns The number of significant bits per color value for
       *   #visual.
       */
      get_bits_per_rgb(): number;
      /**
       * Obtains values that are needed to calculate blue pixel values
       * in TrueColor and DirectColor. The “mask” is the significant
       * bits within the pixel. The “shift” is the number of bits left
       * we must shift a primary for it to be in position (according
       * to the "mask"). Finally, "precision" refers to how much
       ...
       * significant bits within the pixel. The “shift” is the number
       * of bits left we must shift a primary for it to be in position
       * (according to the "mask"). Finally, "precision" refers to how
       * much precision the pixel value contains for a particular
       * primary.
       *
       * @returns A pointer to a #guint32 to be filled in, or %NULL
       *
      ...
      /**
       * Obtains values that are needed to calculate red pixel values
       * in TrueColor and DirectColor. The “mask” is the significant
       * bits within the pixel. The “shift” is the number of bits left
       * we must shift a primary for it to be in position (according
       * to the "mask"). Finally, "precision" refers to how much
      ...
      get_screen(): Screen;
      /**
       * Returns the type of visual this is (PseudoColor, TrueColor,
       * etc).
       *
    ...
    /** A {@link Visual} contains information about a particular visual. */
    ...
    class Visual {
       ...
       *   “Best” means “largest,” i.e. 32 preferred over 24 preferred
       *   over 8 bits per pixel.
       ...
       *   are preferred over grayscale or fixed-colormap visuals. The
       *   return value should not be freed. %NULL may be returned if
       *   no visual supports #depth.
       * @param depth A bit depth
       ...
       *   GDK screen. Visuals with higher color depths are considered
       *   better. The return value should not be freed. %NULL may be
       ...
       *   This is the visual for the root window of the display. The
    ...
    interface IWindow {
       ...
       * {@link Gdk.Window.set_cursor} and gdk_window_get_cursor() for
       * details.
       */
      cursor: Cursor;
      /**
       * Adds an event filter to #window, allowing you to intercept
       * events before they reach GDK. This is a low-level operation
       * and makes it easy to break GDK and/or GTK+, so you have to
       * know what you're doing. Pass %NULL for #window to get all
       * events for all windows, instead of events for a specific
       * window.
       *
       ...
       * display, if supported. Otherwise, emits a short beep on the
       * display just as {@link Gdk.Display.beep}.
       */
      beep(): void;
      /**
       ...
       * large enough to contain #region will be created. The backing
       * store will be initialized with the background color or
       * background surface for #window. Then, all drawing operations
       * performed on #window will be diverted to the backing store.
       * When you call {@link Gdk.Window.end_frame}, the contents of
       * the backing store will be copied to #window, making it
       * visible on screen. Only the part of #window contained in
       * #region will be modified; that is, drawing operations are
       * clipped to #region.
       *
       * The net result of all this is to remove flicker, because the
       * user sees the finished product appear all at once when you
       * call gdk_window_end_draw_frame(). If you draw to #window
       * directly without calling gdk_window_begin_draw_frame(), the
       * user may see flicker as individual drawing operations are
       * performed in sequence.
       *
       * When using GTK+, the widget system automatically places calls
       * to gdk_window_begin_draw_frame() and
       ...
       * widget has a cleared background, is already set as the clip
       * region, and already has a backing store. Therefore in most
       * cases, application code in GTK does not need to call
       * gdk_window_begin_draw_frame() explicitly.
       *
       * @param region A Cairo region
       * @returns A {@link DrawingContext} context that should be used
       *   to draw the contents of the window; the returned context is
       *   owned by GDK.
       */
       ...
       * {@link Gdk.Window.begin_move_drag_for_device} to begin a drag
      ...
      /**
       * Begins a window move operation (for a toplevel window). You
       * might use this function to implement a “window move grip,”
       * for example. The function works best with window managers
       * that support the [Extended Window Manager
       * Hints](http://www.freedesktop.org/Standards/wm-spec) but has
       * a fallback implementation for other window managers.
       *
       ...
       *   initialized with the background color or background surface
       *   for #window. Then, all drawing operations performed on
       *   #window will be diverted to the backing store. When you
       *   call {@link Gdk.Window.end_paint}, the backing store will be
       *   copied to #window, making it visible onscreen. Only the
       *   part of #window contained in #region will be modified; that
       *   is, drawing operations are clipped to #region.
       *
       *   The net result of all this is to remove flicker, because the
       *   user sees the finished product appear all at once when you
       *   call gdk_window_end_paint(). If you draw to #window
       *   directly without calling gdk_window_begin_paint_region(),
       *   the user may see flicker as individual drawing operations
       *   are performed in sequence. The clipping and
       *   background-initializing features of
       *   gdk_window_begin_paint_region() are conveniences for the
       *   programmer, so you can avoid doing that work yourself.
       *
       *   When using GTK+, the widget system automatically places calls
       *   to gdk_window_begin_paint_region() and
       *   gdk_window_end_paint() around emissions of the expose_event
       *   signal. That is, if you’re writing an expose event handler,
       *   you can assume that the exposed area in {@link EventExpose}
       *   has already been cleared to the window background, is
       *   already set as the clip region, and already has a backing
       *   store. Therefore in most cases, application code need not
       *   call gdk_window_begin_paint_region(). (You can disable the
       *   automatic calls around expose events on a widget-by-widget
       *   basis by calling gtk_widget_set_double_buffered().)
       *
       *   If you call this function multiple times before calling the
       *   matching gdk_window_end_paint(), the backing stores are
       *   pushed onto a stack. gdk_window_end_paint() copies the
       *   topmost backing store onscreen, subtracts the topmost
       *   region from all other regions in the stack, and pops the
       *   stack. All drawing operations affect only the topmost
       *   backing store in the stack. One matching call to
       ...
       * might use this function to implement a “window resize grip,”
       * for example; in fact #GtkStatusbar uses it. The function
       * works best with window managers that support the [Extended
       * Window Manager
       * Hints](http://www.freedesktop.org/Standards/wm-spec) but has
       * a fallback implementation for other window managers.
       *
       ...
       * window, where the parent window is the normal parent as
       * returned by {@link Gdk.Window.get_parent} for normal windows,
       * and the window's embedder as returned by
       ...
       * (which can be arbitrarily transformed), this function calls
       * the GdkWindow::from-embedder: signal to translate the
       * coordinates.
       *
       * You should always use this function when writing generic code
       * that walks down a window hierarchy.
       *
       ...
       * parent window, where the parent window is the normal parent
       * as returned by {@link Gdk.Window.get_parent} for normal
       * windows, and the window's embedder as returned by
       * gdk_offscreen_window_get_embedder() for offscreen windows.
       *
       * For normal windows, calling this function is equivalent to
       * adding the return values of gdk_window_get_position() to the
       * child coordinates. For offscreen windows however (which can
       * be arbitrarily transformed), this function calls the
       * GdkWindow::to-embedder: signal to translate the coordinates.
       *
       * You should always use this function when writing generic code
       * that walks up a window hierarchy.
       *
       ...
       *
       * If the creation of the #GdkGLContext failed, #error will be
       * set.
       *
       ...
       * the scaling factor of the #window, or by the #scale argument;
       * they are the size of the surface in device pixels. If you
       * wish to create an image surface capable of holding the
       * contents of #window you can use:
       *
       * |[<!-- language="C" -->
       *
       * Int scale = gdk_window_get_scale_factor (window); int width =
       * gdk_window_get_width (window) * scale; int height =
       ...
       * width, height, scale); ]|
       *
       * Note that unlike {@link Cairo.Surface.create_similar_image},
       * the new surface's device scale is set to #scale, or to the
       * scale factor of #window if #scale is 0.
       *
       ...
       * the given #window. For example the new surface will have the
       * same fallback resolution and font options as #window.
       * Generally, the new surface will also use the same backend as
       * #window, unless that is not possible for some reason. The
       * type of the returned surface may be examined with
       * {@link Cairo.Surface.get_type}.
       *
       * Initially the surface contents are all 0 (transparent if
       * contents have transparency, black otherwise.)
       *
      ...
      /**
       * Attempt to deiconify (unminimize) #window. On X11 the window
       * manager may choose to ignore the request to deiconify. When
       * using GTK+, use {@link Gtk.Window.deiconify} instead of the
       * {@link Window} variant. Or better yet, you probably want to
       * use gtk_window_present_with_time(), which raises the window,
       ...
       * and decrements #window's reference count. The window system
       * resources for all children of #window are also destroyed, but
       * the children’s reference counts are not decremented.
       *
       * Note that a window will not be destroyed automatically when
       * its reference count reaches zero. You must call this function
      ...
      /**
       * Indicates that the drawing of the contents of #window started
       * with {@link Gdk.Window.begin_frame} has been completed.
       *
      ...
      /**
       * Indicates that the backing store created by the most recent
       * call to {@link Gdk.Window.begin_paint_region} should be copied
       * onscreen and deleted, leaving the next-most-recent backing
       * store or no backing store at all as the active paint region.
       * See gdk_window_begin_paint_region() for full details.
       *
       * It is an error to call this function without a matching
      ...
      end_paint(): void;
      /**
       * Tries to ensure that there is a window-system native window
       * for this GdkWindow. This may fail in some situations,
       * returning %FALSE.
       *
       * Offscreen window and children of them can never have native
       * windows.
       *
       ...
       *
       * @param timestamp Timestamp of the event triggering the window
       *   focus
       */
       ...
       *   that it won't receive expose events. The window will begin
       *   receiving expose events again when
       *   {@link Gdk.Window.thaw_toplevel_updates_libgtk_only} is
       *   called. If gdk_window_freeze_toplevel_updates_libgtk_only()
       ...
       * expose events. The window will begin receiving expose events
       * again when {@link Gdk.Window.thaw_updates} is called. If
       * gdk_window_freeze_updates() has been called more than once,
       * gdk_window_thaw_updates() must be called an equal number of
       * times to begin processing exposes.
       */
      freeze_updates(): void;
      /**
       * Moves the window into fullscreen mode. This means the window
       * covers the entire screen and is above any panels or task
       * bars.
       *
       * If the window was already fullscreen, then this function does
       * nothing.
       *
       ...
       * This means the window covers the entire screen and is above
       * any panels or task bars.
       *
       * If the window was already fullscreen, then this function does
       * nothing.
       *
       ...
       * example it’s useless when used with the root window; it only
       * returns windows an application created itself.
       *
       * The returned list must be freed, but the elements in the list
       * need not be.
       *
       ...
       * into account other factors such as if the window is obscured
       * by other windows, but no area outside of this region will be
       * affected by drawing primitives.
       *
       ...
       * specified window, and it is using the cursor for its parent
       * window.
       *
      ...
      /**
       * Retrieves a {@link Cursor} pointer for the #device currently
       * set on the specified #GdkWindow, or %NULL. If the return
       * value is %NULL then there is no custom cursor set on the
       * specified window, and it is using the cursor for its parent
       * window.
      ...
      /**
       * Obtains the current device position and modifier state. The
       * position is given in coordinates relative to the upper left
       * corner of #window.
       *
       * Use {@link Gdk.Window.get_device_position_double} if you need
       * subpixel precision.
       *
       * @param device Pointer {@link Device} to query to.
       * @returns The window underneath #device (as with
       *   {@link Gdk.Device.get_window_at_position}), or %NULL if the
      ...
      /**
       * Obtains the current device position in doubles and modifier
       * state. The position is given in coordinates relative to the
       ...
       *
       *   Location of the window where the drop should happen. This may
       *   be #window or a proxy window, or %NULL if #window does not
       *   support Drag and Drop.
       */
      get_drag_protocol(): [DragProtocol, Window | null];
      /**
       * Obtains the parent of #window, as known to GDK. Works like
       * {@link Gdk.Window.get_parent} for normal windows, but returns
       * the window’s embedder for offscreen windows.
       *
       ...
       * See {@link Gdk.Window.set_events}.
       *
       * @returns Event mask for #window
       */
      get_events(): EventMask;
      /**
       * Determines whether or not the desktop environment should be
       * hinted that the window does not want to receive input focus
       * when it is mapped.
       *
       ...
       * window never changes unless the window is reparented to a new
       * toplevel window.
       *
       ...
       * manager titlebar/borders if any. The frame position is given
       * in root window coordinates. To get the position of the window
       * itself (rather than the frame) in root window coordinates,
       * use {@link Gdk.Window.get_origin}.
       *
       ...
       *
       * @returns The {@link FullscreenMode} applied to the window when
       *   fullscreen.
       */
       ...
       * field.
       *
       * The X and Y coordinates returned are relative to the parent
       * window of #window, which for toplevels usually means relative
       * to the window decorations (titlebar, etc.) rather than
       * relative to the root window (screen-size background window).
       *
       * On the X11 platform, the geometry is obtained from the X
       * server, so reflects the latest position of #window; this may
       * be out-of-sync with the position of #window delivered in the
       * most-recently-processed {@link EventConfigure}.
       * {@link Gdk.Window.get_position} in contrast gets the position
       ...
       * roundtrip to the X server and because these functions support
       * the full 32-bit coordinate space, whereas
       * gdk_window_get_geometry() is restricted to the 16-bit
       * coordinates of X11.
       *
       * @returns Return location for X coordinate of window (relative
       *   to its parent)
       *
       *   Return location for Y coordinate of window (relative to its
       *   parent)
       *
       ...
       *
       * On the X11 platform the returned size is the size reported in
       * the most-recently-processed configure event, rather than the
       ...
       * gdk_window_get_geometry() which return the position of a
       ...
       * query the X server; thus this returns the parent as passed to
       * {@link Gdk.Window.new}, not the actual parent. This should
       * never matter unless you’re using Xlib calls mixed with GDK
       * calls on the X11 platform. It may also matter for toplevel
       * windows, because the window manager may choose to reparent
       * them.
       *
       * Note that you should use gdk_window_get_effective_parent()
       * when writing generic code that walks up a window hierarchy,
       * because gdk_window_get_parent() will most likely not do what
       * you expect if there are offscreen windows in the hierarchy.
       *
       * @returns Parent of #window
       */
      get_parent(): Window;
      /**
       * Returns whether input to the window is passed through to the
       * window below.
       *
       ...
       * {@link Gdk.Window.get_geometry} which queries the X server for
       * the current window position, regardless of which events have
       ...
       * coordinates. This is similar to {@link Gdk.Window.get_origin}
       * but allows you to pass in any position in the window, not
       * just the origin.
       *
       ...
       * be a higher value (often 2).
       *
       * A higher value means that drawing is automatically scaled up
       * to a higher resolution, so any code doing drawing will
       ...
       * toplevel window, as is a %GDK_WINDOW_CHILD window that has a
       * root window as parent.
       *
       * Note that you should use
       * {@link Gdk.Window.get_effective_toplevel} when you want to get
       * to a window’s toplevel as seen on screen, because
       ...
       * caller of the function. That is, after calling this function,
       * #window will no longer have an invalid/dirty region; the
       * update area is removed from #window and handed to you. If a
       * window has no update area, {@link Gdk.Window.get_update_area}
       * returns %NULL. You are responsible for calling
       * cairo_region_destroy() on the returned region if it’s
       * non-%NULL.
       *
       ...
       * visible. This does not necessarily take into account if the
       * window is obscured by other windows, but no area outside of
       ...
       * that you can use {@link Gdk.Window.ensure_native} if a native
       * window is needed.
       *
       * @returns %TRUE if the #window has a native window, %FALSE
       *   otherwise.
      ...
      has_native(): boolean;
      /**
       * For toplevel windows, withdraws them, so they will no longer
       * be known to the window manager; for all windows, unmaps them,
       * so they won’t be displayed. Normally done automatically as
       ...
       * Using {@link Gtk.Window.iconify} is preferred, if you have a
       * #GtkWindow widget.
       *
       ...
       * while the pointer position corresponds to an unset bit in the
       ...
       * extension.
       *
       * On the Win32 platform, this functionality is not present and
      ...
      /**
       * Adds #region to the update area for #window. The update area
       * is the region that needs to be redrawn, or “dirty region.”
       * The call {@link Gdk.Window.process_updates} sends one or more
       * expose events to the window, which together cover the entire
       * update area. An application would normally redraw the
       * contents of #window in response to those expose events.
       *
       * GDK will call gdk_window_process_all_updates() on your behalf
       * whenever your program returns to the main loop and becomes
       * idle, so normally there’s no need to do that manually, you
       * just need to invalidate regions that you know should be
       * redrawn.
       *
       * The #child_func parameter controls whether the region of each
       * child window that intersects #region will also be
       * invalidated. Only children for which #child_func returns
       * #TRUE will have the area invalidated.
       *
       * @param region A #cairo_region_t
       * @param child_func Function to use to decide if to recurse to
       *   a child, %NULL means never recurse.
       */
       ...
       *   the whole window
       * @param invalidate_children Whether to also invalidate child
       *   windows
       */
      invalidate_rect(
        rect: Rectangle | null,
       ...
       * region of each child window that intersects #region will also
       * be invalidated. If %FALSE, then the update area for child
       * windows will remain unaffected. See
       * gdk_window_invalidate_maybe_recurse if you need fine grained
       ...
       * mapped. (This is not necessarily "viewable" in the X sense,
       * since we only check as far as we have GDK window parents, not
       * to the root window.)
       *
      ...
      is_visible(): boolean;
      /**
       * Lowers #window to the bottom of the Z-order (stacking order),
       * so that other windows with the same parent window appear
       * above #window. This is true whether or not the other windows
       * are visible.
       *
       * If #window is a toplevel, the window manager may choose to
       * deny the request to move the window in the Z-order,
       * {@link Gdk.Window.lower} only requests the restack, does not
       * guarantee it.
       *
       * Note that gdk_window_show() raises the window again, so don’t
       * call this function before gdk_window_show(). (Try
       ...
       *
       * On X11, asks the window manager to maximize #window, if the
       * window manager supports this operation. Not all window
       * managers support this, and some deliberately ignore it or
       * don’t have a concept of “maximized”; so you can’t rely on the
       * maximization actually happening. But it will happen with most
       * standard window managers, and GDK makes a best effort to get
       * it to happen.
       *
       ...
       * input shape mask for #window. i.e. the union of all input
       * masks for #window and its children will become the new input
       * mask for #window. See
      ...
      /**
       * Merges the shape masks for any child windows into the shape
       * mask for #window. i.e. the union of all masks for #window and
       * its children will become the new mask for #window. See
       * {@link Gdk.Window.shape_combine_region}.
       *
       * This function is distinct from gdk_window_set_child_shapes()
       * because it includes #window’s shape mask in the set of shapes
       * to be merged.
       */
       ...
       * #GtkWindow widget anyway, instead of using GDK functions. For
       * child windows, the move will reliably succeed.
       *
      ...
      /**
       * Move the part of #window indicated by #region by #dy pixels
       * in the Y direction and #dx pixels in the X direction. The
       * portions of #region that not covered by the new position of
       ...
       * performed at once, avoiding strange visual effects. (i.e. the
       * user may be able to see the window first move, then resize,
       * if you don’t use gdk_window_move_resize().)
       *
       ...
       *
       * #rect is relative to the top-left corner of the window that
       * #window is transient for. #rect_anchor and #window_anchor
       * determine anchor points on #rect and #window to pin together.
       * #rect's anchor point can optionally be offset by
       * #rect_anchor_dx and #rect_anchor_dy, which is equivalent to
       ...
       * %GDK_GRAVITY_NORTH_EAST and vice versa if #window extends
       * beyond the left or right edges of the monitor.
       *
       * Connect to the {@link Window.moved_to_rect} signal to find out
       * how it was actually positioned.
       *
       * @param rect The destination {@link Rectangle} to align #window
       *   with
       * @param rect_anchor The point on #rect to align with #window's
       *   anchor point
       * @param window_anchor The point on #window to align with
       *   #rect's anchor point
       * @param anchor_hints Positioning hints to use when limited on
       *   space
       * @param rect_anchor_dx Horizontal offset to shift #window,
       *   i.e. #rect's anchor point
       * @param rect_anchor_dy Vertical offset to shift #window, i.e.
       *   #rect's anchor point
       */
      move_to_rect(
        rect: Rectangle,
       ...
       * expose event will cover the entire update area for the window
       * (see {@link Gdk.Window.invalidate_region} for details).
       * Normally GDK calls gdk_window_process_all_updates() on your
       * behalf, so there’s no need to call this function unless you
       * want to force expose events to be delivered immediately and
       * synchronously (vs. the usual case, where GDK delivers them in
       * an idle handler). Occasionally this is useful to produce
       ...
       * #window. This is true whether or not the windows are
       * visible.
       *
       * If #window is a toplevel, the window manager may choose to
       * deny the request to move the window in the Z-order,
       * {@link Gdk.Window.raise} only requests the restack, does not
       * guarantee it.
       */
      raise(): void;
       ...
       * order), so that it is above #sibling (if #above is %TRUE) or
       * below #sibling (if #above is %FALSE).
       *
       * If #sibling is %NULL, then this either raises (if #above is
       * %TRUE) or lowers the window.
       *
       * If #window is a toplevel, the window manager may choose to
       * deny the request to move the window in the Z-order,
       * {@link Gdk.Window.restack} only requests the restack, does not
       * guarantee it.
       *
       * @param sibling A {@link Window} that is a sibling of #window,
       *   or %NULL
       * @param above A boolean
       */
      restack(sibling: Window | null, above: boolean): void;
      /**
       * Scroll the contents of #window, both pixels and children, by
       * the given amount. #window itself does not move. Portions of
       * the window that the scroll operation brings in from offscreen
       * areas are invalidated. The invalidated region may be bigger
       * than what would strictly be necessary.
       *
       * For X11, a minimum area will be invalidated if the window has
       * no subwindows, or if the edges of the window’s parent do not
       * extend beyond the edges of the window. In other cases, a
       * multi-step process is used to scroll the window which may
       * produce temporary visual artifacts and unnecessary
       * invalidations.
       *
       ...
       * interpret this hint. ICCCM-compliant window manager usually
       * respect it.
       *
       * @param accept_focus %TRUE if the window should receive input
       *   focus
       */
       ...
       * shape masks for all children of #window, ignoring the input
       * shape mask of #window itself. Contrast with
       * {@link Gdk.Window.merge_child_input_shapes} which includes the
       ...
       * {@link Gdk.Window.merge_child_shapes} which includes the shape
       ...
       *   and an expose event is emitted on the parent of the
       *   composited window. It is the responsibility of the parent’s
       *   expose handler to manually merge the off-screen content
       *   onto the screen in whatever way it sees fit.
       *
       *   It only makes sense for child windows to be composited; see
       *   {@link Gdk.Window.set_opacity} if you need translucent
       *   toplevel windows.
       *
       *   An additional effect of this call is that the area of this
       *   window is no longer clipped from regions marked for
       *   invalidation on its parent. Draws done on the parent window
       *   are also no longer clipped by the child.
       *
       *   This call is only supported on some systems (currently, only
       *   X11 with new enough Xcomposite and Xdamage extensions). You
       *   must call gdk_display_supports_composite() to check if
       ...
       * that #window will use the cursor of its parent window. Most
       ...
       * toplevel {@link Window}. This function sets the traditional
       * Motif window manager hints that tell the window manager which
       * decorations you would like your window to have. Usually you
       * should use {@link Gtk.Window.set_decorated} on a #GtkWindow
       * instead of using the GDK function directly.
       *
       * The #decorations argument is the logical OR of the fields in
       * the #GdkWMDecoration enumeration. If #GDK_DECOR_ALL is
       ...
       * should be turned on.
       *
       * Most window managers honor a decorations hint of 0 to disable
       * all decorations, but very few honor all possible combinations
       * of bits.
       *
      ...
      /**
       * Sets a specific {@link Cursor} for a given device when it gets
       * inside #window. Use {@link Gdk.Cursor.new_for_display} or
       * gdk_cursor_new_from_pixbuf() to create the cursor. To make
       * the cursor invisible, use %GDK_BLANK_CURSOR. Passing %NULL
       * for the #cursor argument to gdk_window_set_cursor() means
       * that #window will use the cursor of its parent window. Most
      ...
      /**
       * Sets the event mask for a given device (Normally a floating
       * device, not attached to any visible pointer) to #window. For
       * example, an event mask including #GDK_BUTTON_PRESS_MASK means
       * the window should report button press events. The event mask
       * is the bitwise OR of values from the {@link EventMask}
       * enumeration.
       *
       ...
       * the event queue can be discarded. If %TRUE only the most
       * recent event will be delivered.
       *
       * Some types of applications, e.g. paint programs, need to see
       * all motion events and will benefit from turning off event
       * compression.
       *
       ...
       * reported for that window from all master input devices. For
       * example, an event mask including #GDK_BUTTON_PRESS_MASK means
       * the window should report button press events. The event mask
       * is the bitwise OR of values from the {@link EventMask}
       * enumeration.
      ...
      /**
       * Setting #focus_on_map to %FALSE hints the desktop environment
       * that the window doesn’t want to receive input focus when it
       * is mapped. focus_on_map should be turned off for windows that
       * aren’t triggered interactively (such as popups from network
       * activity).
       *
       * On X, it is the responsibility of the window manager to
       * interpret this hint. Window managers following the
       * freedesktop.org window manager extension specification should
       * respect it.
       *
       * @param focus_on_map %TRUE if the window should receive input
       *   focus when mapped
       */
      set_focus_on_map(focus_on_map: boolean): void;
      /**
       * Specifies whether the #window should span over all monitors
       * (in a multi-head setup) or only the current monitor when in
       * fullscreen mode.
       *
       * The #mode argument is from the {@link FullscreenMode}
       * enumeration. If #GDK_FULLSCREEN_ON_ALL_MONITORS is specified,
       * the fullscreen #window will span over all monitors from the
       * #GdkScreen.
       *
       * On X11, searches through the list of monitors from the
       * #GdkScreen the ones which delimit the 4 edges of the entire
       * #GdkScreen and will ask the window manager to span the
       * #window over these monitors.
       *
       * If the XINERAMA extension is not available or not usable,
       * this function has no effect.
       *
       * Not all window managers support this, so you can’t rely on
       * the fullscreen window to span over the multiple monitors when
       ...
       * managers do anything reliable or interesting with this hint.
       * Many ignore it entirely.
       *
       * The #functions argument is the logical OR of values from the
       * {@link WMFunction} enumeration. If the bitmask includes
       * #GDK_FUNC_ALL, then the other bits indicate which functions
       * to disable; if it doesn’t include #GDK_FUNC_ALL, it indicates
       ...
       * acceptable sizes for a toplevel window. The purpose of this
       * is to constrain user resizing, but the windowing system will
       * typically (but is not required to) also constrain the current
       * size of the window to the provided values and constrain
       * programatic resizing via {@link Gdk.Window.resize} or
       * gdk_window_move_resize().
       *
       * Note that on X11, this effect has no effect on windows of
       * type %GDK_WINDOW_TEMP or windows where override redirect has
       * been turned on via gdk_window_set_override_redirect() since
       * these windows are not resizable by the user.
       *
       ...
       * window implicitly created by GDK. With this function you can
       * override this default.
       *
       * The group leader window allows the window manager to
       * distinguish all windows that belong to a single application.
       * It may for example allow users to minimize/unminimize all
       * windows belonging to an application at once. You should only
       * set a non-default group window if your application pretends
       ...
       * used to represent the window when it has been iconified. The
       * icon is usually shown in an icon box or some sort of task
       * bar. Which icon size is shown depends on the window manager.
       * The window manager can scale the icon but setting several
       * size icons can give better image quality since the window
       * manager may only need to scale the icon by a small amount or
       * not at all.
       *
      ...
      /**
       * Windows may have a name used while minimized, distinct from
       * the name they display in their titlebar. Most of the time
       * this is a bad idea from a user interface standpoint. But you
       * can set such a name with this function, if you like.
       *
       * After calling this with a non-%NULL #name, calls to
       * {@link Gdk.Window.set_title} will not update the icon title.
       *
       * Using %NULL for #name unsets the icon title; further calls to
       * gdk_window_set_title() will again update the icon title as
       * well.
       *
       * Note that some platforms don't support window icons.
       *
       ...
       * will get called whenever a region in the window or its
       ...
       *
       * @param handler A {@link WindowInvalidateHandlerFunc} callback
       *   function
       */
       ...
       *
       * On X11, asks the window manager to keep #window above, if the
       * window manager supports this operation. Not all window
       * managers support this, and some deliberately ignore it or
       ...
       *
       * On X11, asks the window manager to keep #window below, if the
       * window manager supports this operation. Not all window
       * managers support this, and some deliberately ignore it or
      ...
      /**
       * Set #window to render as partially transparent, with opacity
       * 0 being fully transparent and 1 fully opaque. (Values of the
       * opacity parameter are clamped to the [0,1] range.)
       *
       * For toplevel windows this depends on support from the
       * windowing system that may not always be there. For instance,
       * On X11, this works only on X screens with a compositing
       * manager running. On Wayland, there is no per-window opacity
       * value that the compositor would apply. Instead, use
       * `gdk_window_set_opaque_region (window, NULL)` to tell the
       * compositor that the entire window is (potentially)
       * non-opaque, and draw your content with alpha, or use
       * {@link Gtk.Widget.set_opacity} to set an overall opacity for
       * your widgets.
       *
       ...
       * blending during for these regions. With RGB windows with no
       * transparency, this is just the shape of the window, but with
       * ARGB32 windows, the compositor does not know what regions of
       * the window are transparent or not.
       *
       * This function only works for toplevel windows.
       *
       * GTK+ will update this property automatically if the #window
       * background is opaque, as we know where the opaque regions
       * are. If your window background is not opaque, please update
       * this property in your #GtkWidget::style-updated handler.
       *
       * @param region A region, or %NULL
       */
      set_opaque_region(region: cairo.Region | null): void;
      /**
       * An override redirect window is not under the control of the
       * window manager. This means it won’t have a titlebar, won’t be
       * minimizable, etc. - it will be entirely under the control of
       * the application. The window manager can’t see the override
       ...
       * override redirect window in its implementation, for example.
       *
       * @param override_redirect %TRUE if window should be override
       *   redirect
       */
       ...
       * events that happen inside the window are send first to the
       * window, but if the event is not selected by the event mask
       * then the event is sent to the parent window, and so on up the
       * hierarchy.
       *
       * If #pass_through is %TRUE then such pointer events happen as
       * if the window wasn't there at all, and thus will be sent
       * first to any windows below #window. This is useful if the
       * window is used in a transparent fashion. In the terminology
       * of the web this would be called "pointer-events: none".
       *
       * Note that a window with #pass_through %TRUE can still have a
       * subwindow without pass through, so you can get events on a
       * subset of a window. And in that cases you would get the
       ...
       * saved in a previous session, all windows with the same title
       * and role are treated as interchangeable. So if you have two
       * windows with the same title that should be distinguished for
       * session management purposes, you should set the role on those
       * windows. It doesn’t matter what string you use for the role,
       * as long as you have a different role for each
       ...
       * the actual frame lie, so that users don’t feel like windows
       * are snapping against random invisible edges.
       *
       * Note that this property is automatically updated by GTK+, so
       * this function should only be used by applications which do
       * not use GTK+ to create toplevel windows.
       *
       ...
       * switcher, or other desktop utility program that displays a
       * small thumbnail representation of the windows on the
       * desktop). If a window’s semantic type as specified with
       * {@link Gdk.Window.set_type_hint} already fully describes the
       * window, this function should not be called in addition,
       * instead you should allow the window to be treated according
       ...
       * attached to any visible pointer) that has the source defined
       * as #source. This event mask will be applied both to currently
       * existing, newly added devices after this call, and devices
      ...
      /**
       * @deprecated Static gravities haven't worked on anything but
       *   X11 for a long time.
       *
       *   Used to set the bit gravity of the given window to static,
       *   and flag it so all children get static subwindow gravity.
       *   This is used if you are implementing scary features that
       *   involve deep knowledge of the windowing system. Don’t worry
       *   about it.
       ...
       * titlebar. If you haven’t explicitly set the icon name for the
       * window (using {@link Gdk.Window.set_icon_name}), the icon name
       * will be set to #title as well. #title must be in UTF-8
       ...
       * window manager can use this information when determining the
       * decoration and behaviour of the window.
       *
       ...
       * {@link GObject.set_data}. However, for historical reasons GTK+
       * stores the #GtkWidget that owns a {@link Window} as user data
       * on the #GdkWindow. So, custom widget implementations should
       * use this function for that. If GTK+ receives an event for a
       * #GdkWindow, and the user data for the window is non-%NULL,
       ...
       * widely available on most common platforms, but not available
       * on very old X servers, and occasionally the implementation
       * will be buggy. On servers without the shape extension, this
       ...
       *
       * When implementing a #GtkWidget, you should call this function
       * on the widget's {@link Window} as part of the “map” method.
       */
      show(): void;
      /**
       ...
       * XMapWindow() (it also updates some internal GDK state, which
       * means that you can’t really use XMapWindow() directly on a
       * GDK window).
       */
      show_unraised(): void;
      /**
       * Asks the windowing system to show the window menu. The window
       * menu is the menu shown when right-clicking the titlebar on
       * traditional windows managed by the window manager. This is
       * useful for windows using client-side decorations, activating
       * it with a right-click on the window decorations.
       *
       ...
       * scroll with viewports, for window managers that have
       * scrollable viewports. (When using #GtkWindow,
       * {@link Gtk.Window.stick} may be more useful.)
       *
       * On the X11 platform, this function depends on window manager
       * support, so may have no effect with many window managers.
       * However, GDK will do the best it can to convince the window
       * manager to stick the window. For window managers that don’t
       * support this operation, there’s nothing you can do to force
       * it to happen.
       */
      stick(): void;
      /**
       ...
       * operation. Not all window managers support this, and some
       * deliberately ignore it or don’t have a concept of
       * “fullscreen”; so you can’t rely on the unfullscreenification
       * actually happening. But it will happen with most standard
       * window managers, and GDK makes a best effort to get it to
       * happen.
       */
      unfullscreen(): void;
      /**
       * Unmaximizes the window. If the window wasn’t maximized, then
       * this function does nothing.
       *
       ...
       * managers support this, and some deliberately ignore it or
       * don’t have a concept of “maximized”; so you can’t rely on the
       * unmaximization actually happening. But it will happen with
       * most standard window managers, and GDK makes a best effort to
       * get it to happen.
       *
       ...
       * forget about it). This function is not really useful as
       ...
       * non-%NULL surface will stop any further signal emission, and
       * its surface will be used.
       *
       * Note that it is not possible to access the window's previous
       * surface from within any callback of this signal. Calling
       * {@link Gdk.offscreen.window_get_surface} will lead to a
       * crash.
       *
       * @param signal
       * @param callback Callback function
       ...
       *   - Returns the newly created #cairo_surface_t for the offscreen
       *       window
       *
       * @returns Callback ID
       */
       ...
       *   - Returns return location for the x coordinate in the offscreen
       *       window
       *
      ...
      /**
       * Emitted when the position of #window is finalized after being
       * moved to a destination rectangle.
       *
       ...
       * #final_rect is #flipped_rect, but possibly translated in the
       * case that flipping is still ineffective in keeping #window
       * on-screen.
       *
       * @param signal
       * @param callback Callback function
       ...
       *   - Returns the {@link Window} of the embedded child at #x, #y, or
       *       %NULL
       *
       * @returns Callback ID
       */
       ...
       *   - Returns return location for the x coordinate in the embedder
       *       window
       *
    ...
    class Window {
       ...
       * #attributes. See #GdkWindowAttr and #GdkWindowAttributesType
       * for more details. Note: to use this on displays other than
       * the default display, #parent must be specified.
       *
       * @param parent A {@link Window}, or %NULL to create the window
       ...
       *   %NULL if the window under the mouse pointer is not known to
       *   GDK (if the window belongs to another application and a
       *   {@link Window} hasn’t been created for it with
       ...
       * @param flags A mask indicating what portions of #geometry are
       *   set
      ...
      /**
       * Calls {@link Gdk.Window.process_updates} for all windows (see
       * {@link Window}) in the application.
       */
       ...
       * gdk_window_process_updates(). The net effect is that you can
       * see the invalid region for each window and watch redraws as
       * they occur. This allows you to diagnose inefficiencies in
       * your application.
       *
       * In essence, because the GDK rendering model prevents all
       * flicker, if you are redrawing the same region 400 times you
       * may never notice, aside from noticing a speed problem.
       * Enabling update debugging causes GTK to flicker slowly and
       * noticeably, so you can see exactly what’s being redrawn when,
       * in what order.
       *
    ...
    class Atom {
       ...
       *   corresponding to #atom. When you are done with the return
       *   value, you should free it using {@link G.free}.
       */
      public name(): string;
    }
    ...
    class Color {
      ...
      /**
       * For allocated colors, the pixel value used to draw this color
       * on the screen. Not used anymore.
       */
      public pixel: number;
      /**
    ...
    /**
     * Contains the fields which are common to all event structs. Any
     ...
     * press, while the second click results in a %GDK_2BUTTON_PRESS
     * being received just after the %GDK_BUTTON_PRESS.
     *
     * Triple-clicks are very similar to double-clicks, except that
     * %GDK_3BUTTON_PRESS is inserted after the third click. The order
     * of the events is:
     *
     * - %GDK_BUTTON_PRESS
     * - %GDK_BUTTON_RELEASE
     ...
     * within 1/4 of a second of the first. For a triple click to
     * occur, the third button press must also occur within 1/2 second
    ...
    class EventButton {
      ...
      /**
       * A bit-mask representing the state of the modifier keys (e.g.
       * Control, Shift and Alt) and the pointer buttons. See
       * {@link ModifierType}.
       *
      ...
      /**
       * The button which was pressed or released, numbered from 1 to
       * 5. Normally button 1 is the left mouse button, 2 is the
    ...
    class EventExpose {
       ...
       * handling all contiguous %GDK_EXPOSE events in one go, though
       * GDK performs some exposure compression so this is not
       * normally needed.
       */
      public count: number;
    }
     ...
     * this happens when the grab window becomes unviewable (i.e. it
     * or one of its ancestors is unmapped), or if the same
     * application grabs the pointer or keyboard again. Note that
     * implicit grabs (which are initiated by button presses) can also
     * cause {@link EventGrabBroken} events.
     */
    ...
    class EventGrabBroken {
      ...
      public send_event: number;
      /**
       * %TRUE if a keyboard grab was broken, %FALSE if a pointer grab
       * was broken
       */
    ...
    class EventMotion {
      ...
      public y: number;
      /**
       * #x, #y translated to the axes of #device, or %NULL if #device
       * is the mouse.
       */
      public axes: number;
      /**
     ...
     * information is only available if the X server supports the
     * XFIXES extension.
     */
    ...
    class EventOwnerChange {
      ...
      public owner: Window;
      /**
       * The reason for the ownership change as a {@link OwnerChange}
       * value
       */
    ...
    class EventPadButton {
      ...
      public time: number;
      /**
       * The pad group the button belongs to. A %GDK_SOURCE_TABLET_PAD
     ...
     * XInput extension. The XInput extension is an add-on for
     ...
     * has moved in or out of contact with the tablet, or perhaps that
     * the user’s finger has moved in or out of contact with a touch
     * screen.
     *
     ...
     * mice are usually configured to generate button press events for
     * buttons 4 and 5 when the wheel is turned.
     *
     ...
     * {@link Gdk.event.get_event_sequence}. Each sequence begins with
     * a %GDK_TOUCH_BEGIN event, followed by any number of
     * %GDK_TOUCH_UPDATE events, and ends with a %GDK_TOUCH_END (or
     * %GDK_TOUCH_CANCEL) event. With multitouch devices, there may be
     ...
     * #GdkFrameTimings objects, use {@link Gdk.FrameClock.get_timings}
     * or gdk_frame_clock_get_current_timings(). The information in
     * #GdkFrameTimings is useful for precise synchronization of video
     * with the event or audio streams, and for measuring quality
     * metrics for the application’s display, such as latency and
     * jitter.
     */
    ...
    class FrameTimings {
       ...
       * available at all. Once {@link Gdk.FrameTimings.get_complete}
       * returns %TRUE for a frame, you can be certain that no further
       * values will become available and be stored in the
       * #GdkFrameTimings.
       *
       * @returns %TRUE if all information that will be available for
       *   the frame has been filled in.
       */
       ...
       * {@link Gdk.FrameTimings.get_presentation_time}, which is only
       * available after the frame has been presented. In general, if
       * you are simply animating, you should use
       * gdk_frame_clock_get_frame_time() rather than this function,
       * but this function is useful for applications that want exact
       * control over latency. For example, a movie player may want
       * this information for Audio/Video synchronization.
       *
       * @returns The predicted time at which the frame will be
       *   presented, in the timescale of {@link G.get_monotonic_time},
       *   or 0 if no predicted presentation time is available.
       */
      public get_predicted_presentation_time(): number;
      /**
       * Reurns the presentation time. This is the time at which the
       * frame became visible to the user.
       *
       * @returns The time the frame was displayed to the user, in the
       *   timescale of {@link G.get_monotonic_time}, or 0 if no
       *   presentation time is available. See
       *   gdk_frame_timings_get_complete()
       */
      public get_presentation_time(): number;
      /**
       * Gets the natural interval between presentation times for the
       * display that this frame was displayed on. Frame presentation
      ...
      /**
       * Decreases the reference count of #timings. If #timings is no
    ...
    /**
     * The {@link Geometry} struct gives the window manager information
     * about a window’s geometry constraints. Normally you would set
     * these on the GTK+ level using
     * {@link Gtk.Window.set_geometry_hints}. #GtkWindow then sets the
     * hints on the #GdkWindow it creates.
     *
     * Gdk_window_set_geometry_hints() expects the hints to be fully
     * valid already and simply passes them to the window manager; in
     * contrast, gtk_window_set_geometry_hints() performs some
     * interpretation. For example, #GtkWindow will apply the hints to
     * the geometry widget instead of the toplevel window, if you set
     * a geometry widget. Also, the
     * #min_width/#min_height/#max_width/#max_height fields may be set
     * to -1, and #GtkWindow will substitute the size request of the
     * window or geometry widget. If the minimum size hint is not
     * provided, #GtkWindow will use its requisition as the minimum
     * size. If the minimum size is provided and a geometry widget is
     * set, #GtkWindow will take the minimum size as the minimum size
     * of the geometry widget rather than the entire window. The base
     * size is treated similarly.
     *
     * The canonical use-case for gtk_window_set_geometry_hints() is
     * to get a terminal widget to resize properly. Here, the terminal
     * text area should be the geometry widget; #GtkWindow will then
     * automatically set the base size to the size of other widgets in
     * the terminal window, such as the menubar and scrollbar. Then,
     * the #width_inc and #height_inc fields should be set to the size
     * of one character in the terminal. Finally, the base size should
     * be set to the size of one character. The net effect is that the
     * minimum size of the terminal will have a 1x1 character terminal
     * area, and only terminal sizes on the “character grid” will be
     * allowed.
     *
     * Here’s an example of how the terminal example would be
     * implemented, assuming a terminal area widget called “terminal”
     * and a toplevel window “toplevel”:
     *
     * |[<!-- language="C" -->
     *
     *     GdkGeometry hints;
     ...
     * fields; these contain a width/height ratio as a floating point
     * number. If a geometry widget is set, the aspect applies to the
     * geometry widget rather than the entire window. The most common
     * use of these hints is probably to set #min_aspect and
     * #max_aspect to the same value, thus forcing the window to keep
    ...
    class Geometry {
      ...
      /**
       * Allowed window widths are #base_width + #width_inc * N where
    ...
    class KeymapKey {
      ...
      /**
       * Indicates movement in a horizontal direction. Usually groups
       * are used for two different languages. In group 0, a key might
       * have two English characters, and in group 1 it might have two
       * Hebrew characters. The Hebrew characters will be printed on
       * the key next to the English characters.
       */
      public group: number;
      /**
       * Indicates which symbol on the key will be used, in a vertical
       * direction. So on a standard US keyboard, the key with the
       * number “1” on it also has the exclamation point ("!")
       * character on it. The level indicates whether to use the “1”
       * or the “!” symbol. The letter keys are considered to have a
       * lowercase letter at level 0, and an uppercase letter at level
       * 1, though only the uppercase letter is printed.
       */
      public level: number;
    }
    ...
    class RGBA {
      ...
      public blue: number;
      /**
       * The opacity of the color from 0.0 for completely translucent
       * to 1.0 for opaque
       */
      public alpha: number;
      /**
       ...
       * and “b” are either integers in the range 0 to 255 or
       * percentage values in the range 0% to 100%, and a is a
       ...
       * `rgb(r,g,b)` or `rgba(r g,b,a)`, where “r”, “g”, “b” and “a”
       * represent the red, green, blue and alpha values respectively.
       * “r”, “g”, and “b” are represented as integers in the range 0
       * to 255, and “a” is represented as a floating point value in
       * the range 0 to 1.
       *
       * These string forms are string forms that are supported by the
       * CSS3 colors module, and can be parsed by
       * {@link Gdk.RGBA.parse}.
       *
       * Note that this string representation may lose some precision,
       * since “r”, “g” and “b” are represented as 8-bit integers. If
       * this is a concern, you should use a different
       * representation.
       *
       * @returns A newly allocated text string
       */
    ...
    /**
     * Defines the position and size of a rectangle. It is identical
     * to #cairo_rectangle_int_t.
     */
    interface Rectangle {}
    class Rectangle {
       ...
       * for #dest to be the same as either #src1 or #src2. If the
       * rectangles do not intersect, #dest’s width and height is set
       * to 0 and its x and y values are undefined. If you are only
       * interested in whether the rectangles intersect, but not in
       * the intersecting area itself, pass %NULL for #dest.
       *
       * @param src2 A {@link Rectangle}
       * @returns %TRUE if the rectangles intersect.
       *
       *   Return location for the intersection of #src1 and #src2, or
       *   %NULL
       */
    ...
    interface IDevicePad {
      /**
       * Returns the group the given #feature and #idx belong to, or
       * -1 if feature/index do not exist in #pad.
       *
       * @param feature The feature type to get the group from
       * @param feature_idx The index of the feature to get the group
       *   from
       ...
       *
       * @param group_idx Group to get the number of available modes
       *   from
     ...
     * %GDK_SOURCE_TABLET_PAD, it allows querying the features
     * provided by the pad device.
     *
     * Tablet pads may contain one or more groups, each containing a
     * subset of the buttons/rings/strips available.
     * {@link Gdk.DevicePad.get_n_groups} can be used to obtain the
     * number of groups, gdk_device_pad_get_n_features() and
     * gdk_device_pad_get_feature_group() can be combined to find out
     * the number of buttons/rings/strips the device has, and how are
     * they grouped.
     *
     * Each of those groups have different modes, which may be used to
     * map each individual pad feature to multiple actions. Only one
     * mode is effective (current) for each given group, different
     * groups may have different current modes. The number of
     * available modes in a group can be found out through
     * gdk_device_pad_get_group_n_modes(), and the current mode for a
     * given group will be notified through the #GdkEventPadGroupMode
     * event.
     */
     ...
     * regardless. Whether X and Y are present as axes depends on the
     * GDK backend.
     */
    enum AxisUse {
      /** The axis is ignored. */
      ...
      WHEEL = 6,
      /**
       * The axis is used for pen/tablet distance information. (Since:
       * 3.22)
       */
      DISTANCE = 7,
    ...
    /**
     * A set of values describing the possible byte-orders for storing
     * pixel values in memory.
     */
    enum ByteOrder {
      /**
       * The values are stored with the least-significant byte first.
       * For instance, the 32-bit value 0xffeecc would be stored in
    ...
    enum CrossingMode {
       ...
       * control of the pointer after a touch device), this event is
     ...
     * Predefined cursors.
     *
     * Note that these IDs are directly taken from the X cursor font,
     * and many of these cursors are either not useful, or are not
    ...
    /**
     * Used in {@link DragContext} to indicate the protocol according
    ...
    enum DragProtocol {
      ...
      XDND = 2,
      /**
       * An extension to the Xdnd protocol for unclaimed root window
       * drops.
       */
      ROOTWIN = 3,
     ...
     * invalid (eg `Gdk.EventType.2ButtonPress`, where a symbol is not
     * allowed to start with a number). In that case, the aliases
     * %GDK_DOUBLE_BUTTON_PRESS and %GDK_TRIPLE_BUTTON_PRESS can be
     * used instead.
     */
    enum EventType {
       ...
       * hidden or destroyed, usually when the user clicks on a
       ...
       * a short period of time). Note that each click also generates
       ...
       * time. Note that each click also generates a %GDK_BUTTON_PRESS
       * event.
       */
      _3BUTTON_PRESS = 6,
      ...
      SCROLL = 31,
      /**
       * The state of a window has changed. See {@link WindowState} for
      ...
      DAMAGE = 36,
      /**
       * A new touch event sequence has just started. This event type
       * was added in 3.4.
       */
      TOUCH_BEGIN = 37,
      /**
       * A touch event sequence has been updated. This event type was
       * added in 3.4.
       */
      TOUCH_UPDATE = 38,
      /**
      ...
      TOUCH_END = 39,
      /**
       * A touch event sequence has been canceled. This event type was
       * added in 3.4.
       */
      TOUCH_CANCEL = 40,
      /**
       * A touchpad swipe gesture event, the current state is
       * determined by its phase field. This event type was added in
       * 3.18.
       */
      TOUCHPAD_SWIPE = 41,
      /**
      ...
      TOUCHPAD_PINCH = 42,
      /**
       * A tablet pad button press event. This event type was added in
       * 3.22.
       */
      PAD_BUTTON_PRESS = 43,
      /**
       * A tablet pad button release event. This event type was added
       * in 3.22.
       */
      ...
      PAD_STRIP = 46,
      /**
       * A tablet pad group mode change. This event type was added in
       * 3.22.
       */
      PAD_GROUP_MODE = 47,
    ...
    /**
     * Indicates which monitor (in a multi-head setup) a window should
     * span over when in fullscreen mode.
     */
    enum FullscreenMode {
     ...
     * gdk_keyboard_grab() to indicate success or the reason for the
     ...
     * gtk_window_move() and the "implementation notes" section of the
     * [Extended Window Manager
     * Hints](http://www.freedesktop.org/Standards/wm-spec)
    ...
    enum Gravity {
      ...
      SOUTH_EAST = 9,
      /**
       * The reference point is at the top left corner of the window
    ...
    enum InputMode {
      ...
      DISABLED = 0,
      /**
       * The device is enabled. The device’s coordinate space maps to
       * the entire screen.
       */
      SCREEN = 1,
      /**
       * The device is enabled. The device’s coordinate space is
       * mapped to a single window. The manner in which this window is
       * chosen is undefined, but it will typically be the same way in
    ...
    enum InputSource {
      ...
      TOUCHPAD = 6,
      /**
       * The device is a trackpoint. This device type has been added
       * in 3.22
       */
      TRACKPOINT = 7,
      /**
    ...
    /**
     * This enum is used with {@link Gdk.Keymap.get_modifier_mask} in
     * order to determine what modifiers the currently used windowing
     * system backend uses for particular purposes. For example, on
     * X11/Windows, the Control key is used for invoking menu
     * shortcuts (accelerators), whereas on Apple computers it’s the
     * Command key (which correspond to %GDK_CONTROL_MASK and
    ...
    enum ModifierIntent {
       ...
       * button 3 always triggers context menus. When this modifier is
       * not 0, it additionally triggers context menus when used with
       * mouse button 1.
       */
      CONTEXT_MENU = 1,
      /**
       * The modifier used to extend selections using `modifier`-click
       * or `modifier`-cursor-key
       */
      EXTEND_SELECTION = 2,
      /**
       * The modifier used to modify selections, which in most cases
       * means toggling the clicked item into or out of the
       * selection.
       */
      MODIFY_SELECTION = 3,
      /**
       * When any of these modifiers is pressed, the key event cannot
       * produce a symbol directly. This is meant to be used for input
    ...
    enum NotifyType {
      ...
      NONLINEAR = 3,
      /**
       * The pointer moves between two windows which are not ancestors
       * of each other and the window is part of the ancestor chain
       * between one of these windows and their least common
       * ancestor.
       */
      NONLINEAR_VIRTUAL = 4,
    ...
    enum ScrollDirection {
       ...
       * {@link EventScroll}. See {@link Gdk.event.get_scroll_deltas}.
       * Since: 3.4
       */
      SMOOTH = 4,
    }
    ...
    /**
     * Specifies the current state of a touchpad gesture. All gestures
     * are guaranteed to begin with an event with phase
     ...
     * phase %GDK_TOUCHPAD_GESTURE_PHASE_END will be emitted when the
     * gesture is considered successful, this should be used as the
     * hint to perform any permanent changes.
     *
     * Cancelled gestures may be so for a variety of reasons, due to
     * hardware or the compositor, or due to the gesture recognition
     * layers hinting the gesture did not finish resolutely (eg. a 3rd
     * finger being added during a pinch gesture). In these cases, the
     * last event will report the phase
     * %GDK_TOUCHPAD_GESTURE_PHASE_CANCEL, this should be used as a
     * hint to undo any visible/permanent changes that were done
    ...
    enum VisualType {
       ...
       * values into grayscale values. The color map can be changed by
       * an application.
       */
      GRAYSCALE = 1,
      /**
       ...
       * components. Use {@link Gdk.Visual.get_red_pixel_details}, etc,
       * to obtain information about how the components are assembled
       * into a pixel value.
       */
      TRUE_COLOR = 4,
      /**
    ...
    enum WindowType {
       ...
       * toplevel, and is moved/stacked with it. Currently this window
     ...
     * of function the window has. The window manager can use this
     * when determining decoration and behaviour of the window. The
    ...
    /**
     * #GDK_INPUT_OUTPUT windows are the standard kind of window you
     * might expect. Such windows receive events and are also
     * displayed on screen. #GDK_INPUT_ONLY windows are invisible;
     * they are usually placed above other windows in order to trap or
     * filter the events. You can’t draw on #GDK_INPUT_ONLY windows.
     */
     ...
     * the case that the window would fall off-screen if placed in its
     * ideal position.
     *
     * For example, %GDK_ANCHOR_FLIP_X will replace
     * %GDK_GRAVITY_NORTH_WEST with %GDK_GRAVITY_NORTH_EAST and vice
     * versa if the window extends beyond the left or right edges of
     * the monitor.
     *
     * If %GDK_ANCHOR_SLIDE_X is set, the window can be shifted
     * horizontally to fit on-screen. If %GDK_ANCHOR_RESIZE_X is set,
     * the window can be shrunken horizontally to fit.
     *
     * In general, when multiple flags are set, flipping should take
     * precedence over sliding, which should take precedence over
     * resizing.
     */
    enum AnchorHints {
     ...
     * marked as a hint (the is_hint member is %TRUE). To receive more
     * motion events after a motion hint event, the application needs
     * to asks for more, by calling {@link Gdk.event.request_motions}.
     *
     * Since GTK 3.8, motion events are already compressed by default,
     * independent of this mechanism. This compression can be disabled
     * with gdk_window_set_event_compression(). See the documentation
     * of that function for details.
     *
     * If %GDK_TOUCH_MASK is enabled, the window will receive touch
     * events from touch-enabled devices. Those will come as sequences
     * of #GdkEventTouch with type %GDK_TOUCH_UPDATE, enclosed by two
     * events with type %GDK_TOUCH_BEGIN and %GDK_TOUCH_END (or
     * %GDK_TOUCH_CANCEL). gdk_event_get_event_sequence() returns the
     * event sequence for these events, so different sequences may be
     * distinguished.
     */
    enum EventMask {
    ...
    /**
     * {@link FrameClockPhase} is used to represent the different paint
     * clock phases that can be requested. The elements of the
     * enumeration correspond to the signals of #GdkFrameClock.
     */
    ...
    /**
     * A set of bit-flags to indicate the state of modifier keys and
     * mouse buttons in various event types. Typical modifier keys are
     ...
     * keys are mapped to Mod2 - Mod5, and indicates this by setting
     * %GDK_SUPER_MASK, %GDK_HYPER_MASK or %GDK_META_MASK in the state
     * field of key events.
     *
     * Note that GDK may add internal values to events which include
     * reserved values such as %GDK_MODIFIER_RESERVED_13_MASK. Your
     * code should preserve and ignore them. You can use
     * %GDK_MODIFIER_MASK to remove all reserved values.
     *
     * Also note that the GDK X backend interprets button press events
     * for button 4-7 as scroll events, so %GDK_BUTTON4_MASK and
     * %GDK_BUTTON5_MASK will never be set.
     */
    enum ModifierType {
      /** The Shift key. */
      SHIFT_MASK = 1,
      ...
      MOD3_MASK = 32,
      /**
       * The seventh modifier key (it depends on the modifier mapping
       * of the X server which key is interpreted as this modifier).
       */
      MOD4_MASK = 64,
      /**
       * The eighth modifier key (it depends on the modifier mapping
       * of the X server which key is interpreted as this modifier).
       */
      MOD5_MASK = 128,
    ...
    /**
     * These are hints originally defined by the Motif toolkit. The
     * window manager can use them when determining the functions to
     * offer for the window. The hint must be set before mapping the
     * window.
     */
    enum WMFunction {
    ...
    /**
     * Used to indicate which fields in the {@link WindowAttr} struct
     * should be honored. For example, if you filled in the “cursor”
     * and “x” fields of #GdkWindowAttr, pass “#GDK_WA_X |
     * #GDK_WA_CURSOR” to {@link Gdk.Window.new}. Fields in
     * #GdkWindowAttr not covered by a bit in this enum are required;
     * for example, the #width/#height, #wclass, and #window_type
     ...
     * #GDK_HINT_POS, #GDK_HINT_USER_POS, and #GDK_HINT_USER_SIZE is
     * significant, though they don't directly refer to #GdkGeometry
     * fields. #GDK_HINT_USER_POS will be set automatically by
     * #GtkWindow if you call {@link Gtk.Window.move}.
     * #GDK_HINT_USER_POS and #GDK_HINT_USER_SIZE should be set if the
     * user specified a size/position using a --geometry command-line
     * argument; gtk_window_parse_geometry() automatically sets these
     * flags.
     */
    enum WindowHints {
     ...
     * `event->window`. The filter may translate the native event to a
     * GDK event and store the result in #event, or handle it without
     * translation. If the filter translates the event and processing
     * should continue, it should return %GDK_FILTER_TRANSLATE.
     */
     ...
     * {@link Gdk.Window.invalidate_maybe_recurse}. It gets called for
     * each child of the window to determine whether to recursively
     * invalidate it or now.
     */
     ...
     * the window or in a child window) this gets called with #region
     * in the coordinate space of #window. You can use #region to just
     * keep track of the dirty region, or you can actually change
     * #region in case you are doing display tricks like showing a
     * child in multiple places.
     */
    ...
    interface IEvent {
       ...
       * both coordinates (as in a straight line going from #event1 to
      ...
      /**
       * Copies a {@link Event}, copying or incrementing the reference
       * count of the resources associated with it (e.g. #GdkWindow’s
       * and strings).
       *
       * @returns A copy of #event. The returned {@link Event} should
       *   be freed with {@link Gdk.Event.free}.
       */
      copy(): Event;
      /**
       * Frees a {@link Event}, freeing or decrementing any resources
       * associated with it. Note that this function should only be
       * called with events returned from functions such as
       * {@link Gdk.event.peek}, gdk_event_get(), gdk_event_copy() and
       * gdk_event_new().
       */
      free(): void;
      /**
       ...
       * different tools (eg. a tablet), this function will return a
       * {@link DeviceTool} representing the tool that caused the
       * event. Otherwise, %NULL will be returned.
       *
       * Note: the #GdkDeviceTool<!-- -->s will be constant during the
       * application lifetime, if settings must be stored persistently
       * across runs, see {@link Gdk.DeviceTool.get_serial}
       *
       ...
       * {@link EventSequence} to which the event belongs. Otherwise,
       * return %NULL.
       *
       ...
       * 'emulated' pointer event (typically from a touch event), as
       ...
       * word of WM_KEY{DOWN,UP} lParam which contains the scancode
      ...
      get_scancode(): number;
      /**
       * Returns the screen for the event. The screen is typically the
       * screen for `event->any.window`, but for events such as mouse
       * events, it is the screen where the pointer was when the event
       * occurs - that is, the screen which has the root window to
       * which `event->motion.x_root` and `event->motion.y_root` are
       * relative.
       *
       ...
       * Handle discrete scrolling with a known constant delta; const
       * double delta = 12.0;
       *
       *       switch (direction)
       *         {
       ...
       *
       * @returns %TRUE if the event delivered a scroll direction and
       *   %FALSE otherwise
       *
      ...
      get_seat(): Seat;
      /**
       * This function returns the hardware (slave) {@link Device} that
       * has triggered the event, falling back to the virtual (master)
       * device (as in {@link Gdk.event.get_device}) if the event
       * wasn’t caused by interaction with a hardware device. This may
       * happen for example in synthesized crossing events after a
       * #GdkWindow updates its geometry or a grab is
       * acquired/released.
       *
       * If the event does not contain a device field, this function
       * will return %NULL.
       *
       * @returns A {@link Device}, or %NULL.
       */
       ...
       * #state. Otherwise stores an empty state (0). Returns %TRUE if
       * there was a state field in the event. #event may be %NULL, in
       * which case it’s treated as if the event had no state field.
       *
       * @returns %TRUE if there was a state field in the event
       *
      ...
      get_window(): Window;
      /**
       * Check whether a scroll event is a stop scroll event. Scroll
       * sequences with smooth scroll information may provide a stop
       * scroll event once the interaction with the device finishes,
       * e.g. by lifting a finger. This stop scroll event is the
       * signal that a widget may trigger kinetic scrolling based on
       ...
       * queue if event->any.window is %NULL. See
       ...
       * Additionally, if {@link Gdk.Keymap.get_modifier_mask} returns
       * a non-0 mask for %GDK_MODIFIER_INTENT_CONTEXT_MENU, then the
       * left mouse button will also trigger a context menu if this
       * modifier is pressed.
       *
    ...
    /**
     * A {@link Event} contains a union of all of the event types, and
     * allows access to the data fields in a number of ways.
     *
     * The event type is always the first field in all of the event
     * types, and can always be accessed with the following code, no
     ...
     * %GDK_BUTTON_PRESS then the x coordinate of the button press can
     * be accessed with:
     *
     * |[<!-- language="C" -->
     *
    ...
    class Event {
      ...
      /**
       * Creates a new event of the given type. All fields are set to
       * 0.
       *
       ...
       *
       * Note that GTK+ uses this to install its own event handler, so
       * it is usually not useful for GTK+ applications. (Although an
       * application can call this function then call
    ...
    /**
     * Appends gdk option entries to the passed in option group. This
     * is not public API and must not be used by applications.
     *
     ...
     * @param only_if_exists If %TRUE, GDK is allowed to not create a
     *   new atom, but just return %GDK_NONE if the requested atom
     *   doesn’t already exists. Currently, the flag is ignored, since
     *   checking the existance of an atom is as expensive as creating
     *   it.
     ...
     * Finds or creates an atom corresponding to a given string.
     *
     * Note that this function is identical to {@link Gdk.atom.intern}
     * except that if a new {@link Atom} is created the string itself
     * is used rather than a copy. This saves memory, but can only be
     * used if the string will always exist. It can be used with
     * statically allocated strings in the main program, but not with
     * statically allocated memory in dynamically loaded modules, if
     * you expect to ever unload the module again (e.g. do not use
     * this function in GTK+ theme engines).
     *
     ...
     * costs.
     *
     * Typically, this function is used to draw on a {@link Window} out
     * of the paint cycle of the toolkit; this should be avoided, as
     * it breaks various assumptions and optimizations.
     *
     * If you are drawing on a native #GdkWindow in response to a
     * %GDK_EXPOSE event you should use gdk_window_begin_draw_frame()
     * and gdk_drawing_context_get_cairo_context() instead. GTK will
     ...
     * texture id (#source_type == #GL_TEXTURE) and draws it onto #cr
     * with an OVER operation, respecting the current clip. The top
     * left corner of the rectangle specified by #x, #y, #width and
     * #height will be drawn at the current (0,0) position of the
     * cairo_t.
     *
     * This will work for _all_ cairo_t, as long as #window is
     * realized, but the fallback implementation that reads back the
     * pixels from the buffer may be used in the general case. In the
     * case of direct drawing to a window with no special effects
     * applied to #cr it will however use a more efficient approach.
     *
     * For #GL_RENDERBUFFER the code will always fall back to software
     * for buffers with alpha components, so make sure you use
     * #GL_TEXTURE if using alpha.
     *
     * Calling this may change the current GL context.
     *
     * @param cr A cairo context
     * @param window The window we're rendering for (not necessarily
     *   into)
     * @param source The GL ID of the source buffer
     * @param source_type The type of the #source
     * @param buffer_scale The scale-factor that the #source buffer is
     *   allocated for
     * @param x The source x position in #source to start copying from
     *   in GL coordinates
     * @param y The source y position in #source to start copying from
     *   in GL coordinates
     ...
     * integer coordinates and returns a boolean indicating if a clip
     * area exists.
     *
     * @param cr A cairo context
     * @returns %TRUE if a clip rectangle exists, %FALSE if all of #cr
     *   is clipped and all drawing can be skipped
     *
     ...
     * aligned so that the origin of #pixbuf is #pixbuf_x, #pixbuf_y.
     *
     * @param cr A cairo context
     ...
     *
     * @param pixbuf A {@link Pixbuf}
     * @param scale The scale of the new surface, or 0 to use same as
     *   #window
     ...
     * #green, and #blue fields of a {@link Color}.
     *
     ...
     * disable multidevice support if such Xlib code deals with input
     * devices in any way and doesn’t observe the presence of XInput
     * 2.
     */
     ...
     * drop operations. See {@link Gdk.DragContext.manage_dnd} for more
     * information.
     *
     ...
     * pointer device, use {@link Gdk.drag.begin_for_device} to begin a
     * drag with a different device.
     *
     * This function is called by the drag source.
     *
    ...
    /**
     * Inform GDK if the drop ended successfully. Passing %FALSE for
     * #success may trigger a drag cancellation animation.
     *
     * This function is called by the drag source, and should be the
     * last call before dropping the reference to the #context.
     *
     * The {@link DragContext} will only take the first
     * {@link Gdk.drag.drop_done} call as effective, if this function
     * is called multiple times, all subsequent calls will be
     * ignored.
     *
     * @param context A {@link DragContext}
     ...
     * May block until an error has been definitively received or not
     * received from the X server. gdk_error_trap_pop_ignored() is
     * preferred if you don’t need to know whether an error occurred,
     * because it never has to block. If you don't need the return
     * value of gdk_error_trap_pop(), use
    ...
    /**
     * Removes an error trap pushed with {@link Gdk.error.trap_push},
     * but without bothering to wait and see whether an error
     * occurred. If an error arrives later asynchronously that was
     * triggered while the trap was pushed, that error will be
     * ignored.
     */
     ...
     * normal behavior of exiting the application. It should only be
     * used if it is not possible to avoid the X error in any other
     * way. Errors are ignored on all {@link Display} currently known
     * to the #GdkDisplayManager. If you don’t care which error
     * happens and just want to ignore everything, pop with
     * {@link Gdk.error.trap_pop_ignored}. If you need the error code,
     * use gdk_error_trap_pop() which may have to block and wait for
     * the error to arrive from the X server.
     *
     ...
     * necessary. See {@link Gdk.Display.get_event}.
     *
     * @returns The next {@link Event} to be processed, or %NULL if no
     *   events are pending. The returned #GdkEvent should be freed
     ...
     *   removed, i.e. when {@link Gdk.event.handler_set} is called
     ...
     * {@link Gdk.Display.peek_event}.
     *
     * @returns A copy of the first {@link Event} on some event queue,
     *   or %NULL if no events are in any queues. The returned
     *   #GdkEvent should be freed with {@link Gdk.Event.free}.
     */
     ...
     * notifies, because it also works for extension events where
     * motion notifies are provided for devices other than the core
     * pointer. Coordinate extraction, processing and requesting more
     * motion events from a %GDK_MOTION_NOTIFY event usually works
     * like this:
     *
     * |[<!-- language="C" -->
     *
     ...
     * If both events contain X/Y information, this function will
     * return %TRUE and return in #angle the relative angle from
     * #event1 to #event2. The rotation direction for positive angles
     * is from the positive X axis towards the positive Y axis.
     *
     ...
     * coordinates (as in a straight line going from #event1 to
     * #event2) will be returned.
     *
     * @param event1 First {@link Event}
     * @param event2 Second {@link Event}
     ...
     * until all requests have been processed. This is rarely needed
     * by applications.
     */
    function flush(): void;
     ...
     * `DISPLAY` environment variable or the `--display` command line
     * option.
     *
    ...
    /**
     * Gets the display name specified in the command line arguments
     * passed to gdk_init() or gdk_parse_args(), if any.
     *
     * @returns The display name, if specified explicitly, otherwise
     *   %NULL this string is owned by GTK+ and must not be modified
     *   or freed.
     */
     ...
     * been set with {@link Gdk.set.program_class} or with the
     * `--class` commandline option, the default value is the program
     * name (determined with g_get_prgname()) with the first character
     ...
     *
     * GTK+ initializes GDK in gtk_init() and so this function is not
     * usually needed by GTK+ applications.
     */
    function init(): void;
     ...
     *
     * If you set up anything at the time you take the grab that needs
     * to be cleaned up when the grab ends, you should handle the
     * {@link EventGrabBroken} events that are emitted when the grab
     * ends unvoluntarily.
     *
     * @param window The {@link Window} which will own the grab (the
     *   grab window).
     ...
     *   respect to #window. Both key press and key release events are
     *   always reported, independant of the event mask set by the
     *   application.
    ...
    /**
     * Ungrabs the keyboard on the default display, if it is grabbed
     * by this application.
     *
     * @param time_ A timestamp from a {@link Event}, or
     *   %GDK_CURRENT_TIME if no timestamp is available.
     ...
     * @returns The corresponding key value, or %GDK_KEY_VoidSymbol if
     ...
     *
     * @param keyval A key value
     * @returns A string containing the name of the key, or %NULL if
     *   #keyval is not a valid key. The string should not be
     *   modified.
     */
     ...
     *
     * @param keyval A key value.
     * @returns The lower case form of #keyval, or #keyval itself if
     *   it is already in lower case or it is not subject to case
     *   conversion.
     */
     ...
     * @returns The corresponding unicode character, or 0 if there is
     ...
     *
     * @param keyval A key value.
     * @returns The upper case form of #keyval, or #keyval itself if
     *   it is already in upper case or it is not subject to case
     *   conversion.
     */
     ...
     * {@link Gdk.Screen.list_visuals}) A visual describes a hardware
     * image data format. For example, a visual might support 24-bit
     * color, or 8-bit color, and might expect pixels to be in a
     * certain format.
     *
     * Call g_list_free() on the return value when you’re finished
     * with it.
     *
     * @returns A list of visuals; the list must be freed, but not its
     *   contents
     */
     ...
     * into. If you need to keep this around over window resizes, you
     ...
     * {@link Window.pick_embedded_child} signal on the #embedder and
     * the #GdkWindow::to-embedder and #GdkWindow::from-embedder
     * signals on #window.
     *
     * @param window A {@link Window}
     ...
     * {@link Gtk.Widget.get_pango_context} instead of this function,
     * to get the appropriate context for the widget you intend to
     * render text onto.
     *
     * The newly created context will have the default font options
     * (see #cairo_font_options_t) for the default screen; if these
     * options change it will not be updated. Using
     * gtk_widget_get_pango_context() is more convenient if you want
     * to keep a context around and track changes to the screen’s font
     * rendering settings.
     *
     ...
     * gtk_widget_get_pango_context() is more convenient if you want
     * to keep a context around and track changes to the font
     * rendering settings.
     *
     * @param display The {@link Display} for which the context is to
     *   be created
     ...
     * rendering settings.
     *
     * @param screen The {@link Screen} for which the context is to be
     *   created.
     ...
     * ranges of text would be drawn. #x_origin and #y_origin are the
     * top left point to center the layout. #index_ranges should
     * contain ranges of bytes in the layout’s text.
     *
     * Note that the regions returned correspond to logical extents of
     * the text ranges, not ink extents. So the drawn layout may in
     * fact touch areas out of the clip region. The clip region is
     * mainly useful for highlightling parts of text, such as when
     * text is selected.
     *
     ...
     * Obtains a clip region which contains the areas where the given
     * ranges of text would be drawn. #x_origin and #y_origin are the
     * top left position of the layout. #index_ranges should contain
     * ranges of bytes in the layout’s text. The clip region will
     * include space to the left or right of the line (to the layout
     * bounding box) if you have indexes above or below the indexes
     * contained inside the line. This is to draw the selection all
     * the way to the side of the layout. However, the clip region is
     * in line coordinates, not layout coordinates.
     *
     * Note that the regions returned correspond to logical extents of
    ...
    /**
     * Parse command line arguments, and store for future use by calls
     * to {@link Gdk.Display.open}.
     *
     * Any arguments used by GDK are removed from the array and #argc
     * and #argv are updated accordingly.
     *
    ...
    /**
     * Transfers image data from a #cairo_surface_t and converts it to
     * an RGB(A) representation inside a {@link Pixbuf}. This allows
     * you to efficiently read individual pixels from cairo surfaces.
     * For #GdkWindows, use {@link GdkPixbuf.get_from_window} instead.
     *
    ...
    /**
     * Transfers image data from a {@link Window} and converts it to an
     * RGB(A) representation inside a #GdkPixbuf. In other words,
     * copies image data from a server-side drawable to a client-side
     * RGB(A) buffer. This allows you to efficiently read individual
     * pixels on the client side.
     *
     * This function will create an RGB pixbuf with 8 bits per channel
     * with the size specified by the #width and #height arguments
     * scaled by the scale factor of #window. The pixbuf will contain
     * an alpha channel if the #window contains one.
     *
     * If the window is off the screen, then there is no image data in
     * the obscured/offscreen regions to be placed in the pixbuf. The
     * contents of portions of the pixbuf corresponding to the
     * offscreen region are undefined.
     *
     * If the window you’re obtaining data from is partially obscured
     * by other windows, then the contents of the pixbuf areas
     ...
     *
     * If memory can’t be allocated for the return value, %NULL will
     * be returned instead.
     *
     * (In short, there are several ways this function can fail, and
     * if it fails it returns %NULL; so check the return value.)
     *
     ...
     * {@link Gdk.pointer.ungrab}, or the grab window becomes
     ...
     * application. For example in GTK+ it is used for Drag and Drop,
     * for dragging the handle in the #GtkHPaned and #GtkVPaned
     * widgets.
     *
     * Note that if the event mask of an X window has selected both
     * button press and button release events, then a button press
     * event will cause an automatic pointer grab until the button is
     * released. X does this automatically since most applications
     * expect to receive button press and release events in pairs. It
     * is equivalent to a pointer grab on the window with
     ...
     *   and only if selected by #event_mask. In either mode,
     *   unreported events are discarded.
     * @param event_mask Specifies the event mask, which is used in
     *   accordance with #owner_events. Note that only pointer events
     *   (i.e. button and motion events) may be selected.
     * @param confine_to If non-%NULL, the pointer will be confined to
     *   this window during the grab. If the pointer is outside
     ...
     * @param cursor The cursor to display while the grab is active.
     *   If this is %NULL then the normal cursors are used for #window
     *   and its descendants, and the cursor for #window is used for
     *   all other windows.
     * @param time_ The timestamp of the event which led to this
    ...
    /**
     * Ungrabs the pointer on the default display, if it is grabbed by
     * this application.
     *
     * @param time_ A timestamp from a {@link Event}, or
     *   %GDK_CURRENT_TIME if no timestamp is available.
     ...
     * @param nelements The number of elements of size determined by
     *   the format, contained in #data.
     */
     ...
     * property does not exist, then the function returns %FALSE, and
     ...
     * undefined), and also prints warnings to stderr in cases where
     * it should return a useful error to the program. You are advised
     * to use XGetWindowProperty() directly until a replacement
     ...
     *   type, then #actual_format and #actual_length will be filled
     ...
     * @param length The length of the data to retrieve in bytes. Data
     *   is considered to be retrieved in 4 byte chunks, so #length
     *   will be rounded up to the next highest 4 byte boundary (so be
     *   careful not to pass a value that might overflow when rounded
     *   up).
     * @param pdelete If %TRUE, delete the property after retrieving
     *   the data.
     * @returns %TRUE if data was successfully received and stored in
     *   #data, otherwise %FALSE.
     *
     *   Location to store the actual type of the property.
     *
     *   Location to store the actual return format of the data; either
     *   8, 16 or 32 bits.
     *
     *   Location to store the length of the retrieved data, in bytes.
     *   Data returned in the 32 bit format is stored in a long
     *   variable, so the actual number of 32 bit elements should be
     *   be calculated via #actual_length / sizeof(glong) to ensure
     ...
     * {@link (gdk.list_visuals}) and then looking at the depth field
     ...
     * {@link (gdk.list_visuals}) and then looking at the type field in
     * each visual, removing duplicates.
     *
     * The array returned by this function should not be freed.
     *
     ...
     *   selection. The selection owner may refuse the request if it
     *   did not own the selection at the time indicated by the
     *   timestamp.
     */
     ...
     * @returns If there is a selection owner for this window, and it
     *   is a window known to the current process, the {@link Window}
     *   that owns the selection, otherwise %NULL. Note that the
     *   return value may be owned by a different process if a foreign
     *   window was previously created for that window, but a new
     ...
     *
     * Note that the return value may be owned by a different process
     * if a foreign window was previously created for that window, but
     * a new foreign window will never be created by this call.
     *
     * @param display A {@link Display}
     * @param selection An atom indentifying a selection
     * @returns If there is a selection owner for this window, and it
     *   is a window known to the current process, the {@link Window}
     *   that owns the selection, otherwise %NULL.
     */
     ...
     *
     * @param owner A {@link Window} or %NULL to indicate that the the
     ...
     *   ignored.
     * @param send_event If %TRUE, and the new owner is different from
     *   the current owner, the current owner will be sent a
     *   SelectionClear event.
     * @returns %TRUE if the selection owner was successfully changed
     *   to #owner, otherwise %FALSE.
     */
     ...
     * @param time_ Timestamp to use when setting the selection If
     *   this is older than the timestamp given last time the owner
     *   was set for the given selection, the request will be ignored
     * @param send_event If %TRUE, and the new owner is different from
     *   the current owner, the current owner will be sent a
     *   SelectionClear event
     * @returns %TRUE if the selection owner was successfully changed
    ...
    /**
     * Retrieves selection data that was stored by the selection data
     * in response to a call to {@link Gdk.selection.convert}. This
     * function will not be used by applications, who should use the
     ...
     *   If the retrieval failed, %NULL we be stored here, otherwise,
     *   it will be non-%NULL and the returned data should be freed
     *   with {@link G.free} when you are finished using it. The length
     *   of the allocated memory is one more than the length of the
     *   returned data, and the final byte will always be zero, to
     *   ensure nul-termination of strings
     ...
     * |[<!-- language="C" -->
     *
     * Gdk_set_allowed_backends ("wayland,quartz,*"); ]| instructs GDK
     * to try the Wayland backend first, followed by the Quartz
     * backend, and then all others.
     *
     * If the `GDK_BACKEND` environment variable is set, it determines
     * what backends are tried in what order, while still respecting
     * the set of allowed backends that are specified by this
     * function.
     *
     ...
     * gtk_init(), gtk_init_with_args() or gtk_init_check() in order
     * to take effect.
     *
     ...
     * not set this, it is a global user-configured setting.
     *
     * @param msec Double click time in milliseconds (thousandths of a
     *   second)
     */
    function set_double_click_time(msec: number): void;

    /**
     * Sets the program class. The X11 backend uses the program class
     * to set the class name part of the `WM_CLASS` property on
     ...
     * GTK+ must be compiled with debugging (that is, configured using
     ...
     * @param name The name of the setting.
     * @param value Location to store the value of the setting.
     * @returns %TRUE if the setting existed and a value was stored in
    ...
    /**
     * Retrieves a pixel from #window to force the windowing system to
    ...
    /**
     * This function is intended to be used in GTK+ test programs. It
     * will warp the mouse pointer to the given (#x,#y) coordinates
     ...
     * function, for most testing purposes, gtk_test_widget_click() is
     * the right function to call which will generate a button press
     * event followed by its accompanying button release event.
     *
     ...
     * (#x,#y) are > (-1,-1), it will warp the mouse pointer to the
     * given (#x,#y) coordinates within #window and simulate a key
     * press or release event.
     *
     * When the mouse pointer is warped to the target location, use of
     * this function outside of test programs that run in their own
     * virtual windowing system (e.g. Xvfb) is not recommended. If
     * (#x,#y) are passed as (-1,-1), the mouse pointer will not be
     * warped and #window origin will be used as mouse pointer
     * location for the event.
     *
     * Also, {@link Gdk.test.simulate_key} is a fairly low level
     * function, for most testing purposes, gtk_test_widget_send_key()
     * is the right function to call which will generate a key press
     * event followed by its accompanying key release event.
     *
     ...
     *
     *   Location to store the list of strings or %NULL. The list should
     *   be freed with {@link G.strfreev}.
     */
     ...
     * priority events pending. If the function returns %FALSE it is
     * automatically removed from the list of event sources and will
     * not be called again.
     *
     * This variant of {@link G.idle_add_full} calls #function with the
     * GDK lock held. It can be thought of a MT-safe version for GTK+
     * widgets for the following use case, where you have to worry
     * about idle_callback() running in thread A and accessing #self
     ...
     * Self->idle_id = 0;
     *
     * // gdk_threads_leave(); would be needed for g_idle_add() return
     * FALSE; }
     *
     ...
     * callback }
     *
     * Static void some_widget_finalize (GObject *object) { SomeWidget
     * *self = SOME_WIDGET (object); if (self->idle_id)
     * g_source_remove (self->idle_id); G_OBJECT_CLASS
     * (parent_class)->finalize (object); } ]|
     *
     * @param priority The priority of the idle source. Typically this
     *   will be in the range between #G_PRIORITY_DEFAULT_IDLE and
    ...
    /**
     * Sets a function to be called at regular intervals holding the
     * GDK lock, with the given priority. The function is called
     * repeatedly until it returns %FALSE, at which point the timeout
     * is automatically destroyed and the function will not be called
     * again. The #notify function is called when the timeout is
     * destroyed. The first call to the function will be at the end of
     * the first #interval.
     *
     ...
     * function, the time of the next timeout is recalculated based on
     * the current time and the given interval (it does not try to
     * “catch up” time lost in delays).
     *
     * This variant of {@link G.timeout_add_full} can be thought of a
     * MT-safe version for GTK+ widgets for the following use case:
     *
     * |[<!-- language="C" -->
     *
     * Static gboolean timeout_callback (gpointer data) { SomeWidget
     * *self = data;
     *
     * // do stuff with self
     *
     ...
     *
     * Static void some_widget_finalize (GObject *object) { SomeWidget
     * *self = SOME_WIDGET (object);
     *
     * If (self->timeout_id) g_source_remove (self->timeout_id);
     *
     * G_OBJECT_CLASS (parent_class)->finalize (object); } ]|
     *
     * @param priority The priority of the timeout source. Typically
     ...
     * @param _function Function to call
     * @param data Data to pass to #function
     * @param notify Function to call when the timeout is removed, or
     *   %NULL
     * @returns The ID (greater than 0) of the event source.
     */
     ...
     * discussion of why it is a good idea to use this function if you
     * don’t need finer granularity.
     *
     * @param priority The priority of the timeout source. Typically
     *   this will be in the range between #G_PRIORITY_DEFAULT_IDLE
     *   and #G_PRIORITY_HIGH_IDLE.
     ...
     * which GDK and GTK+ functions can be called safely and without
     * causing race conditions. Only one thread at a time can be in
     * such a critial section.
     */
    function threads_enter(): void;

    /**
     * Initializes GDK so that it can be used from multiple threads in
     * conjunction with {@link Gdk.threads.enter} and
     * gdk_threads_leave().
     *
    ...
    /**
     * Allows the application to replace the standard method that GDK
     * uses to protect its data structures. Normally, GDK creates a
     * single #GMutex that is locked by {@link Gdk.threads.enter}, and
     * released by gdk_threads_leave(); using this function an
     ...
     * When GTK+ unlocks the GTK+ lock when entering a recursive main
     * loop, the application must temporarily release its lock as
     * well.
     *
     ...
     * @returns The corresponding GDK key symbol, if one exists. or,
     *   if there is no corresponding symbol, wc | 0x01000000
     */
    function unicode_to_keyval(wc: number): number;

    /**
     * Converts an UTF-8 string into the best possible representation
     * as a STRING. The representation of characters not in STRING is
     * not specified; it may be as pseudo-escape sequences \x{ABCD},
     * or it may be in some other form of approximation.
     *
     * @param str A UTF-8 string
     * @returns The newly-allocated string, or %NULL if the conversion
     *   failed. (It should not fail for any properly formed UTF-8
     *   string unless system limits like memory or file descriptors
     *   are exceeded.)
     */
    ...
    /**
     * The primary button. This is typically the left mouse button, or
     * the right button in a left-handed setup.
     *
     * @returns The primary button. This is typically the left mouse
     *   button, or the right button in a left-handed setup.
     */
    const BUTTON_PRIMARY: number;

    /**
     * The secondary button. This is typically the right mouse button,
     ...
     * expected.
     *
     * @returns Represents the current time, and can be used anywhere
     *   a time is expected.
     */
     ...
     *
     * @returns Use this macro as the return value for continuing the
     *   propagation of an event handler.
     */
    const EVENT_PROPAGATE: boolean;

    /**
     * Use this macro as the return value for stopping the propagation
     * of an event handler.
     *
     ...
     *
     * @returns A special value, indicating that the background for a
     *   window should be inherited from the parent window.
     */
     ...
     *
     * @returns This is the priority that the idle handler processing
     *   window updates is given in the [GLib Main
     *   Loop][glib-The-Main-Event-Loop].
     */
