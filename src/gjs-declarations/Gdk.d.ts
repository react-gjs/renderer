...
declare module "gi://Gdk" {
  ...
  interface IAppLaunchContext {
     ...
     * when using this context when running under a window
     * manager that supports multiple workspaces, as described in
     * the [Extended Window Manager
     * Hints](http://www.freedesktop.org/Standards/wm-spec).
     *
     * When the workspace is not specified or #desktop is set to
     * -1, it is up to the window manager to pick one, typically
     * it will be the current workspace.
     *
     * @param desktop The number of a workspace, or -1
     */
    set_desktop(desktop: number): void;
    /**
     * @deprecated Use {@link Gdk.Display.get_app_launch_context}
     *   instead
     *
     *   Sets the display on which applications will be launched
     *   when using this context. See also
     *   {@link Gdk.AppLaunchContext.set_screen}.
     ...
     * context.
     *
     * Window Managers can use this information when displaying
    ...
    /**
     * Sets the icon for applications that are launched with this
     * context. The #icon_name will be interpreted in the same
     * way as the Icon field in desktop files. See also
     * {@link Gdk.AppLaunchContext.set_icon}.
     *
     * If both #icon and #icon_name are set, the #icon_name takes
     * priority. If neither #icon or #icon_name is set, the icon
     * is taken from either the file that is passed to launched
     * application or from the #GAppInfo for the launched
     * application itself.
     *
    ...
    /**
     * Sets the screen on which applications will be launched
     * when using this context. See also
     * {@link Gdk.AppLaunchContext.set_display}.
     *
     * If both #screen and #display are set, the #screen takes
     * priority. If neither #screen or #display are set, the
     * default screen and display are used.
     *
     * @param screen A {@link Screen}
     */
    set_screen(screen: Screen): void;
    /**
     * Sets the timestamp of #context. The timestamp should
     * ideally be taken from the event that triggered the
     * launch.
     *
     * Window managers can use this information to avoid moving
     * the focus to the newly launched application when the user
     * is busy typing in another window. This is also known as
  ...
  type AppLaunchContextInitOptionsMixin = Gio.AppLaunchContextInitOptions &
    Pick<IAppLaunchContext, "display">;

  export interface AppLaunchContextInitOptions
    extends AppLaunchContextInitOptionsMixin {}
   ...
   * #GAppLaunchContext that handles launching an application in
   * a graphical context. It provides startup notification and
   * allows to launch applications on a specific screen or
   * workspace.
   *
   ...
   * ("http://www.gtk.org", context, &error)) g_warning
   * ("Launching failed: %s\n", error->message);
   *
  ...
  interface ICursor {
    ...
    get_display(): Display;
    /**
     * Returns a {@link Pixbuf} with the image used to display the
     * cursor.
     *
     * Note that depending on the capabilities of the windowing
     * system and on the cursor, GDK may not be able to obtain
     * the image data. In this case, %NULL is returned.
     *
     * @returns A {@link Pixbuf} representing #cursor, or %NULL
     */
    get_image(): GdkPixbuf.Pixbuf | null;
    /**
     * Returns a cairo image surface with the image used to
     * display the cursor.
     *
     * Note that depending on the capabilities of the windowing
     * system and on the cursor, GDK may not be able to obtain
    ...
    /**
     * @deprecated Use {@link GObject.ref} instead
     *
    ...
    ref(): Cursor;
    /**
     * @deprecated Use {@link GObject.unref} instead
     *
  ...
  type CursorInitOptionsMixin = GObject.ObjectInitOptions &
    Pick<ICursor, "cursor_type" | "display">;
  ...
  /**
   * A {@link Cursor} represents a cursor. Its contents are
   * private.
   */
  ...
  class Cursor {
    ...
    /**
     * @deprecated Use {@link Gdk.Cursor.new_for_display} instead.
     *
     *   Creates a new cursor from the set of builtin cursors for
     *   the default display. See
     *   {@link Gdk.Cursor.new_for_display}.
     *
     ...
     *
     * @param display The {@link Display} for which the cursor
     *   will be created
     * @param cursor_type Cursor to create
     * @returns A new {@link Cursor}, or %NULL on failure
     */
    public static new_for_display(
      display: Display,
      cursor_type: CursorType
    ): Cursor | null;
    /**
     * Creates a new cursor by looking up #name in the current
     * cursor theme.
     *
     * A recommended set of cursor names that will work across
     * different platforms can be found in the CSS
     * specification:
     *
     * - "none"
     ...
     * gdk_display_supports_cursor_color() can be used to
     * determine whether RGBA cursors are supported;
     * gdk_display_get_default_cursor_size() and
     * gdk_display_get_maximal_cursor_size() give information
     * about cursor sizes.
     *
     * If #x or #y are `-1`, the pixbuf must have options named
     * “x_hot” and “y_hot”, resp., containing integer values
     * between `0` and the width resp. height of the pixbuf.
     * (Since: 3.0)
     *
     ...
     * @param display The {@link Display} for which the cursor
     *   will be created
     * @param pixbuf The {@link Pixbuf} containing the cursor
     *   image
     * @param x The horizontal offset of the “hotspot” of the
     *   cursor.
     * @param y The vertical offset of the “hotspot” of the
     *   cursor.
     * @returns A new {@link Cursor}.
     */
    public static new_from_pixbuf(
      display: Display,
     ...
     * @param display The {@link Display} for which the cursor
     *   will be created
     * @param surface The cairo image surface containing the
     *   cursor pixel data
     * @param x The horizontal offset of the “hotspot” of the
     *   cursor
     * @param y The vertical offset of the “hotspot” of the
     ...
     * @returns A new {@link Cursor}.
     */
    public static new_from_surface(
      display: Display,
  ...
  interface IDevice {
     ...
     * Devices of type #GDK_DEVICE_TYPE_MASTER always come in
     * keyboard/pointer pairs. Other device types will have a
     ...
     * screen. Devices of type %GDK_DEVICE_TYPE_MASTER will have
     * %TRUE here.
     */
    has_cursor: boolean;
    ...
    name: string;
    /**
     * The maximal number of concurrent touches on a touch
     * device. Will be 0 if the device is not a touch device or
     * if the number of touches is unknown.
     */
    num_touches: number;
    /**
     * Product ID of this device, see
     * {@link Gdk.Device.get_product_id}.
     */
    product_id: string;
     ...
     * {@link Gdk.Device.get_vendor_id}.
     */
    vendor_id: string;
    /**
     * Returns the associated device to #device, if #device is of
     * type %GDK_DEVICE_TYPE_MASTER, it will return the paired
     * pointer or keyboard.
     *
     * If #device is of type %GDK_DEVICE_TYPE_SLAVE, it will
     * return the master device to which #device is attached to.
     *
     * If #device is of type %GDK_DEVICE_TYPE_FLOATING, %NULL
     ...
     * device, and locates the value in the array for a given
     * axis use.
     *
     * @param axes Pointer to an array of axes
     * @param use The use to look for
     * @returns %TRUE if the given axis use was found, otherwise
     *   %FALSE
     *
     ...
     * Interprets an array of double as axis values for a given
     * device, and locates the value in the array for a given
     * axis label, as returned by {@link Gdk.Device.list_axes}
     *
     * @param axes Pointer to an array of axes
     ...
     *
     * @returns A {@link Display}. This memory is owned by GTK+,
     *   and must not be freed or unreffed.
     */
    get_display(): Display;
    /**
     * Determines whether the pointer follows device motion. This
     * is not meaningful for keyboard devices, which don't have a
     * pointer.
     *
    ...
    /**
     * Obtains the motion history for a pointer device; given a
     * starting and ending timestamp, return all events in the
     * motion history for the device in the given range of time.
     * Some windowing systems do not support motion history, in
     * which case, %FALSE will be returned. (This is not
     * distinguishable from the case where motion history is
     * supported and no events were found.)
     *
     * Note that there is also
     * {@link Gdk.Window.set_event_compression} to get more motion
     * events delivered directly, independent of the windowing
     * system.
     *
     * @param window The window with respect to which which the
     *   event coordinates will be reported
     * @param start Starting timestamp for range of events to
     *   return
     * @param stop Ending timestamp for the range of events to
     *   return
     * @returns %TRUE if the windowing system supports motion
     *   history and at least one event was found.
     *
     *   Location to store a newly-allocated array of
     *   {@link TimeCoord}, or %NULL
     *
    ...
    get_history(
      ...
      start: number,
      stop: number
    ): [boolean, TimeCoord[] | null, number | null];
    /**
     * If #index_ has a valid keyval, this function will return
     * %TRUE and fill in #keyval and #modifiers with the keyval
     * settings.
     *
    ...
    /**
     * Gets information about which window the given pointer
     * device is in, based on events that have been received so
     * far from the display server. If another application has a
     * pointer grab, or this application has a grab with
     * owner_events = %FALSE, %NULL may be returned even if the
     * pointer is physically over one of this application's
     * windows.
     *
     ...
     * may not be called on devices of type
     * %GDK_DEVICE_TYPE_SLAVE, unless there is an ongoing grab on
     * them, see {@link Gdk.Device.grab}.
     *
     * @returns Location to store the {@link Screen} the #device
     *   is on, or %NULL.
     *
     *   Location to store root window X coordinate of #device, or
     *   %NULL.
     *
     *   Location to store root window Y coordinate of #device, or
     *   %NULL.
     */
    get_position(): [screen: Screen | null, x: number | null, y: number | null];
    /**
     * Gets the current location of #device in double precision.
     * As a slave device's coordinates are those of its master
     * pointer, this function may not be called on devices of
     * type %GDK_DEVICE_TYPE_SLAVE, unless there is an ongoing
     * grab on them. See {@link Gdk.Device.grab}.
     *
     * @returns Location to store the {@link Screen} the #device
     *   is on, or %NULL.
     *
    ...
    /**
     * Returns the product ID of this device, or %NULL if this
     * information couldn't be obtained. This ID is retrieved
     * from the device, and is thus constant for it. See
     * {@link Gdk.Device.get_vendor_id} for more information.
     *
     ...
     * #window. As a slave device’s coordinates are those of its
     * master pointer, this function may not be called on devices
     * of type %GDK_DEVICE_TYPE_SLAVE, unless there is an ongoing
     * grab on them. See {@link Gdk.Device.grab}.
     *
     * @param window A {@link Window}.
     * @param axes An array of doubles to store the values of the
     *   axes of #device in, or %NULL.
     * @returns Location to store the modifiers, or %NULL.
     */
    get_state(window: Window, axes: number[] | null): ModifierType | null;
    /**
     * Returns the vendor ID of this device, or %NULL if this
     * information couldn't be obtained. This ID is retrieved
     * from the device, and is thus constant for it.
     *
     * This function, together with
     * {@link Gdk.Device.get_product_id}, can be used to eg.
     * compose #GSettings paths to store settings for this
     * device.
     *
     * |[<!-- language="C" -->
     *
     * Static GSettings * get_device_settings (GdkDevice *device)
     * { const gchar *vendor, *product; GSettings *settings;
     * GdkDevice *device; gchar *path;
     *
     * Vendor = gdk_device_get_vendor_id (device); product =
     * gdk_device_get_product_id (device);
     *
     * Path = g_strdup_printf ("/org/example/app/devices/%s:%s/",
     * vendor, product); settings = g_settings_new_with_path
     * (DEVICE_SCHEMA, path); g_free (path);
     *
     * Return settings; } ]|
     *
    ...
    /**
     * Obtains the window underneath #device, returning the
     * location of the device in #win_x and #win_y. Returns %NULL
     * if the window tree under #device is not known to GDK (for
     * example, belongs to another application).
     *
     * As a slave device coordinates are those of its master
     * pointer, This function may not be called on devices of
     * type %GDK_DEVICE_TYPE_SLAVE, unless there is an ongoing
     * grab on them, see {@link Gdk.Device.grab}.
     *
     * @returns The {@link Window} under the device position, or
     *   %NULL.
     *
     *   Return location for the X coordinate of the device
     *   location, relative to the window origin, or %NULL.
     *
     *   Return location for the Y coordinate of the device
     *   location, relative to the window origin, or %NULL.
     */
    get_window_at_position(): [Window | null, number | null, number | null];
    /**
     * Obtains the window underneath #device, returning the
     * location of the device in #win_x and #win_y in double
     * precision. Returns %NULL if the window tree under #device
     * is not known to GDK (for example, belongs to another
     * application).
     *
     * As a slave device coordinates are those of its master
     * pointer, This function may not be called on devices of
    ...
    /**
     * @deprecated Use {@link Gdk.Seat.grab} instead.
     *
     *   Grabs the device so that all events coming from this
     *   device are passed to this application until the device
     *   is ungrabbed with {@link Gdk.Device.ungrab}, or the
     *   window becomes unviewable. This overrides any previous
     *   grab on the device by this client.
     *
     *   Note that #device and #window need to be on the same
     *   display.
     *
     *   Device grabs are used for operations which need complete
     *   control over the given device events (either pointer or
     *   keyboard). For example in GTK+ this is used for Drag and
     *   Drop operations, popup menus and such.
     *
     *   Note that if the event mask of an X window has selected
     *   both button press and button release events, then a
     *   button press event will cause an automatic pointer grab
     *   until the button is released. X does this automatically
     *   since most applications expect to receive button press
     *   and release events in pairs. It is equivalent to a
     *   pointer grab on the window with #owner_events set to
     *   %TRUE.
     *
     *   If you set up anything at the time you take the grab that
     *   needs to be cleaned up when the grab ends, you should
     *   handle the {@link EventGrabBroken} events that are
     ...
     * @param owner_events If %FALSE then all device events are
     *   reported with respect to #window and are only reported
     *   if selected by #event_mask. If %TRUE then pointer events
     *   for this application are reported as normal, but pointer
     *   events outside this application are reported with
     *   respect to #window and only if selected by #event_mask.
     *   In either mode, unreported events are discarded.
     * @param event_mask Specifies the event mask, which is used
     *   in accordance with #owner_events.
     * @param cursor The cursor to display while the grab is
     *   active if the device is a pointer. If this is %NULL then
     *   the normal cursors are used for #window and its
     *   descendants, and the cursor for #window is used
     *   elsewhere.
     * @param time_ The timestamp of the event which led to this
     *   pointer grab. This usually comes from the {@link Event}
    ...
    /**
     * Returns a #GList of {@link Atoms}, containing the labels
     * for the axes that #device currently has.
     *
     * @returns A #GList of {@link Atoms}, free with
     *   {@link G.list_free}.
     */
    list_axes(): GLib.List;
    /**
     * If the device if of type %GDK_DEVICE_TYPE_MASTER, it will
     * return the list of slave devices attached to it, otherwise
     * it will return %NULL
     *
     * @returns The list of slave devices, or %NULL. The list
     *   must be freed with {@link G.list_free}, the contents of
     *   the list are owned by GTK+ and should not be freed.
     */
    ...
    /**
     * Specifies the X key event to generate when a macro button
     * of a device is pressed.
     *
    ...
    /**
     * Sets a the mode of an input device. The mode controls if
     * the device is active and whether the device’s range is
     * mapped to the entire screen or to a single window.
     *
     * Note: This is only meaningful for floating devices, master
     * devices (and slaves connected to these) drive the pointer
     * cursor, which is not limited by the input mode.
     *
    ...
    /**
     * @deprecated Use {@link Gdk.Seat.ungrab} instead.
     *
    ...
    /**
     * Warps #device in #display to the point #x,#y on the screen
     * #screen, unless the device is confined to a window by a
     * grab, in which case it will be moved as far as allowed by
     * the grab. Warping the pointer creates events as if the
     * user had moved the mouse instantaneously to the
     * destination.
     *
     * Note that the pointer should normally be under the control
     * of the user. This function was added to cover some rare
     * use cases like keyboard navigation support for the color
     * picker in the #GtkColorSelectionDialog.
     *
    ...
    /**
     * The ::changed signal is emitted either when the
     * {@link Device} has changed the number of either axes or
     * keys. For example In X this will normally happen when the
     * slave device routing events through the master device
     * changes (for example, user switches from the USB mouse to
     * a tablet), in that case the master device will change to
     * reflect the new slave device axes and keys.
     *
     * @param signal
    ...
    connect(
      signal: "tool-changed",
      callback: (owner: this, tool: DeviceTool) => void
    ): number;

    connect(
      signal: "notify::associated-device",
      callback: (owner: this, ...args: any) => void
    ): number;
    connect(
      signal: "notify::axes",
    ...
    connect(
      signal: "notify::has-cursor",
      callback: (owner: this, ...args: any) => void
    ): number;
    connect(
      signal: "notify::input-mode",
      callback: (owner: this, ...args: any) => void
    ): number;
    connect(
      signal: "notify::input-source",
      callback: (owner: this, ...args: any) => void
    ): number;
    connect(
      signal: "notify::n-axes",
      callback: (owner: this, ...args: any) => void
    ): number;
    connect(
      signal: "notify::name",
    ...
    connect(
      signal: "notify::num-touches",
      callback: (owner: this, ...args: any) => void
    ): number;
    connect(
      signal: "notify::product-id",
      callback: (owner: this, ...args: any) => void
    ): number;
    connect(
      signal: "notify::seat",
  ...
  /**
   * The {@link Device} object represents a single input device,
   * such as a keyboard, a mouse, a touchpad, etc.
   *
   * See the #GdkDeviceManager documentation for more information
   * about the various kinds of master and slave devices, and
   * their relationships.
   */
  ...
  class Device {
    ...
    /**
     * Frees an array of {@link TimeCoord} that was returned by
     * {@link Gdk.Device.get_history}.
     *
     ...
     *   This is not public API and must not be used by
     *   applications.
     * @param display The display for which to get the grab
     *   information
     ...
     *   #owner_events flag to {@link Gdk.keyboard.grab} or
     *   gdk_pointer_grab() was %TRUE.
     */
    public static grab_info_libgtk_only(
      display: Display,
      device: Device
  ...
  interface IDeviceManager {
    display: Display;
    /**
     * @deprecated Use {@link Gdk.Seat.get_pointer} instead.
     *
     *   Returns the client pointer, that is, the master pointer
     *   that acts as the core pointer for this application. In
     *   X11, window managers may change this depending on the
     *   interaction pattern under the presence of several
     *   pointers.
     *
     *   You should use this function seldomly, only in code that
     *   isn’t triggered by a {@link Event} and there aren’t other
     *   means to get a meaningful #GdkDevice to operate on.
     * @returns The client pointer. This memory is owned by GDK
     ...
     *
     * @returns The {@link Display} to which #device_manager is
     *   associated to, or %NULL. This memory is owned by GDK and
     *   must not be freed or unreferenced.
     */
    get_display(): Display | null;
    /**
     * @deprecated , use {@link Gdk.Seat.get_pointer},
     *   gdk_seat_get_keyboard() and gdk_seat_get_slaves()
     *   instead.
     *
     *   Returns the list of devices of type #type currently
     *   attached to #device_manager.
     * @param type Device type to get.
     * @returns A list of {@link Devices}. The returned list must
     *   be freed with g_list_free (). The list elements are
     *   owned by GTK+ and must not be freed or unreffed.
     */
    list_devices(type: DeviceType): GLib.List;
    /**
     * The ::device-added signal is emitted either when a new
     * master pointer is created, or when a slave (Hardware)
    ...
    connect(
      signal: "device-added",
      callback: (owner: this, device: Device) => void
    ): number;
    /**
     * The ::device-changed signal is emitted whenever a device
     * has changed in the hierarchy, either slave devices being
     * disconnected from their master device or connected to
     * another one, or master devices being added or removed a
     * slave device.
     *
     * If a slave device is detached from all master devices
     * {@link (gdk.device_get_associated_device} returns %NULL),
     * its {@link DeviceType} will change to
     * %GDK_DEVICE_TYPE_FLOATING, if it's attached, it will
     * change to %GDK_DEVICE_TYPE_SLAVE.
     *
     * @param signal
     * @param callback Callback function
     *
    ...
    /**
     * The ::device-removed signal is emitted either when a
     * master pointer is removed, or when a slave (Hardware)
  ...
  type DeviceManagerInitOptionsMixin = GObject.ObjectInitOptions &
    Pick<IDeviceManager, "display">;

  export interface DeviceManagerInitOptions
    extends DeviceManagerInitOptionsMixin {}
  ...
  /**
   * In addition to a single pointer and keyboard for user
   * interface input, GDK contains support for a variety of input
   * devices, including graphics tablets, touchscreens and
   * multiple pointers/keyboards interacting simultaneously with
   * the user interface. Such input devices often have additional
   * features, such as sub-pixel positioning information and
   * additional device-dependent information.
   *
   * In order to query the device hierarchy and be aware of
   * changes in the device hierarchy (such as virtual devices
   * being created or removed, or physical devices being plugged
   * or unplugged), GDK provides {@link DeviceManager}.
   *
   * By default, and if the platform supports it, GDK is aware of
   * multiple keyboard/pointer pairs and multitouch devices. This
   * behavior can be changed by calling
   * {@link Gdk.disable.multidevice} before gdk_display_open().
   * There should rarely be a need to do that though, since GDK
   * defaults to a compatibility mode in which it will emit just
   * one enter/leave event pair for all devices on a window. To
   * enable per-device enter/leave events and other multi-pointer
   * interaction features, gdk_window_set_support_multidevice()
   * must be called on #GdkWindows (or
   * gtk_widget_set_support_multidevice() on widgets). window.
   * See the gdk_window_set_support_multidevice() documentation
   * for more information.
   *
   * On X11, multi-device support is implemented through XInput
   * 2. Unless gdk_disable_multidevice() is called, the XInput 2
   * #GdkDeviceManager implementation will be used as the input
   * source. Otherwise either the core or XInput 1
   * implementations will be used.
   *
   * For simple applications that don’t have any special interest
   * in input devices, the so-called “client pointer” provides a
   * reasonable approximation to a simple setup with a single
   * pointer and keyboard. The device that has been set as the
   * client pointer can be accessed via
   * gdk_device_manager_get_client_pointer().
   *
   * Conceptually, in multidevice mode there are 2 device types.
   * Virtual devices (or master devices) are represented by the
   * pointer cursors and keyboard foci that are seen on the
   * screen. Physical devices (or slave devices) represent the
   * hardware that is controlling the virtual devices, and thus
   * have no visible cursor on the screen.
   *
   * Virtual devices are always paired, so there is a keyboard
   * device for every pointer device. Associations between
   * devices may be inspected through
   * gdk_device_get_associated_device().
   *
   * There may be several virtual devices, and several physical
   * devices could be controlling each of these virtual devices.
   * Physical devices may also be “floating”, which means they
   * are not attached to any virtual device.
   *
   * # Master and slave devices
   *
   * |[
   *
   * Carlos#sacarino:~$ xinput list ⎡ Virtual core pointer id=2
   * [master pointer (3)] ⎜ ↳ Virtual core XTEST pointer id=4
   * [slave pointer (2)] ⎜ ↳ Wacom ISDv4 E6 Pen stylus id=10
   * [slave pointer (2)] ⎜ ↳ Wacom ISDv4 E6 Finger touch id=11
   * [slave pointer (2)] ⎜ ↳ SynPS/2 Synaptics TouchPad id=13
   * [slave pointer (2)] ⎜ ↳ TPPS/2 IBM TrackPoint id=14 [slave
   * pointer (2)] ⎜ ↳ Wacom ISDv4 E6 Pen eraser id=16 [slave
   * pointer (2)] ⎣ Virtual core keyboard id=3 [master keyboard
   * (2)] ↳ Virtual core XTEST keyboard id=5 [slave keyboard (3)]
   * ↳ Power Button id=6 [slave keyboard (3)] ↳ Video Bus id=7
   * [slave keyboard (3)] ↳ Sleep Button id=8 [slave keyboard
   * (3)] ↳ Integrated Camera id=9 [slave keyboard (3)] ↳ AT
   * Translated Set 2 keyboard id=12 [slave keyboard (3)] ↳
   * ThinkPad Extra Buttons id=15 [slave keyboard (3)] ]|
   *
   * By default, GDK will automatically listen for events coming
   * from all master devices, setting the #GdkDevice for all
   * events coming from input devices. Events containing device
   * information are #GDK_MOTION_NOTIFY, #GDK_BUTTON_PRESS,
   * #GDK_2BUTTON_PRESS, #GDK_3BUTTON_PRESS, #GDK_BUTTON_RELEASE,
   * #GDK_SCROLL, #GDK_KEY_PRESS, #GDK_KEY_RELEASE,
   * #GDK_ENTER_NOTIFY, #GDK_LEAVE_NOTIFY, #GDK_FOCUS_CHANGE,
   * #GDK_PROXIMITY_IN, #GDK_PROXIMITY_OUT, #GDK_DRAG_ENTER,
   * #GDK_DRAG_LEAVE, #GDK_DRAG_MOTION, #GDK_DRAG_STATUS,
   * #GDK_DROP_START, #GDK_DROP_FINISHED and #GDK_GRAB_BROKEN.
   * When dealing with an event on a master device, it is
   * possible to get the source (slave) device that the event
   * originated from via gdk_event_get_source_device().
   *
   * On a standard session, all physical devices are connected by
   * default to the "Virtual Core Pointer/Keyboard" master
   * devices, hence routing all events through these. This
   * behavior is only modified by device grabs, where the slave
   * device is temporarily detached for as long as the grab is
   * held, and more permanently by user modifications to the
   * device hierarchy.
   *
   * On certain application specific setups, it may make sense to
   * detach a physical device from its master pointer, and
   * mapping it to an specific window. This can be achieved by
   * the combination of gdk_device_grab() and
   * gdk_device_set_mode().
   *
   * In order to listen for events coming from devices other than
   * a virtual device, gdk_window_set_device_events() must be
   * called. Generally, this function can be used to modify the
   * event mask for any given device.
   *
   * Input devices may also provide additional information
   * besides X/Y. For example, graphics tablets may also provide
   * pressure and X/Y tilt information. This information is
   * device-dependent, and may be queried through
   * gdk_device_get_axis(). In multidevice mode, virtual devices
   * will change axes in order to always represent the physical
   * device that is routing events through it. Whenever the
   * physical device changes, the #GdkDevice:n-axes property will
   * be notified, and gdk_device_list_axes() will return the new
   * device axes.
   *
   * Devices may also have associated “keys” or macro buttons.
   * Such keys can be globally set to map into normal X keyboard
   * events. The mapping is set using gdk_device_set_key().
   *
   * In GTK+ 3.20, a new #GdkSeat object has been introduced that
   * supersedes #GdkDeviceManager and should be preferred in
   * newly written code.
   */
  ...
  interface IDeviceTool {
     ...
     * When non-zero, the identificator is unique for the given
     * tool model, meaning that two identical tools will share
     * the same #hardware_id, but will have different serial
     * numbers (see {@link Gdk.DeviceTool.get_serial}).
     *
     * This is a more concrete (and device specific) method to
     * identify a {@link DeviceTool} than
     * gdk_device_tool_get_tool_type(), as a tablet may support
     * multiple devices with the same #GdkDeviceToolType, but
    ...
    /**
     * Gets the serial of this tool, this value can be used to
     * identify a physical tool (eg. a tablet pen) across program
     * executions.
     *
     ...
     *
     * @returns The physical type for this tool. This can be used
     *   to figure out what sort of pen is being used, such as an
    ...
    connect(
      signal: "notify::hardware-id",
      callback: (owner: this, ...args: any) => void
    ): number;
    connect(
      signal: "notify::serial",
      callback: (owner: this, ...args: any) => void
    ): number;
    connect(
      signal: "notify::tool-type",
      callback: (owner: this, ...args: any) => void
    ): number;
  }

  type DeviceToolInitOptionsMixin = GObject.ObjectInitOptions &
    Pick<IDeviceTool, "axes" | "hardware_id" | "serial" | "tool_type">;
  ...
  interface IDisplay {
    ...
    beep(): void;
    /**
     * Closes the connection to the windowing system for the
     * given display, and cleans up associated resources.
     */
    close(): void;
    /**
     * Returns %TRUE if there is an ongoing grab on #device for
     * #display.
     *
     ...
     * happens automatically when the main loop blocks waiting
     * for new events, but if your application is drawing without
     * returning control to the main loop, you may need to call
     * this function explicitly. A common case where this
     * function needs to be called is when an application is
     * executing drawing commands from a thread other than the
     * thread where the main loop is running.
     *
     * This is most useful for X11. On windowing systems where
     * requests are handled synchronously, this function will do
     * nothing.
     */
    flush(): void;
    /**
     ...
     *
     * @returns A new {@link AppLaunchContext} for #display. Free
     *   with {@link GObject.unref} when done
     */
     ...
     * windows on #display. This window is implicitly created by
     * GDK. See {@link Gdk.Window.set_group}.
     *
    ...
    /**
     * @deprecated Use {@link Gdk.Display.get_default_seat} and
     *   {@link Seat} operations.
     *
     *   Returns the {@link DeviceManager} associated to #display.
     * @returns A {@link DeviceManager}, or %NULL. This memory is
     *   owned by GDK and must not be freed or unreferenced.
     */
    get_device_manager(): DeviceManager | null;
    /**
     * Gets the next {@link Event} to be processed for #display,
     * fetching events from the windowing system if necessary.
     *
     * @returns The next {@link Event} to be processed, or %NULL
     *   if no events are pending. The returned #GdkEvent should
     *   be freed with {@link Gdk.Event.free}.
     */
     ...
     * @returns The {@link Monitor}, or %NULL if #monitor_num is
     *   not a valid monitor number
     */
     ...
     * resides, or a monitor close to #window if it is outside of
     * all monitors.
     *
     * @param window A {@link Window}
     ...
     *
     * The returned number is valid until the next emission of
     * the {@link Display.monitor_added} or
     ...
     *
     * @returns A string representing the display name. This
     *   string is owned by GDK and should not be modified or
     *   freed.
     */
    get_name(): string;
    /**
     * @deprecated Use {@link Gdk.Device.get_position} instead.
     *
     *   Gets the current location of the pointer and the current
     *   modifier mask for a given display.
     * @returns Location to store the screen that the cursor is
     *   on, or %NULL.
     *
     ...
     *
     * The primary monitor is considered the monitor where the
     * “main desktop” lives. While normal application windows
     * typically allow the window manager to place the windows,
     * specialized desktop applications such as panels should
     ...
     *   {@link Gdk.Display.get_default_screen} to get it.
     *
     *   Returns a screen object for one of the screens of the
     *   display.
    ...
    /**
     * @deprecated Use {@link Gdk.Device.get_window_at_position}
     *   instead.
     *
     *   Obtains the window underneath the mouse pointer, returning
     *   the location of the pointer in that window in #win_x,
     *   #win_y for #screen. Returns %NULL if the window under
     *   the mouse pointer is not known to GDK (for example,
     *   belongs to another application).
     * @returns The window under the mouse pointer, or %NULL
     *
     *   Return location for x coordinate of the pointer location
     *   relative to the window origin, or %NULL
     *
     *   Return location for y coordinate of the pointer location
     *   relative & to the window origin, or %NULL
     */
    get_window_at_pointer(): [Window | null, number | null, number | null];
    /**
     * Returns whether the display has events that are waiting to
     * be processed.
     *
    ...
    is_closed(): boolean;
    /**
     * @deprecated Use {@link Gdk.Device.ungrab}, together with
    ...
    /**
     * @deprecated Use {@link Gdk.DeviceManager.list_devices}
     *   instead.
     *
     *   Returns the list of available input devices attached to
     *   #display. The list is statically allocated and should
     *   not be freed.
     * @returns A list of {@link Device}
     */
    list_devices(): GLib.List;
    /**
     * Returns the list of seats known to #display.
     *
     * @returns The list of seats known to the {@link Display}
     */
    ...
    /**
     * Gets a copy of the first {@link Event} in the #display’s
     * event queue, without removing the event from the queue.
     * (Note that this function will not get more events from the
     * windowing system. It only checks the events that have
     * already been moved to the GDK event queue.)
     *
     * @returns A copy of the first {@link Event} on the event
     *   queue, or %NULL if no events are in the queue. The
     *   returned #GdkEvent should be freed with
     *   {@link Gdk.Event.free}.
     */
    peek_event(): Event | null;
    /**
     * @deprecated Use {@link Gdk.Display.device_is_grabbed}
     *   instead.
     *
    ...
    /**
     * Appends a copy of the given event onto the front of the
     * event queue for #display.
     *
     * @param event A {@link Event}.
     */
    put_event(event: Event): void;
    /**
     * Request {@link EventOwnerChange} events for ownership
     * changes of the selection named by the given atom.
     *
     * @param selection The {@link Atom} naming the selection for
     *   which ownership change notification is requested
     * @returns Whether {@link EventOwnerChange} events will be
     *   sent.
     */
    request_selection_notification(selection: Atom): boolean;
    /**
     * Sets the double click distance (two clicks within this
     * distance count as a double click and result in a
     * #GDK_2BUTTON_PRESS event). See also
     * {@link Gdk.Display.set_double_click_time}. Applications
    ...
    /**
     * Sets the double click time (two clicks within this time
     * interval count as a double click and result in a
     * #GDK_2BUTTON_PRESS event). Applications should not set
     * this, it is a global user-configured setting.
     *
     * @param msec Double click time in milliseconds (thousandths
     *   of a second)
     */
    set_double_click_time(msec: number): void;
    /**
     * Issues a request to the clipboard manager to store the
     * clipboard data. On X11, this is a special program that
     * works according to the [FreeDesktop Clipboard
     * Specification](http://www.freedesktop.org/Standards/clipboard-manager-spec).
     *
     * @param clipboard_window A {@link Window} belonging to the
     *   clipboard owner
     * @param time_ A timestamp
     * @param targets An array of targets that should be saved,
     *   or %NULL if all available targets should be saved.
     ...
     * persistance; i.e. if it’s possible to store the clipboard
     * data after an application has quit. On X11 this checks if
     * a clipboard daemon is running.
     *
     * @returns %TRUE if the display supports clipboard
     *   persistance.
     */
     ...
     *
     *   Returns %TRUE if {@link Gdk.Window.set_composited} can be
     *   used to redirect drawing on the window using
     *   compositing.
     *
    ...
    /**
     * Returns %TRUE if cursors can use an 8bit alpha channel on
     * #display. Otherwise, cursors are restricted to bilevel
     * alpha (i.e. a mask).
     *
    ...
    /**
     * Returns %TRUE if multicolored cursors are supported on
     * #display. Otherwise, cursors have only a forground and a
     * background color.
     *
    ...
    /**
     * Returns %TRUE if
     * {@link Gdk.Window.input_shape_combine_mask} can be used to
     * modify the input shape of windows on #display.
     *
     * @returns %TRUE if windows with modified input shape are
     *   supported
     */
     ...
     * waits until all requests have been handled. This is often
     * used for making sure that the display is synchronized with
     * the current state of the program. Calling
     * {@link Gdk.Display.sync} before gdk_error_trap_pop() makes
     * sure that any errors generated from earlier requests are
    ...
    sync(): void;
    /**
     * @deprecated Use {@link Gdk.Device.warp} instead.
     *
     *   Warps the pointer of #display to the point #x,#y on the
     *   screen #screen, unless the pointer is confined to a
     *   window by a grab, in which case it will be moved as far
     *   as allowed by the grab. Warping the pointer creates
     *   events as if the user had moved the mouse
     ...
     *   use cases like keyboard navigation support for the color
     *   picker in the #GtkColorSelectionDialog.
     * @param screen The screen of #display to warp the pointer
     *   to
     * @param x The x coordinate of the destination
     * @param y The y coordinate of the destination
     */
     ...
     * windowing system for #display is closed.
     *
     * @param signal
     * @param callback Callback function
     *
    ...
    connect(
      signal: "closed",
      callback: (owner: this, is_error: boolean) => void
    ): number;
    /**
     * The ::monitor-added signal is emitted whenever a monitor
     * is added.
     *
     * @param signal
     * @param callback Callback function
    ...
    connect(
      ...
      callback: (owner: this, monitor: Monitor) => void
    ): number;
    /**
     * The ::monitor-removed signal is emitted whenever a monitor
     * is removed.
     *
     * @param signal
     * @param callback Callback function
    ...
    /**
     * The ::opened signal is emitted when the connection to the
     * windowing system for #display is opened.
     *
     * @param signal
     * @param callback Callback function
     *
    ...
    /**
     * The ::seat-added signal is emitted whenever a new seat is
     * made known to the windowing system.
     *
     * @param signal
     * @param callback Callback function
     *
    ...
    connect(
      signal: "seat-added",
      callback: (owner: this, seat: Seat) => void
    ): number;
    /**
     * The ::seat-removed signal is emitted whenever a seat is
     * removed by the windowing system.
     *
     * @param signal
     * @param callback Callback function
     *
   ...
   *
   * - To manage and provide information about input devices
   *   (pointers and keyboards)
   * - To manage and provide information about the available
   *   #GdkScreens
   *
   * GdkDisplay objects are the GDK representation of an X
   * Display, which can be described as a workstation consisting
   * of a keyboard, a pointing device (such as a mouse) and one
   * or more screens. It is used to open and keep track of
   * various GdkScreen objects currently instantiated by the
   * application. It is also used to access the keyboard(s) and
   * mouse pointer(s) of the display.
   *
  ...
  class Display {
     ...
     *
     * @returns A {@link Display}, or %NULL if there is no default
     *   display.
     */
     ...
     * @returns A {@link Display}, or %NULL if the display could
     *   not be opened
     */
     ...
     *   of GTK+
     *
     *   Opens the default display specified by command line
     *   arguments or environment variables, sets it as the
     *   default display, and returns it. {@link Gdk.parse.args}
     *   must have been called first. If the default display has
     *   previously been set, simply returns that. An internal
  ...
  interface IDisplayManager {
     ...
     *
     * @returns A newly allocated #GSList of {@link Display}
     *   objects. Free with {@link G.slist_free} when you are done
     *   with it.
     */
    ...
    /**
     * The ::display-opened signal is emitted when a display is
     * opened.
     *
     * @param signal
     * @param callback Callback function
    ...
    connect(
      ...
      callback: (owner: this, display: Display) => void
    ): number;

    connect(
      signal: "notify::default-display",
      callback: (owner: this, ...args: any) => void
    ): number;
  }

  type DisplayManagerInitOptionsMixin = GObject.ObjectInitOptions &
    Pick<IDisplayManager, "default_display">;

  export interface DisplayManagerInitOptions
    extends DisplayManagerInitOptionsMixin {}
  ...
  /**
   * The purpose of the {@link DisplayManager} singleton object is
   * to offer notification when displays appear or disappear or
   * the default display changes.
   *
   * You can use {@link Gdk.DisplayManager.get} to obtain the
   * #GdkDisplayManager singleton, but that should be rarely
   * necessary. Typically, initializing GTK+ opens a display that
   * you can work with without ever accessing the
   * #GdkDisplayManager.
   *
   * The GDK library can be built with support for multiple
   * backends. The #GdkDisplayManager object determines which
   * backend is used at runtime.
   *
   * When writing backend-specific code that is supposed to work
   * with multiple GDK backends, you have to consider both
   * compile time and runtime. At compile time, use the
   * #GDK_WINDOWING_X11, #GDK_WINDOWING_WIN32 macros, etc. to
   * find out which backends are present in the GDK library you
   * are building your application against. At runtime, use
   * type-check macros like GDK_IS_X11_DISPLAY() to find out
   ...
   * |[<!-- language="C" -->
   *
   * #ifdef GDK_WINDOWING_X11 if (GDK_IS_X11_DISPLAY (display)) {
   * // make X11-specific calls here } else #endif #ifdef
   * GDK_WINDOWING_QUARTZ if (GDK_IS_QUARTZ_DISPLAY (display)) {
   * // make Quartz-specific calls here } else #endif g_error
  ...
  class DisplayManager {
     ...
     * `GDK_BACKEND` environment variable to find out which of
     * the supported GDK backends to use (in case GDK has been
     * compiled with multiple backends). Applications can use
     * {@link Gdk.set.allowed_backends} to limit what backends can
     * be used.
     *
     * @returns The global {@link DisplayManager} singleton;
     *   {@link Gdk.parse.args}, gdk_init(), or gdk_init_check()
  ...
  interface IDragContext {
    /**
     * Determines the bitmask of actions proposed by the source
     * if {@link Gdk.DragContext.get_suggested_action} returns
     * %GDK_ACTION_ASK.
     *
    ...
    get_device(): Device;
    /**
     * Returns the window on which the drag icon should be
     * rendered during the drag operation. Note that the window
     * may not be available until the drag operation has begun.
     * GDK will move the window in accordance with the ongoing
     * drag operation. The window is owned by #context and will
     ...
     * #context. When a drag and drop operation becomes managed,
     * the {@link DragContext} will internally handle all input
     * and source-side #GdkEventDND events as required by the
     * windowing system.
     *
     * Once the drag and drop operation is managed, the drag
     * context will emit the following signals:
     *
     * - The #GdkDragContext::action-changed signal whenever the
     *   final action to be performed by the drag and drop
     *   operation changes.
     * - The #GdkDragContext::drop-performed signal after the user
     *   performs the drag and drop gesture (typically by
     *   releasing the mouse button).
     * - The #GdkDragContext::dnd-finished signal after the drag
     *   and drop operation concludes (after all #GdkSelection
     *   transfers happen).
     * - The #GdkDragContext::cancel signal if the drag and drop
     *   operation is finished but doesn't happen over an
     *   accepting destination, or is cancelled through other
     *   means.
     *
    ...
    /**
     * Associates a {@link Device} to #context, so all Drag and
     * Drop events for #context are emitted as if they came from
     * this device.
     *
     * @param device A {@link Device}
     */
    ...
    /**
     * A new action is being chosen for the drag and drop
     * operation.
     *
     * This signal will only be emitted if the {@link DragContext}
     * manages the drag and drop operation. See
     * {@link Gdk.DragContext.manage_dnd} for more information.
     *
     * @param signal
     * @param callback Callback function
     *
    ...
    connect(
      ...
      callback: (owner: this, action: DragAction) => void
    ): number;
    /**
    ...
    connect(
      signal: "cancel",
      callback: (owner: this, reason: DragCancelReason) => void
    ): number;
    /**
     * The drag and drop operation was finished, the drag
     * destination finished reading all data. The drag source can
     * now free all miscellaneous data.
     *
     * This signal will only be emitted if the {@link DragContext}
     * manages the drag and drop operation. See
     * {@link Gdk.DragContext.manage_dnd} for more information.
    ...
    /**
     * The drag and drop operation was performed on an accepting
     * client.
     *
     * This signal will only be emitted if the {@link DragContext}
     * manages the drag and drop operation. See
    ...
    connect(
      ...
      callback: (owner: this, time: number) => void
    ): number;
  }
  ...
  interface IDrawingContext {
     ...
     * {@link Window} that created the #GdkDrawingContext.
     *
     * The returned context is guaranteed to be valid as long as
     * the #GdkDrawingContext is valid, that is between a call to
     * {@link Gdk.Window.begin_draw_frame} and
     * gdk_window_end_draw_frame().
     *
     * @returns A Cairo context to be used to draw the contents
     *   of the {@link Window}. The context is owned by the
    ...
    connect(
      signal: "notify::window",
      callback: (owner: this, ...args: any) => void
    ): number;
  }

  type DrawingContextInitOptionsMixin = GObject.ObjectInitOptions &
    Pick<IDrawingContext, "clip" | "window">;

  export interface DrawingContextInitOptions
    extends DrawingContextInitOptionsMixin {}
  ...
  /**
   * {@link DrawingContext} is an object that represents the
   * current drawing state of a #GdkWindow.
   *
   * It's possible to use a #GdkDrawingContext to draw on a
   * #GdkWindow via rendering API like Cairo or OpenGL.
   *
   * A #GdkDrawingContext can only be created by calling
   * {@link Gdk.Window.begin_draw_frame} and will be valid until a
   * call to gdk_window_end_draw_frame().
   *
  ...
  interface IFrameClock {
     ...
     * {@link Gdk.FrameClock.end_updating} is made, the frame
     * clock will continually request a new frame with the
     * %GDK_FRAME_CLOCK_PHASE_UPDATE phase. This function may be
     * called multiple times and frames will be requested until
     * gdk_frame_clock_end_updating() is called the same number
     * of times.
     */
    begin_updating(): void;
    /**
     ...
     *
     * @returns The {@link FrameTimings} for the frame currently
     *   being processed, or even no frame is being processed,
     *   for the previous frame. Before any frames have been
     *   processed, returns %NULL.
     */
    get_current_timings(): FrameTimings | null;
    /**
     * A {@link FrameClock} maintains a 64-bit counter that
     * increments for each frame drawn.
     *
     * @returns Inside frame processing, the value of the frame
     *   counter for the current frame. Outside of frame
     *   processing, the frame counter for the last frame.
     */
    get_frame_counter(): number;
    /**
     * Gets the time that should currently be used for
     * animations. Inside the processing of a frame, it’s the
     * time used to compute the animation position of everything
     * in a frame. Outside of a frame, it's the time of the
     * conceptual “previous frame,” which may be either the
     * actual previous frame time, or if that’s too old, an
     * updated time.
     *
    ...
    /**
     * {@link FrameClock} internally keeps a history of
     * #GdkFrameTimings objects for recent frames that can be
     * retrieved with {@link Gdk.FrameClock.get_timings}. The set
     * of stored frames is the set from the counter values given
     * by gdk_frame_clock_get_history_start() and
     * gdk_frame_clock_get_frame_counter(), inclusive.
     *
     * @returns The frame counter value for the oldest frame that
     *   is available in the internal frame history of the
     *   {@link FrameClock}.
     */
    get_history_start(): number;
    /**
     * Using the frame history stored in the frame clock, finds
     * the last known presentation time and refresh interval, and
     * assuming that presentation times are separated by the
     * refresh interval, predicts a presentation time that is a
     * multiple of the refresh interval after the last
     * presentation time, and later than #base_time.
     *
     * @param base_time Base time for determining a presentaton
     *   time
     * @returns A location to store the determined refresh
     *   interval, or %NULL. A default refresh interval of 1/60th
     *   of a second will be stored if no history is present.
     *
     *   A location to store the next candidate presentation time
     *   after the given base time. 0 will be will be stored if
    ...
    /**
     * Retrieves a {@link FrameTimings} object holding timing
     * information for the current frame or a recent frame. The
     * #GdkFrameTimings object may not yet be complete: see
     * {@link Gdk.FrameTimings.get_complete}.
     *
     * @param frame_counter The frame counter value identifying
     *   the frame to be received.
     * @returns The {@link FrameTimings} object for the specified
     ...
     * {@link Gdk.FrameClock.request_phase} will be combined
     * together and only one frame processed. If you are
     * displaying animated content and want to continually
     * request the %GDK_FRAME_CLOCK_PHASE_UPDATE phase for a
     * period of time, you should use
     * gdk_frame_clock_begin_updating() instead, since this
     * allows GTK+ to adjust system parameters to get maximally
     * smooth animations.
     *
     ...
     * finished, and is handled internally by GTK+ to resume
     * normal event processing. Applications should not handle
     * this signal.
     *
     * @param signal
     * @param callback Callback function
     ...
     * application processing of the frame. Animations should be
     * updated using {@link Gdk.FrameClock.get_frame_time}.
     * Applications can connect directly to this signal, or use
     * gtk_widget_add_tick_callback() as a more convenient
     * interface.
     *
     * @param signal
     * @param callback Callback function
  ...
  /**
   * A {@link FrameClock} tells the application when to update and
   * repaint a window. This may be synced to the vertical refresh
   * rate of the monitor, for example. Even when the frame clock
   * uses a simple timer rather than a hardware-based vertical
   * sync, the frame clock helps because it ensures everything
   * paints at the same time (reducing the total number of
   * frames). The frame clock can also automatically stop
   * painting when it knows the frames will not be visible, or
   * scale back animation framerates.
   *
   * #GdkFrameClock is designed to be compatible with an
   * OpenGL-based implementation or with mozRequestAnimationFrame
   * in Firefox, for example.
   *
   * A frame clock is idle until someone requests a frame with
   * {@link Gdk.FrameClock.request_phase}. At some later point
   * that makes sense for the synchronization being implemented,
   * the clock will process a frame and emit signals for each
   * phase that has been requested. (See the signals of the
   * #GdkFrameClock class for documentation of the phases.
   * %GDK_FRAME_CLOCK_PHASE_UPDATE and the #GdkFrameClock::update
   * signal are most interesting for application writers, and are
   * used to update the animations, using the frame time given by
   * gdk_frame_clock_get_frame_time().
   *
   * The frame time is reported in microseconds and generally in
   * the same timescale as g_get_monotonic_time(), however, it is
   * not the same as g_get_monotonic_time(). The frame time does
   * not advance during the time a frame is being painted, and
   * outside of a frame, an attempt is made so that all calls to
   * gdk_frame_clock_get_frame_time() that are called at a
   * “similar” time get the same value. This means that if
   * different animations are timed by looking at the difference
   * in time between an initial value from
   * gdk_frame_clock_get_frame_time() and the value inside the
   * #GdkFrameClock::update signal of the clock, they will stay
  ...
  interface IGLContext {
    ...
    display: Display;
    /**
     * The {@link GLContext} that this context is sharing data
     * with, or %NULL
     */
     ...
     * {@link Gdk.gl_context_set_debug_enabled}.
     *
    ...
    /**
     * Retrieves the value set using
     * {@link Gdk.gl_context_set_forward_compatible}.
     *
    ...
    /**
     * Retrieves the {@link GLContext} that this #context share
     * data with.
     *
    ...
    /**
     * Checks whether the #context is using an OpenGL or OpenGL
     * ES profile.
     *
     * @returns %TRUE if the {@link GLContext} is using an OpenGL
     *   ES profile
     */
    get_use_es(): boolean;
    /**
     ...
     *
     * The #context must be realized prior to calling this
     * function.
     *
     ...
     *
     * The #GdkGLContext must be realized before calling this
     * function.
     *
     * When realizing a GL context, GDK will try to use the
     * OpenGL 3.2 core profile; this profile removes all the
     * OpenGL API that was deprecated prior to the 3.2 version of
     * the specification. If the realization is successful, this
     * function will return %FALSE.
     *
     * If the underlying OpenGL implementation does not support
     * core profiles, GDK will fall back to a pre-3.2
     * compatibility profile, and this function will return
     * %TRUE.
     *
     * You can use the value returned by this function to decide
     * which kind of OpenGL API to use, or whether to do
     * extension discovery, or what kind of shader programs to
     * load.
     *
    ...
    realize(): boolean;
    /**
     * Sets whether the {@link GLContext} should perform extra
     * validations and run time checking. This is useful during
     * development, but has additional overhead.
     *
     * The #GdkGLContext must not be realized or made current
    ...
    /**
     * Sets whether the {@link GLContext} should be forward
     * compatible.
     *
     * Forward compatibile contexts must not support OpenGL
     * functionality that has been marked as deprecated in the
     * requested version; non-forward compatible contexts, on the
     * other hand, must support both deprecated and non
     * deprecated functionality.
     *
     * The #GdkGLContext must not be realized or made current
     * prior to calling this function.
     *
     * @param compatible Whether the context should be forward
     *   compatible
     */
     ...
     *
     * Setting #major and #minor to zero will use the default
     * values.
     *
     * The {@link GLContext} must not be realized or made current
     * prior to calling this function.
     *
    ...
    /**
     * Requests that GDK create a OpenGL ES context instead of an
     * OpenGL one, if the platform and windowing system allows
     * it.
     *
     * The #context must not have been realized.
     *
     * By default, GDK will attempt to automatically detect
     * whether the underlying GL implementation is OpenGL or
     * OpenGL ES once the #context is realized.
     *
     * You should check the return value of
     * {@link Gdk.gl_context_get_use_es} after calling
     * gdk_gl_context_realize() to decide whether to use the
     * OpenGL or OpenGL ES API, extensions, or shaders.
     *
     * @param use_es Whether the context should use OpenGL ES
     *   instead of OpenGL, or -1 to allow auto-detection
     */
  ...
  type GLContextInitOptionsMixin = GObject.ObjectInitOptions &
    Pick<IGLContext, "display" | "shared_context" | "window">;
  ...
  /**
   * {@link GLContext} is an object representing the
   * platform-specific OpenGL drawing context.
   *
   * #GdkGLContexts are created for a #GdkWindow using
   * {@link Gdk.Window.create_gl_context}, and the context will
   * match the #GdkVisual of the window.
   *
   * A #GdkGLContext is not tied to any particular normal
   * framebuffer. For instance, it cannot draw to the #GdkWindow
   * back buffer. The GDK repaint system is in full control of
   * the painting to that. Instead, you can create render buffers
   * or textures and use gdk_cairo_draw_from_gl() in the draw
   * function of your widget to draw them. Then GDK will handle
   * the integration of your rendering with that of other
   * widgets.
   *
   * Support for #GdkGLContext is platform-specific, context
   * creation can fail, returning %NULL context.
   *
   * A #GdkGLContext has to be made "current" in order to start
   ...
   * gdk_gl_context_make_current(), or until it is realized using
   * gdk_gl_context_realize(). It is possible to specify details
   * of the GL context like the OpenGL version to be used, or
   * whether the GL context should have extra state validation
   * enabled after calling gdk_window_create_gl_context() by
   * calling gdk_gl_context_realize(). If the realization fails
   * you have the option to change the settings of the
   * #GdkGLContext and try again.
   *
   * ## Using a GdkGLContext
   *
   * You will need to make the #GdkGLContext the current context
   * before issuing OpenGL calls; the system sends OpenGL
   * commands to whichever context is current. It is possible to
   * have multiple contexts, so you always need to ensure that
   * the one which you want to draw with is the current one
   ...
   *
   * You can check which #GdkGLContext is the current one by
   * using gdk_gl_context_get_current(); you can also unset any
   * #GdkGLContext that is currently set by calling
  ...
  class GLContext {
     ...
     *
     * Any OpenGL call after this function returns will be
     * ignored until {@link Gdk.gl_context_make_current} is
     * called.
     */
  ...
  interface IKeymap {
    /**
     * Maps the non-virtual modifiers (i.e Mod2, Mod3, ...) which
     * are set in #state to the virtual modifiers (i.e. Super,
     * Hyper and Meta) and set the corresponding bits in #state.
     *
     * GDK already does this before delivering key events, but
     * for compatibility reasons, it only sets the first virtual
     * modifier it finds, whereas this function sets all matching
     * virtual modifiers.
     *
     ...
     *
     * @returns %PANGO_DIRECTION_LTR or %PANGO_DIRECTION_RTL if
     *   it can determine the direction. %PANGO_DIRECTION_NEUTRAL
     *   otherwise.
     */
    get_direction(): Pango.Direction;
    /**
     * Returns the keyvals bound to #hardware_keycode. The Nth
     * {@link KeymapKey} in #keys is bound to the Nth keyval in
     * #keyvals. Free the returned arrays with {@link G.free}.
     * When a keycode is pressed by the user, the keyval from
     * this list of entries is selected by considering the
     * effective keyboard group and level. See
     * gdk_keymap_translate_keyboard_state().
     *
     * @param hardware_keycode A keycode
     * @returns %TRUE if there were any entries
     *
     *   Return location for array of {@link KeymapKey}, or %NULL
     *
    ...
    ): [boolean, KeymapKey[] | null, number[] | null, number];
    /**
     * Obtains a list of keycode/group/level combinations that
     * will generate #keyval. Groups and levels are two kinds of
     * keyboard mode; in general, the level determines whether
     * the top or bottom symbol on a key is used, and the group
     * determines whether the left or right symbol is used. On US
     * keyboards, the shift key changes the keyboard level, and
     * there are no groups. A group switch key might convert a
     * keyboard between Hebrew to English modes, for example.
     * {@link EventKey} contains a %group field that indicates the
     * active keyboard group. The level is computed from the
     * modifier mask. The returned array should be freed with
     * {@link G.free}.
     *
     * @param keyval A keyval, such as %GDK_KEY_a, %GDK_KEY_Up,
     *   %GDK_KEY_Return, etc.
     * @returns %TRUE if keys were found and returned
     *
     *   Return location for an array of {@link KeymapKey}
     *
     *   Return location for number of elements in returned array
     */
    get_entries_for_keyval(keyval: number): [boolean, KeymapKey[], number];
    /**
     * Returns the modifier mask the #keymap’s windowing system
     * backend uses for a particular purpose.
     *
     * Note that this function always returns real hardware
     * modifiers, not virtual ones (e.g. it will return
     * #GDK_MOD1_MASK rather than #GDK_META_MASK if the backend
     * maps MOD1 to META), so there are use cases where the
     * return value of this function has to be transformed by
     * {@link Gdk.Keymap.add_virtual_modifiers} in order to
     ...
     *
     * @returns %TRUE if there are layouts in both directions,
     *   %FALSE otherwise
     */
    have_bidi_layouts(): boolean;
    /**
     * Looks up the keyval mapped to a keycode/group/level
     * triplet. If no keyval is bound to #key, returns 0. For
     * normal user input, you want to use
     * {@link Gdk.Keymap.translate_keyboard_state} instead of this
     * function, since the effective group/level may not be the
     * same as the current keyboard state.
     *
     * @param key A {@link KeymapKey} with keycode, group, and
     *   level initialized
     * @returns A keyval, or 0 if none was mapped to the given
     *   #key
     */
    lookup_key(key: KeymapKey): number;
    /**
     * Maps the virtual modifiers (i.e. Super, Hyper and Meta)
     * which are set in #state to their non-virtual counterparts
     * (i.e. Mod2, Mod3,...) and set the corresponding bits in
     * #state.
     *
     * This function is useful when matching key events against
     * accelerators.
     *
     * @returns %FALSE if two virtual modifiers were mapped to
     *   the same non-virtual modifier. Note that %FALSE is also
     *   returned if a virtual modifier is mapped to a
     *   non-virtual modifier that was already set in #state.
     */
    map_virtual_modifiers(): boolean;
    /**
     * Translates the contents of a {@link EventKey} into a
     * keyval, effective group, and level. Modifiers that
     * affected the translation and are thus unavailable for
     * application use are returned in #consumed_modifiers. See
     * [Groups][key-group-explanation] for an explanation of
     * groups and levels. The #effective_group is the group that
     * was actually used for the translation; some keys such as
     * Enter are not affected by the active keyboard group. The
     * #level is derived from #state. For convenience,
     * #GdkEventKey already contains the translated keyval, so
     * this function isn’t as useful as you might think.
     *
     * #consumed_modifiers gives modifiers that should be masked
     * outfrom #state when comparing this key press to a hot key.
     * For instance, on a US keyboard, the `plus` symbol is
     * shifted, so when comparing a key press to a
     * `<Control>plus` accelerator `<Shift>` should be masked
     * out.
     *
     * |[<!-- language="C" -->
     *
     ...
     * ALL_ACCELS_MASK) == GDK_CONTROL_MASK) // Control was
     * pressed ]|
     *
     * An older interpretation #consumed_modifiers was that it
     * contained all modifiers that might affect the translation
     * of the key; this allowed accelerators to be stored with
     * irrelevant consumed modifiers, by doing:
     *
     * |[<!-- language="C" -->
     *
     * // XXX Don’t do this XXX if (keyval == accel_keyval &&
     * (event->state & ~consumed & ALL_ACCELS_MASK) ==
     * (accel_mods & ~consumed)) // Accelerator was pressed ]|
     *
     * However, this did not work if multi-modifier combinations
     * were used in the keymap, since, for instance, `<Control>`
     * would be masked out even if only `<Control><Alt>` was used
     * in the keymap. To support this usage as well as well as
     * possible, all single modifier combinations that could
     * affect the key for any combination of modifiers will be
     * returned in #consumed_modifiers; multi-modifier
     * combinations are returned only when actually found in
     * #state. When you store accelerators, you should always
     * store them with consumed modifiers removed. Store
     * `<Control>plus`, not `<Control><Shift>plus`,
     *
     * @param hardware_keycode A keycode
     * @param state A modifier state
     * @param group Active keyboard group
     * @returns %TRUE if there was a keyval bound to the
     *   keycode/state/group
     *
     ...
     *
     *   Return location for modifiers that were used to determine
     *   the group or level, or %NULL
     */
    ...
    /**
     * The ::direction-changed signal gets emitted when the
     ...
     * the keyboard changes, e.g when Caps Lock is turned on or
     * off. See {@link Gdk.Keymap.get_caps_lock_state}.
     *
     * @param signal
     * @param callback Callback function
     *
   ...
   * (including a hardware key, a modifier mask, and active
   * keyboard group) to a keyval. This translation has two
   * phases. The first phase is to determine the effective
   * keyboard group and level for the keyboard state; the second
   * phase is to look up the keycode/group/level triplet in the
   * keymap and see what keyval it corresponds to.
   */
  ...
  class Keymap {
    ...
    /**
     * @deprecated Use {@link Gdk.Keymap.get_for_display} instead
     *
     *   Returns the {@link Keymap} attached to the default display.
     * @returns The {@link Keymap} attached to the default
     *   display.
     */
  ...
  interface IMonitor {
    ...
    get_display(): Display;
    /**
     * Retrieves the size and position of an individual monitor
     * within the display coordinate space. The returned geometry
     * is in ”application pixels”, not in ”device pixels” (see
     * {@link Gdk.Monitor.get_scale_factor}).
     *
     * @returns A {@link Rectangle} to be filled with the monitor
     *   geometry
     */
    ...
    /**
     * Gets the name or PNP ID of the monitor's manufacturer, if
     * available.
     *
    ...
    /**
     * Gets the a string identifying the monitor model, if
     * available.
     *
    ...
    /**
     * Gets the internal scale factor that maps from monitor
     * coordinates to the actual device pixels. On traditional
     * systems this is 1, but on very high density outputs this
     * can be a higher value (often 2).
     *
     * This can be used if you want to create pixel based data
     * for a particular monitor, but most of the time you’re
     * drawing to a window where it is better to use
     * {@link Gdk.Window.get_scale_factor} instead.
     *
    ...
    get_width_mm(): number;
    /**
     * Retrieves the size and position of the “work area” on a
     * monitor within the display coordinate space. The returned
     * geometry is in ”application pixels”, not in ”device
     * pixels” (see {@link Gdk.Monitor.get_scale_factor}).
     *
     * The work area should be considered when positioning menus
     * and similar popups, to avoid placing them below panels,
     * docks or other desktop components.
     *
     * Note that not all backends may have a concept of workarea.
     * This function will return the monitor geometry if a
     * workarea is not available, or does not apply.
     *
     * @returns A {@link Rectangle} to be filled with the monitor
     *   workarea
     */
    ...
    connect(
      signal: "notify::geometry",
      callback: (owner: this, ...args: any) => void
    ): number;
    connect(
      signal: "notify::height-mm",
      callback: (owner: this, ...args: any) => void
    ): number;
    connect(
      signal: "notify::manufacturer",
      callback: (owner: this, ...args: any) => void
    ): number;
    connect(
      signal: "notify::model",
    ...
    connect(
      signal: "notify::refresh-rate",
      callback: (owner: this, ...args: any) => void
    ): number;
    connect(
      signal: "notify::scale-factor",
      callback: (owner: this, ...args: any) => void
    ): number;
    connect(
      signal: "notify::subpixel-layout",
      callback: (owner: this, ...args: any) => void
    ): number;
    connect(
      signal: "notify::width-mm",
      callback: (owner: this, ...args: any) => void
    ): number;
    connect(
      signal: "notify::workarea",
      callback: (owner: this, ...args: any) => void
    ): number;
  }

  type MonitorInitOptionsMixin = GObject.ObjectInitOptions &
    Pick<IMonitor, "display">;
   ...
   * associated with a {@link Display}. GdkDisplay has APIs to
   * enumerate monitors with {@link Gdk.Display.get_n_monitors}
   * and gdk_display_get_monitor(), and to find particular
   * monitors with gdk_display_get_primary_monitor() or
   * gdk_display_get_monitor_at_window().
   *
   * GdkMonitor was introduced in GTK+ 3.22 and supersedes
   * earlier APIs in GdkScreen to obtain monitor-related
   * information.
   */
  ...
  interface IScreen {
     ...
     *
     * On X11, this is done by inspecting the _NET_ACTIVE_WINDOW
     * property on the root window, as described in the [Extended
     * Window Manager
     * Hints](http://www.freedesktop.org/Standards/wm-spec). If
     * there is no currently currently active window, or the
     * window manager does not support the _NET_ACTIVE_WINDOW
     * hint, this function returns %NULL.
     *
     * On other platforms, this function may return %NULL,
     * depending on whether it is implementable on that
     * platform.
     *
     ...
     * {@link Gdk.Screen.set_font_options}.
     *
     ...
     *
     *   Gets the height of #screen in pixels. The returned size is
     ...
     *
     *   Note that this value is somewhat ill-defined when the
     *   screen has multiple monitors of different resolution. It
     *   is recommended to use the monitor dimensions instead.
     * @returns The heigth of #screen in millimeters.
     */
    get_height_mm(): number;
    /**
     * @deprecated Use {@link Gdk.Display.get_monitor_at_point}
     *   instead
     *
     *   Returns the monitor number in which the point (#x,#y) is
     *   located.
     ...
     *   lies, or a monitor close to (#x,#y) if the point is not
     *   in any monitor.
     */
    get_monitor_at_point(x: number, y: number): number;
    /**
     * @deprecated Use {@link Gdk.Display.get_monitor_at_window}
     *   instead
     *
     *   Returns the number of the monitor in which the largest
     *   area of the bounding rectangle of #window resides.
     * @param window A {@link Window}
     * @returns The monitor number in which most of #window is
     *   located, or if #window does not intersect any monitors,
     *   a monitor, close to #window.
     */
    get_monitor_at_window(window: Window): number;
    /**
     * @deprecated Use {@link Gdk.Monitor.get_geometry} instead
     *
     *   Retrieves the {@link Rectangle} representing the size and
     *   position of the individual monitor within the entire
     *   screen area. The returned geometry is in ”application
     *   pixels”, not in ”device pixels” (see
     *   {@link Gdk.Screen.get_monitor_scale_factor}).
     *
     *   Monitor numbers start at 0. To obtain the number of
     *   monitors of #screen, use gdk_screen_get_n_monitors().
     *
     *   Note that the size of the entire screen area can be
     *   retrieved via gdk_screen_get_width() and
    ...
    /**
     * @deprecated Use {@link Gdk.Monitor.get_height_mm} instead
     *
    ...
    /**
     * @deprecated Use {@link Gdk.Monitor.get_model} instead
     *
     *   Returns the output name of the specified monitor. Usually
     *   something like VGA, DVI, or TV, not the actual product
     *   name of the display device.
     * @param monitor_num Number of the monitor, between 0 and
     *   gdk_screen_get_n_monitors (screen)
     * @returns A newly-allocated string containing the name of
     *   the monitor, or %NULL if the name cannot be determined
     */
    get_monitor_plug_name(monitor_num: number): string | null;
    /**
     * @deprecated Use {@link Gdk.Monitor.get_scale_factor}
     *   instead
     *
     *   Returns the internal scale factor that maps from monitor
     *   coordinates to the actual device pixels. On traditional
     *   systems this is 1, but on very high density outputs this
     *   can be a higher value (often 2).
     *
    ...
    /**
     * @deprecated Use {@link Gdk.Monitor.get_width_mm} instead
     *
     *   Gets the width in millimeters of the specified monitor, if
     *   available.
     * @param monitor_num Number of the monitor, between 0 and
     *   gdk_screen_get_n_monitors (screen)
    ...
    /**
     * @deprecated Use {@link Gdk.Monitor.get_workarea} instead
     *
     *   Retrieves the {@link Rectangle} representing the size and
     *   position of the “work area” on a monitor within the
     *   entire screen area. The returned geometry is in
    ...
    /**
     * @deprecated Use {@link Gdk.Display.get_n_monitors} instead
     *
    ...
    get_number(): number;
    /**
     * @deprecated Use {@link Gdk.Display.get_primary_monitor}
     *   instead
     *
     *   Gets the primary monitor for #screen. The primary monitor
     *   is considered the monitor where the “main desktop”
     *   lives. While normal application windows typically allow
     *   the window manager to place the windows, specialized
     *   desktop applications such as panels should place
     *   themselves on the primary monitor.
     *
     *   If no primary monitor is configured by the user, the
     *   return value will be 0, defaulting to the first
     *   monitor.
     * @returns An integer index for the primary monitor, or 0 if
     *   none is configured.
     */
     ...
     * channel. The windowing system on which GTK+ is running may
     * not support this capability, in which case %NULL will be
     * returned. Even if a non-%NULL value is returned, its
     * possible that the window’s alpha channel won’t be honored
     * when displaying the window on the screen: in particular,
     * for X an appropriate windowing manager and compositing
     * manager must be running to provide appropriate display.
     *
     * This functionality is not implemented in the Windows
     * backend.
     *
     ...
     * @returns %TRUE if the setting existed and a value was
     *   stored in #value, %FALSE otherwise.
     */
    get_setting(name: string, value: GObject.Value): boolean;
    /**
     * Get the system’s default visual for #screen. This is the
     * visual for the root window of the display. The return
    ...
    /**
     * Obtains a list of all toplevel windows known to GDK on the
     * screen #screen. A toplevel window is a child of the root
     * window (see {@link Gdk.get.default_root_window}).
     *
     * The returned list should be freed with g_list_free(), but
     * its elements need not be freed.
     *
     * @returns List of toplevel windows, free with
     *   {@link G.list_free}
     */
     ...
     * @deprecated Use per-monitor information instead
     *
     *   Gets the width of #screen in pixels. The returned size is
     *   in ”application pixels”, not in ”device pixels” (see
     *   {@link Gdk.Screen.get_monitor_scale_factor}).
    ...
    get_width_mm(): number;
    /**
     * Returns a #GList of {@link Windows} representing the
     * current window stack.
     *
     * On X11, this is done by inspecting the
     * _NET_CLIENT_LIST_STACKING property on the root window, as
     * described in the [Extended Window Manager
     * Hints](http://www.freedesktop.org/Standards/wm-spec). If
     * the window manager does not support the
     ...
     * platform.
     *
     * The returned list is newly allocated and owns references
     * to the windows it contains, so it should be freed using
     * {@link G.list_free} and its windows unrefed using
     * g_object_unref() when no longer needed.
     *
     * @returns A list of {@link Windows} for the current window
     *   stack, or %NULL.
     */
     ...
     * is compositing #screen.
     *
     * @returns Whether windows with RGBA visuals can reasonably
     *   be expected to have their alpha channels drawn correctly
     *   on the screen.
     */
    is_composited(): boolean;
    /**
     * Lists the available visuals for the specified #screen. A
     * visual describes a hardware image data format. For
     * example, a visual might support 24-bit color, or 8-bit
     * color, and might expect pixels to be in a certain format.
     *
     * Call {@link G.list_free} on the return value when you’re
     * finished with it.
     *
     * @returns A list of visuals; the list must be freed, but
     *   not its contents
     */
    list_visuals(): GLib.List;
    /**
     * Determines the name to pass to {@link Gdk.Display.open} to
     * get a {@link Display} with this screen as the default
     * screen.
     *
    ...
    /**
     * Sets the resolution for font handling on the screen. This
     * is a scale factor between points specified in a
     * #PangoFontDescription and cairo units. The default value
     * is 96, meaning that a 10 point font will be 13 units high.
     * (10 * 96. / 72. = 13.3).
     *
     * @param dpi The resolution in “dots per inch”. (Physical
     *   inches aren’t actually involved; the terminology is
     *   conventional.)
     */
    set_resolution(dpi: number): void;
    /**
     * The ::composited-changed signal is emitted when the
     ...
     * size or position of the monitors attached to the screen
     * change.
     *
     * Only for X11 and OS X for now. A future implementation for
     * Win32 may be a possibility.
     *
     * @param signal
     * @param callback Callback function
     *
    ...
    connect(
      signal: "notify::font-options",
      callback: (owner: this, ...args: any) => void
    ): number;
    connect(
      signal: "notify::resolution",
      callback: (owner: this, ...args: any) => void
    ): number;
  }

  type ScreenInitOptionsMixin = GObject.ObjectInitOptions &
    Pick<IScreen, "font_options" | "resolution">;
  ...
  /**
   * {@link Screen} objects are the GDK representation of the
   * screen on which windows can be displayed and on which the
   * pointer moves. X originally identified screens with physical
   * screens, but nowadays it is more common to have a single
   * #GdkScreen which combines several physical monitors (see
   * {@link Gdk.Screen.get_n_monitors}).
   *
   * GdkScreen is used throughout GDK and GTK+ to specify which
   * screen the top level windows are to be displayed on. it is
   * also used to query the screen specification and default
   * settings such as the default visual
   * (gdk_screen_get_system_visual()), the dimensions of the
   * physical monitors (gdk_screen_get_monitor_geometry()), etc.
   */
  ...
  class Screen {
     ...
     *
     * @returns A {@link Screen}, or %NULL if there is no default
     *   display.
     */
     ...
     *
     *   Gets the height of the default screen in pixels. The
     *   returned size is in ”application pixels”, not in ”device
     *   pixels” (see
     *   {@link Gdk.Screen.get_monitor_scale_factor}).
     ...
     *   Note that on many X servers this value will not be
     *   correct.
     * @returns The height of the default screen in millimeters,
     ...
     * @deprecated Use per-monitor information
     *
     *   Gets the width of the default screen in pixels. The
     *   returned size is in ”application pixels”, not in ”device
     *   pixels” (see
     *   {@link Gdk.Screen.get_monitor_scale_factor}).
  ...
  interface ISeat {
     ...
     *   capabilities. This object is owned by GTK+ and must not
     *   be freed.
     */
     ...
     *
     * @returns A master {@link Device} with pointer capabilities.
     *   This object is owned by GTK+ and must not be freed.
     */
    get_pointer(): Device | null;
    /**
     * Returns the slave devices that match the given
     * capabilities.
     *
     * @param capabilities Capabilities to get devices for
     * @returns A list of {@link Devices}. The list must be freed
     *   with {@link G.list_free}, the elements are owned by GDK
     *   and must not be freed.
     */
    get_slaves(capabilities: SeatCapabilities): GLib.List;
    /**
     * Grabs the seat so that all events corresponding to the
     * given #capabilities are passed to this application until
     * the seat is ungrabbed with {@link Gdk.Seat.ungrab}, or the
     * window becomes hidden. This overrides any previous grab on
     * the seat by this client.
     *
     * As a rule of thumb, if a grab is desired over
     * %GDK_SEAT_CAPABILITY_POINTER, all other "pointing"
     * capabilities (eg. %GDK_SEAT_CAPABILITY_TOUCH) should be
     * grabbed too, so the user is able to interact with all of
     * those while the grab holds, you should thus use
     * %GDK_SEAT_CAPABILITY_ALL_POINTING most commonly.
     *
     * Grabs are used for operations which need complete control
     * over the events corresponding to the given capabilities.
     * For example in GTK+ this is used for Drag and Drop
     * operations, popup menus and such.
     *
     * Note that if the event mask of a {@link Window} has
     * selected both button press and button release events, or
     * touch begin and touch end, then a press event will cause
     * an automatic grab until the button is released, equivalent
     * to a grab on the window with #owner_events set to %TRUE.
     * This is done because most applications expect to receive
     * paired press and release events.
     *
     * If you set up anything at the time you take the grab that
     * needs to be cleaned up when the grab ends, you should
     * handle the #GdkEventGrabBroken events that are emitted
     ...
     *   In either mode, unreported events are discarded.
     * @param cursor The cursor to display while the grab is
     *   active. If this is %NULL then the normal cursors are
     *   used for #window and its descendants, and the cursor for
     *   #window is used elsewhere.
     * @param event The event that is triggering the grab, or
     *   %NULL if none is available.
     * @param prepare_func Function to prepare the window to be
     *   grabbed, it can be %NULL if #window is visible before
     *   this call.
     * @param prepare_func_data User data to pass to
     *   #prepare_func
     * @returns %GDK_GRAB_SUCCESS if the grab was successful.
     */
    grab(
    ...
    /**
     * The ::device-removed signal is emitted when an input
     * device is removed (e.g. unplugged).
     *
     * @param signal
     * @param callback Callback function
     *
    ...
    /**
     * The ::tool-added signal is emitted whenever a new tool is
     * made known to the seat. The tool may later be assigned to
     * a device (i.e. on proximity with a tablet). The device
     * will emit the {@link Device.tool_changed} signal
     * accordingly.
     *
  ...
  /**
   * The {@link Seat} object represents a collection of input
  ...
  interface IVisual {
    /**
     * @deprecated Use {@link Gdk.Visual.get_red_pixel_details}
     *   and its variants to learn about the pixel layout of
     *   TrueColor and DirectColor visuals
     *
     *   Returns the number of significant bits per red, green and
     *   blue value.
     *
     *   Not all GDK backend provide a meaningful value for this
     *   function.
     * @returns The number of significant bits per color value
     *   for #visual.
     */
    get_bits_per_rgb(): number;
    /**
     * Obtains values that are needed to calculate blue pixel
     * values in TrueColor and DirectColor. The “mask” is the
     * significant bits within the pixel. The “shift” is the
     * number of bits left we must shift a primary for it to be
     * in position (according to the "mask"). Finally,
     * "precision" refers to how much precision the pixel value
     ...
     * @returns A {@link ByteOrder} stating the byte order of
     *   #visual.
     */
    get_byte_order(): ByteOrder;
    /**
     * @deprecated This information is not useful, since GDK does
     *   not provide APIs to operate on colormaps.
     *
     *   Returns the size of a colormap for this visual.
     *
     *   You have to use platform-specific APIs to manipulate
     *   colormaps.
     * @returns The size of a colormap that is suitable for
     *   #visual.
     */
    ...
    get_depth(): number;
    /**
     * Obtains values that are needed to calculate green pixel
     * values in TrueColor and DirectColor. The “mask” is the
     * significant bits within the pixel. The “shift” is the
     * number of bits left we must shift a primary for it to be
     * in position (according to the "mask"). Finally,
    ...
    /**
     * Obtains values that are needed to calculate red pixel
     * values in TrueColor and DirectColor. The “mask” is the
     * significant bits within the pixel. The “shift” is the
     * number of bits left we must shift a primary for it to be
     * in position (according to the "mask"). Finally,
  ...
  /**
   * A {@link Visual} contains information about a particular
   * visual.
   */
  ...
  class Visual {
     ...
     *   {@link Gdk.Screen.get_system_visual} and
     *   gdk_screen_get_rgba_visual()
     *
     *   Get the visual with the most available colors for the
     *   default GDK screen. The return value should not be
     *   freed.
     * @returns Best visual
     */
     ...
     *   “Best” means “largest,” i.e. 32 preferred over 24
     ...
     *   gdk_screen_get_rgba_visual()
     *
     *   Return the best available visual type for the default GDK
     *   screen.
     ...
     * @returns Best visual with both #depth and #visual_type, or
     *   %NULL if none
     */
    public static get_best_with_both(
      depth: number,
     ...
     *   The return value should not be freed. %NULL may be
     ...
     *   default GDK screen. Visuals with higher color depths are
     *   considered better. The return value should not be freed.
     *   %NULL may be returned if no visual has type
     *   #visual_type.
     * @param visual_type A visual type
    ...
    /**
     * @deprecated Use gdk_screen_get_system_visual
     *   (gdk_screen_get_default ()).
     *
     *   Get the system’s default visual for the default GDK
  ...
  interface IWindow {
    /**
     * The mouse pointer for a {@link Window}. See
     * {@link Gdk.Window.set_cursor} and gdk_window_get_cursor()
     * for details.
     */
    cursor: Cursor;
    /**
     * Adds an event filter to #window, allowing you to intercept
     * events before they reach GDK. This is a low-level
     * operation and makes it easy to break GDK and/or GTK+, so
     * you have to know what you're doing. Pass %NULL for #window
     * to get all events for all windows, instead of events for a
     * specific window.
     *
     * If you are interested in X GenericEvents, bear in mind
     * that XGetEventData() has been already called on the event,
     * and XFreeEventData() must not be called within #function.
     *
    ...
    /**
     * Emits a short beep associated to #window in the
     * appropriate display, if supported. Otherwise, emits a
     * short beep on the display just as
     * {@link Gdk.Display.beep}.
     */
    beep(): void;
    /**
     * Indicates that you are beginning the process of redrawing
     * #region on #window, and provides you with a
     * {@link DrawingContext}.
     *
     * If #window is a top level #GdkWindow, backed by a native
     * window implementation, a backing store (offscreen buffer)
     * large enough to contain #region will be created. The
     * backing store will be initialized with the background
     * color or background surface for #window. Then, all drawing
     * operations performed on #window will be diverted to the
     * backing store. When you call {@link Gdk.Window.end_frame},
     * the contents of the backing store will be copied to
     * #window, making it visible on screen. Only the part of
     * #window contained in #region will be modified; that is,
     * drawing operations are clipped to #region.
     *
     * The net result of all this is to remove flicker, because
     * the user sees the finished product appear all at once when
     * you call gdk_window_end_draw_frame(). If you draw to
     * #window directly without calling
     * gdk_window_begin_draw_frame(), the user may see flicker as
     * individual drawing operations are performed in sequence.
     *
     * When using GTK+, the widget system automatically places
     * calls to gdk_window_begin_draw_frame() and
     * gdk_window_end_draw_frame() around emissions of the
     * `GtkWidget::draw` signal. That is, if you’re drawing the
     * contents of the widget yourself, you can assume that the
     * widget has a cleared background, is already set as the
     * clip region, and already has a backing store. Therefore in
     * most cases, application code in GTK does not need to call
     * gdk_window_begin_draw_frame() explicitly.
     *
     * @param region A Cairo region
     * @returns A {@link DrawingContext} context that should be
     *   used to draw the contents of the window; the returned
     *   context is owned by GDK.
     */
     ...
     * {@link Gdk.Window.begin_move_drag_for_device} to begin a
     * drag with a different device.
     *
     * @param button The button being used to drag, or 0 for a
     *   keyboard-initiated drag
     * @param root_x Root window X coordinate of mouse click that
     *   began the drag
     * @param root_y Root window Y coordinate of mouse click that
     *   began the drag
     * @param timestamp Timestamp of mouse click that began the
     *   drag
     */
    begin_move_drag(
      button: number,
    ...
    /**
     * Begins a window move operation (for a toplevel window).
     * You might use this function to implement a “window move
     * grip,” for example. The function works best with window
     * managers that support the [Extended Window Manager
     * Hints](http://www.freedesktop.org/Standards/wm-spec) but
    ...
    /**
     * @deprecated Use {@link Gdk.Window.begin_draw_frame} instead
     *
     *   A convenience wrapper around
     *   {@link Gdk.Window.begin_paint_region} which creates a
     *   rectangular region for you. See
     ...
     *
     *   Indicates that you are beginning the process of redrawing
     *   #region. A backing store (offscreen buffer) large enough
     *   to contain #region will be created. The backing store
     *   will be initialized with the background color or
     *   background surface for #window. Then, all drawing
     *   operations performed on #window will be diverted to the
     *   backing store. When you call
     *   {@link Gdk.Window.end_paint}, the backing store will be
     *   copied to #window, making it visible onscreen. Only the
     *   part of #window contained in #region will be modified;
     *   that is, drawing operations are clipped to #region.
     *
     *   The net result of all this is to remove flicker, because
     *   the user sees the finished product appear all at once
     *   when you call gdk_window_end_paint(). If you draw to
     *   #window directly without calling
     *   gdk_window_begin_paint_region(), the user may see
     *   flicker as individual drawing operations are performed
     *   in sequence. The clipping and background-initializing
     *   features of gdk_window_begin_paint_region() are
     *   conveniences for the programmer, so you can avoid doing
     *   that work yourself.
     *
     *   When using GTK+, the widget system automatically places
     *   calls to gdk_window_begin_paint_region() and
     *   gdk_window_end_paint() around emissions of the
     *   expose_event signal. That is, if you’re writing an
     *   expose event handler, you can assume that the exposed
     *   area in {@link EventExpose} has already been cleared to
     *   the window background, is already set as the clip
     *   region, and already has a backing store. Therefore in
     *   most cases, application code need not call
     *   gdk_window_begin_paint_region(). (You can disable the
     *   automatic calls around expose events on a
     *   widget-by-widget basis by calling
     *   gtk_widget_set_double_buffered().)
     *
     *   If you call this function multiple times before calling
     *   the matching gdk_window_end_paint(), the backing stores
     *   are pushed onto a stack. gdk_window_end_paint() copies
     *   the topmost backing store onscreen, subtracts the
     *   topmost region from all other regions in the stack, and
     *   pops the stack. All drawing operations affect only the
     *   topmost backing store in the stack. One matching call to
     ...
     * This function assumes that the drag is controlled by the
     * client pointer device, use
     * {@link Gdk.Window.begin_resize_drag_for_device} to begin a
     * drag with a different device.
     *
     * @param edge The edge or corner from which the drag is
     *   started
     * @param button The button being used to drag, or 0 for a
     *   keyboard-initiated drag
     * @param root_x Root window X coordinate of mouse click that
     ...
     *   began the drag
     * @param timestamp Timestamp of mouse click that began the
     *   drag (use {@link Gdk.event.get_time})
     */
    begin_resize_drag(
      edge: WindowEdge,
      button: number,
     ...
     * grip,” for example; in fact #GtkStatusbar uses it. The
     * function works best with window managers that support the
    ...
    /**
     * Transforms window coordinates from a parent window to a
     * child window, where the parent window is the normal parent
     * as returned by {@link Gdk.Window.get_parent} for normal
     * windows, and the window's embedder as returned by
     * gdk_offscreen_window_get_embedder() for offscreen
     * windows.
     *
     * For normal windows, calling this function is equivalent to
     * subtracting the return values of gdk_window_get_position()
     * from the parent coordinates. For offscreen windows however
     * (which can be arbitrarily transformed), this function
     * calls the GdkWindow::from-embedder: signal to translate
     * the coordinates.
     *
     * You should always use this function when writing generic
     * code that walks down a window hierarchy.
     *
     ...
     * @returns Return location for X coordinate in child’s
     *   coordinate system
     *
     *   Return location for Y coordinate in child’s coordinate
     *   system
     */
    coords_from_parent(
      parent_x: number,
     ...
     * parent window, where the parent window is the normal
     * parent as returned by {@link Gdk.Window.get_parent} for
     * normal windows, and the window's embedder as returned by
     * gdk_offscreen_window_get_embedder() for offscreen
     * windows.
     *
     * For normal windows, calling this function is equivalent to
     * adding the return values of gdk_window_get_position() to
     * the child coordinates. For offscreen windows however
     * (which can be arbitrarily transformed), this function
     * calls the GdkWindow::to-embedder: signal to translate the
     * coordinates.
     *
     * You should always use this function when writing generic
     * code that walks up a window hierarchy.
     *
     * See also: gdk_window_coords_from_parent()
     *
     * @param x X coordinate in child’s coordinate system
     * @param y Y coordinate in child’s coordinate system
     * @returns Return location for X coordinate in parent’s
     *   coordinate system, or %NULL
     *
     *   Return location for Y coordinate in parent’s coordinate
     *   system, or %NULL
     */
    coords_to_parent(
      x: number,
    ...
    /**
     * Creates a new {@link GLContext} matching the framebuffer
     * format to the visual of the #GdkWindow. The context is
     * disconnected from any particular window or surface.
     *
     * If the creation of the #GdkGLContext failed, #error will
     * be set.
     *
     * Before using the returned #GdkGLContext, you will need to
     * call {@link Gdk.gl_context_make_current} or
     * gdk_gl_context_realize().
     *
     * @returns The newly created {@link GLContext}, or %NULL on
     *   error
     */
    create_gl_context(): GLContext;
    /**
     * Create a new image surface that is efficient to draw on
     * the given #window.
     *
     * Initially the surface contents are all 0 (transparent if
     * contents have transparency, black otherwise.)
     *
     * The #width and #height of the new surface are not affected
     * by the scaling factor of the #window, or by the #scale
     * argument; they are the size of the surface in device
     * pixels. If you wish to create an image surface capable of
     * holding the contents of #window you can use:
     *
     * |[<!-- language="C" -->
     *
     * Int scale = gdk_window_get_scale_factor (window); int
     * width = gdk_window_get_width (window) * scale; int height
     * = gdk_window_get_height (window) * scale;
     *
     * // format is set elsewhere cairo_surface_t *surface =
     * gdk_window_create_similar_image_surface (window, format,
     * width, height, scale); ]|
     *
     * Note that unlike
     * {@link Cairo.Surface.create_similar_image}, the new
     * surface's device scale is set to #scale, or to the scale
     * factor of #window if #scale is 0.
     *
     * @param format The format for the new surface
     * @param width Width of the new surface
     * @param height Height of the new surface
     * @param scale The scale of the new surface, or 0 to use
     *   same as #window
     * @returns A pointer to the newly allocated surface. The
     *   caller owns the surface and should call
     *   {@link Cairo.Surface.destroy} when done with it.
     *
     *   This function always returns a valid pointer, but it will
     *   return a pointer to a “nil” surface if #other is already
     ...
     * with the given #window. For example the new surface will
     * have the same fallback resolution and font options as
     * #window. Generally, the new surface will also use the same
     * backend as #window, unless that is not possible for some
     * reason. The type of the returned surface may be examined
     * with {@link Cairo.Surface.get_type}.
     *
     * Initially the surface contents are all 0 (transparent if
     * contents have transparency, black otherwise.)
     *
     ...
     * #window and decrements #window's reference count. The
     * window system resources for all children of #window are
     * also destroyed, but the children’s reference counts are
     * not decremented.
     *
     ...
     * started with {@link Gdk.Window.begin_frame} has been
     * completed.
     *
     ...
     *
     * @param context The {@link DrawingContext} created by
     *   {@link Gdk.Window.begin_draw_frame}
     */
    end_draw_frame(context: DrawingContext): void;
    /**
     * Indicates that the backing store created by the most
     * recent call to {@link Gdk.Window.begin_paint_region} should
     * be copied onscreen and deleted, leaving the
     * next-most-recent backing store or no backing store at all
     * as the active paint region. See
    ...
    end_paint(): void;
    /**
     * Tries to ensure that there is a window-system native
     * window for this GdkWindow. This may fail in some
     * situations, returning %FALSE.
     *
     * Offscreen window and children of them can never have
     * native windows.
     *
     * Some backends may not support native child windows.
     *
     * @returns %TRUE if the window has a native window, %FALSE
     *   otherwise
     */
     ...
     *   of GTK+
     *
     *   Temporarily freezes a window and all its descendants such
     *   that it won't receive expose events. The window will
     *   begin receiving expose events again when
     *   {@link Gdk.Window.thaw_toplevel_updates_libgtk_only} is
     *   called. If
     *   gdk_window_freeze_toplevel_updates_libgtk_only() has
     *   been called more than once,
     *   gdk_window_thaw_toplevel_updates_libgtk_only() must be
     *   called an equal number of times to begin processing
     *   exposes.
     *
     *   This function is not part of the GDK public API and is
     *   only for use by GTK+.
     */
    freeze_toplevel_updates_libgtk_only(): void;
    /**
     * Temporarily freezes a window such that it won’t receive
     * expose events. The window will begin receiving expose
     * events again when {@link Gdk.Window.thaw_updates} is
     * called. If gdk_window_freeze_updates() has been called
     * more than once, gdk_window_thaw_updates() must be called
     * an equal number of times to begin processing exposes.
     */
    freeze_updates(): void;
    /**
     * Moves the window into fullscreen mode. This means the
     * window covers the entire screen and is above any panels or
     * task bars.
     *
     * If the window was already fullscreen, then this function
     * does nothing.
     *
     * On X11, asks the window manager to put #window in a
     * fullscreen state, if the window manager supports this
     * operation. Not all window managers support this, and some
     * deliberately ignore it or don’t have a concept of
     * “fullscreen”; so you can’t rely on the fullscreenification
     * actually happening. But it will happen with most standard
     * window managers, and GDK makes a best effort to get it to
     * happen.
     */
    fullscreen(): void;
    /**
     * Moves the window into fullscreen mode on the given
     * monitor. This means the window covers the entire screen
     * and is above any panels or task bars.
     *
     * If the window was already fullscreen, then this function
     * does nothing.
     *
     ...
     * offscreen window has changed. This is necessary for GDK to
     * keep track of which offscreen window the pointer is in.
     */
    geometry_changed(): void;
    /**
     * Determines whether or not the desktop environment shuld be
     * hinted that the window does not want to receive input
     * focus.
     *
     * @returns Whether or not the window should receive input
     *   focus.
     */
     ...
     * @returns The pattern to use for the background or %NULL if
     *   there is no background.
     */
    get_background_pattern(): cairo.Pattern | null;
    /**
     * Gets the list of children of #window known to GDK. This
     * function only returns children created via GDK, so for
     * example it’s useless when used with the root window; it
     * only returns windows an application created itself.
     *
     ...
     * list need not be.
     *
     * The list is returned in (relative) stacking order, i.e.
     * the lowest window is first.
     *
     * @returns List of child windows inside #window
     */
    get_children_with_user_data(): GLib.List;
    /**
     * Computes the region of a window that potentially can be
     * written to by drawing primitives. This region may not take
     ...
     *
     * @returns A #cairo_region_t. This must be freed with
     *   {@link Cairo.Region.destroy} when you are done.
     */
    ...
    /**
     * Retrieves a {@link Cursor} pointer for the cursor currently
     * set on the specified #GdkWindow, or %NULL. If the return
     * value is %NULL then there is no custom cursor set on the
     * specified window, and it is using the cursor for its
     * parent window.
     *
     * @returns A {@link Cursor}, or %NULL. The returned object is
     *   owned by the #GdkWindow and should not be unreferenced
     *   directly. Use {@link Gdk.Window.set_cursor} to unset the
     ...
     *
     * @returns %TRUE if the window has decorations set, %FALSE
     *   otherwise.
     *
    ...
    /**
     * Retrieves a {@link Cursor} pointer for the #device
     * currently set on the specified #GdkWindow, or %NULL. If
     * the return value is %NULL then there is no custom cursor
     * set on the specified window, and it is using the cursor
    ...
    /**
     * Returns the event mask for #window corresponding to an
     * specific device.
     *
     * @param device A {@link Device}.
     ...
     *
     * Use {@link Gdk.Window.get_device_position_double} if you
     * need subpixel precision.
     *
     * @param device Pointer {@link Device} to query to.
     * @returns The window underneath #device (as with
     *   {@link Gdk.Device.get_window_at_position}), or %NULL if
     *   the window is not known to GDK.
     *
    ...
    get_device_position(
      device: Device
    ): [Window | null, number | null, number | null, ModifierType | null];
    /**
     * Obtains the current device position in doubles and
     * modifier state. The position is given in coordinates
     ...
     *
     *   Location of the window where the drop should happen. This
     *   may be #window or a proxy window, or %NULL if #window
     *   does not support Drag and Drop.
     */
    get_drag_protocol(): [DragProtocol, Window | null];
    /**
     * Obtains the parent of #window, as known to GDK. Works like
     * {@link Gdk.Window.get_parent} for normal windows, but
     ...
     *
     * Works like {@link Gdk.Window.get_toplevel}, but treats an
     * offscreen window's embedder as its parent, using
    ...
    /**
     * Gets the event mask for #window for all master input
     * devices. See {@link Gdk.Window.set_events}.
     *
     * @returns Event mask for #window
     */
    get_events(): EventMask;
    /**
     * Determines whether or not the desktop environment should
     * be hinted that the window does not want to receive input
     * focus when it is mapped.
     *
     * @returns Whether or not the window wants to receive input
     *   focus when it is mapped.
     */
    get_focus_on_map(): boolean;
    /**
     * Gets the frame clock for the window. The frame clock for a
     * window never changes unless the window is reparented to a
     * new toplevel window.
     *
    ...
    /**
     * Obtains the bounding box of the window, including window
     * manager titlebar/borders if any. The frame position is
     * given in root window coordinates. To get the position of
     * the window itself (rather than the frame) in root window
     * coordinates, use {@link Gdk.Window.get_origin}.
     *
     * @returns Rectangle to fill with bounding box of the window
     *   frame
     */
     ...
     *
     * @returns The {@link FullscreenMode} applied to the window
     *   when fullscreen.
     */
    get_fullscreen_mode(): FullscreenMode;
    /**
     * Any of the return location arguments to this function may
     * be %NULL, if you aren’t interested in getting the value of
     * that field.
     *
     * The X and Y coordinates returned are relative to the
     * parent window of #window, which for toplevels usually
     * means relative to the window decorations (titlebar, etc.)
     * rather than relative to the root window (screen-size
     * background window).
     *
     * On the X11 platform, the geometry is obtained from the X
     * server, so reflects the latest position of #window; this
     * may be out-of-sync with the position of #window delivered
     * in the most-recently-processed {@link EventConfigure}.
     * {@link Gdk.Window.get_position} in contrast gets the
     * position from the most recent configure event.
     *
     * Note: If #window is not a toplevel, it is much better to
     * call gdk_window_get_position(), gdk_window_get_width() and
     * gdk_window_get_height() instead, because it avoids the
     * roundtrip to the X server and because these functions
     * support the full 32-bit coordinate space, whereas
     * gdk_window_get_geometry() is restricted to the 16-bit
     * coordinates of X11.
     *
     * @returns Return location for X coordinate of window
     *   (relative to its parent)
     *
     *   Return location for Y coordinate of window (relative to
     *   its parent)
     *
     ...
     *
     * On the X11 platform the returned size is the size reported
     * in the most-recently-processed configure event, rather
    ...
    get_height(): number;
    /**
     * Determines whether or not the window manager is hinted
     * that #window has modal behaviour.
     *
     * @returns Whether or not the window has the modal hint set.
     */
    get_modal_hint(): boolean;
    /**
     * Obtains the position of a window in root window
     * coordinates. (Compare with {@link Gdk.Window.get_position}
     * and gdk_window_get_geometry() which return the position of
     * a window relative to its parent window.)
     *
    ...
    /**
     * Obtains the parent of #window, as known to GDK. Does not
     * query the X server; thus this returns the parent as passed
     * to {@link Gdk.Window.new}, not the actual parent. This
     * should never matter unless you’re using Xlib calls mixed
     * with GDK calls on the X11 platform. It may also matter for
     * toplevel windows, because the window manager may choose to
     * reparent them.
     *
     * Note that you should use gdk_window_get_effective_parent()
     * when writing generic code that walks up a window
     * hierarchy, because gdk_window_get_parent() will most
     * likely not do what you expect if there are offscreen
    ...
    get_parent(): Window;
    /**
     * Returns whether input to the window is passed through to
     * the window below.
     *
    ...
    /**
     * @deprecated Use {@link Gdk.Window.get_device_position}
     *   instead.
     *
     *   Obtains the current pointer position and modifier state.
     *   The position is given in coordinates relative to the
     *   upper left corner of #window.
     * @returns The window containing the pointer (as with
     *   {@link Gdk.Window.at_pointer}), or %NULL if the window
     *   containing the pointer isn’t known to GDK
     *
     *   Return location for X coordinate of pointer or %NULL to
     *   not return the X coordinate
     *
     *   Return location for Y coordinate of pointer or %NULL to
     *   not return the Y coordinate
     *
     *   Return location for modifier mask or %NULL to not return
     *   the modifier mask
     */
    get_pointer(): [
      Window | null,
     ...
     * most-recently-processed {@link EventConfigure}. Contrast
     * with {@link Gdk.Window.get_geometry} which queries the X
     * server for the current window position, regardless of
     * which events have been received or processed.
     *
     * The position coordinates are relative to the window’s
     * parent window.
     *
    ...
    /**
     * Obtains the position of a window position in root window
     * coordinates. This is similar to
     * {@link Gdk.Window.get_origin} but allows you to pass in any
    ...
    /**
     * Obtains the top-left corner of the window manager frame in
    ...
    /**
     * Returns the internal scale factor that maps from window
     * coordiantes to the actual device pixels. On traditional
     * systems this is 1, but on very high density outputs this
     * can be a higher value (often 2).
     *
     * A higher value means that drawing is automatically scaled
     * up to a higher resolution, so any code doing drawing will
     * automatically look nicer. However, if you are supplying
     * pixel-based data the scale value can be used to determine
     * whether to use a pixel resource with higher resolution
     * data.
     *
     * The scale of a window may change during runtime, if this
     * happens a configure event will be sent to the toplevel
     * window.
     *
     * @returns The scale factor
     */
    ...
    get_screen(): Screen;
    /**
     * Returns the event mask for #window corresponding to the
     * device class specified by #source.
     *
     * @param source A {@link InputSource} to define the source
     *   class.
     * @returns Source event mask for #window
     */
    get_source_events(source: InputSource): EventMask;
    /**
     * Gets the bitwise OR of the currently active window state
     * flags, from the {@link WindowState} enumeration.
     *
     ...
     * toplevel window, as is a %GDK_WINDOW_CHILD window that has
     * a root window as parent.
     *
     * Note that you should use
     * {@link Gdk.Window.get_effective_toplevel} when you want to
     * get to a window’s toplevel as seen on screen, because
     * gdk_window_get_toplevel() will most likely not do what you
     ...
     * caller of the function. That is, after calling this
     * function, #window will no longer have an invalid/dirty
     * region; the update area is removed from #window and handed
     * to you. If a window has no update area,
     * {@link Gdk.Window.get_update_area} returns %NULL. You are
     * responsible for calling cairo_region_destroy() on the
     ...
     * widget that #window belongs to. See
     * {@link Gdk.Window.set_user_data}.
     *
     * @returns Return location for user data
     */
    get_user_data(): any | null;
    /**
     * Computes the region of the #window that is potentially
     * visible. This does not necessarily take into account if
     * the window is obscured by other windows, but no area
     * outside of this region is visible.
     *
     * @returns A #cairo_region_t. This must be freed with
     *   {@link Cairo.Region.destroy} when you are done.
     */
    ...
    /**
     * Gets the {@link Visual} describing the pixel format of
     * #window.
     *
    ...
    /**
     * Checks whether the window has a native window or not. Note
     * that you can use {@link Gdk.Window.ensure_native} if a
     * native window is needed.
     *
     * @returns %TRUE if the #window has a native window, %FALSE
     *   otherwise.
     */
    ...
    iconify(): void;
    /**
     * Like {@link Gdk.Window.shape_combine_region}, but the shape
     * applies only to event handling. Mouse events which happen
     * while the pointer position corresponds to an unset bit in
     * the mask will be passed on the window below #window.
     *
     * An input shape is typically used with RGBA windows. The
     * alpha channel of the window defines which pixels are
     * invisible and allows for nicely antialiased borders, and
     * the input shape controls where the window is “clickable”.
     *
     * On the X11 platform, this requires version 1.1 of the
     * shape extension.
     *
     * On the Win32 platform, this functionality is not present
     * and the function does nothing.
     *
     * @param shape_region Region of window to be non-transparent
     * @param offset_x X position of #shape_region in #window
     *   coordinates
     * @param offset_y Y position of #shape_region in #window
     *   coordinates
     */
    ...
    /**
     * Adds #region to the update area for #window. The update
     * area is the region that needs to be redrawn, or “dirty
     * region.” The call {@link Gdk.Window.process_updates} sends
     * one or more expose events to the window, which together
     * cover the entire update area. An application would
     * normally redraw the contents of #window in response to
     * those expose events.
     *
     * GDK will call gdk_window_process_all_updates() on your
     * behalf whenever your program returns to the main loop and
     * becomes idle, so normally there’s no need to do that
     * manually, you just need to invalidate regions that you
     * know should be redrawn.
     *
     * The #child_func parameter controls whether the region of
     * each child window that intersects #region will also be
     * invalidated. Only children for which #child_func returns
     * #TRUE will have the area invalidated.
     *
     * @param region A #cairo_region_t
     * @param child_func Function to use to decide if to recurse
    ...
    /**
     * A convenience wrapper around
     * {@link Gdk.Window.invalidate_region} which invalidates a
     * rectangular region. See gdk_window_invalidate_region() for
     * details.
     *
     * @param rect Rectangle to invalidate or %NULL to invalidate
     *   the whole window
     * @param invalidate_children Whether to also invalidate
     *   child windows
     */
     ...
     * know should be redrawn.
     *
     * The #invalidate_children parameter controls whether the
     * region of each child window that intersects #region will
     * also be invalidated. If %FALSE, then the update area for
     * child windows will remain unaffected. See
     * gdk_window_invalidate_maybe_recurse if you need fine
     * grained control over which children are invalidated.
     *
     * @param region A #cairo_region_t
     * @param invalidate_children %TRUE to also invalidate child
     *   windows
     */
    ...
    /**
     * Determines whether or not the window is an input only
     * window.
     *
     ...
     * mapped. (This is not necessarily "viewable" in the X
     * sense, since we only check as far as we have GDK window
     ...
     * {@link Gdk.Window.show} or gdk_window_show_unraised()).
     *
     * @returns %TRUE if the window is mapped
     */
    is_visible(): boolean;
    /**
     * Lowers #window to the bottom of the Z-order (stacking
     * order), so that other windows with the same parent window
     * appear above #window. This is true whether or not the
     * other windows are visible.
     *
     * If #window is a toplevel, the window manager may choose to
     * deny the request to move the window in the Z-order,
     * {@link Gdk.Window.lower} only requests the restack, does
     * not guarantee it.
     *
     * Note that gdk_window_show() raises the window again, so
     * don’t call this function before gdk_window_show(). (Try
     * gdk_window_show_unraised().)
     */
    lower(): void;
    /**
     * If you call this during a paint (e.g. between
     * {@link Gdk.Window.begin_paint_region} and
     * gdk_window_end_paint() then GDK will mark the current clip
     * region of the window as being drawn. This is required when
     * mixing GL rendering via gdk_cairo_draw_from_gl() and cairo
     * rendering, as otherwise GDK has no way of knowing when
     * something paints over the GL-drawn regions.
     *
     * This is typically called automatically by GTK+ and you
    ...
    /**
     * Maximizes the window. If the window was already maximized,
     * then this function does nothing.
     *
     * On X11, asks the window manager to maximize #window, if
     * the window manager supports this operation. Not all window
     * managers support this, and some deliberately ignore it or
     * don’t have a concept of “maximized”; so you can’t rely on
     * the maximization actually happening. But it will happen
     * with most standard window managers, and GDK makes a best
    ...
    maximize(): void;
    /**
     * Merges the input shape masks for any child windows into
     * the input shape mask for #window. i.e. the union of all
     * input masks for #window and its children will become the
     * new input mask for #window. See
     * {@link Gdk.Window.input_shape_combine_region}.
     *
     * This function is distinct from
     * gdk_window_set_child_input_shapes() because it includes
     * #window’s input shape mask in the set of shapes to be
     * merged.
     */
    merge_child_input_shapes(): void;
    /**
     * Merges the shape masks for any child windows into the
     * shape mask for #window. i.e. the union of all masks for
     * #window and its children will become the new mask for
     * #window. See {@link Gdk.Window.shape_combine_region}.
     *
     * This function is distinct from
     * gdk_window_set_child_shapes() because it includes
     ...
     * simultaneously, for a nicer visual effect.
     *
    ...
    /**
     * Move the part of #window indicated by #region by #dy
     * pixels in the Y direction and #dx pixels in the X
     * direction. The portions of #region that not covered by the
     * new position of #region are invalidated.
     *
    ...
    /**
     * Equivalent to calling {@link Gdk.Window.move} and
     * gdk_window_resize(), except that both operations are
     * performed at once, avoiding strange visual effects. (i.e.
     * the user may be able to see the window first move, then
     ...
     *
     * #rect is relative to the top-left corner of the window
     * that #window is transient for. #rect_anchor and
     * #window_anchor determine anchor points on #rect and
     * #window to pin together. #rect's anchor point can
     * optionally be offset by #rect_anchor_dx and
     * #rect_anchor_dy, which is equivalent to offsetting the
     * position of #window.
     *
     * #anchor_hints determines how #window will be moved if the
     * anchor points cause it to move off-screen. For example,
     * %GDK_ANCHOR_FLIP_X will replace %GDK_GRAVITY_NORTH_WEST
     * with %GDK_GRAVITY_NORTH_EAST and vice versa if #window
     * extends beyond the left or right edges of the monitor.
     *
     * Connect to the {@link Window.moved_to_rect} signal to find
     * out how it was actually positioned.
     *
     * @param rect The destination {@link Rectangle} to align
     *   #window with
     * @param rect_anchor The point on #rect to align with
     *   #window's anchor point
     * @param window_anchor The point on #window to align with
     *   #rect's anchor point
     * @param anchor_hints Positioning hints to use when limited
     *   on space
     * @param rect_anchor_dx Horizontal offset to shift #window,
     *   i.e. #rect's anchor point
     * @param rect_anchor_dy Vertical offset to shift #window,
     *   i.e. #rect's anchor point
     */
    move_to_rect(
      rect: Rectangle,
    ...
    /**
     * Like {@link Gdk.Window.get_children}, but does not copy the
     * list of children, so the list does not need to be freed.
     *
     * @returns A reference to the list of child windows in
     *   #window
     */
    peek_children(): GLib.List;
    /**
     * Sends one or more expose events to #window. The areas in
     * each expose event will cover the entire update area for
     * the window (see {@link Gdk.Window.invalidate_region} for
     * details). Normally GDK calls
     * gdk_window_process_all_updates() on your behalf, so
     * there’s no need to call this function unless you want to
     * force expose events to be delivered immediately and
     * synchronously (vs. the usual case, where GDK delivers them
     * in an idle handler). Occasionally this is useful to
     * produce nicer scrolling behavior, for example.
     *
     * @param update_children Whether to also process updates for
     *   child windows
     */
    process_updates(update_children: boolean): void;
    /**
     * Raises #window to the top of the Z-order (stacking order),
     * so that other windows with the same parent window appear
     * below #window. This is true whether or not the windows are
     * visible.
     *
     * If #window is a toplevel, the window manager may choose to
     * deny the request to move the window in the Z-order,
     * {@link Gdk.Window.raise} only requests the restack, does
     * not guarantee it.
     */
    raise(): void;
    ...
    /**
     * Reparents #window into the given #new_parent. The window
    ...
    /**
     * Changes the position of #window in the Z-order (stacking
     * order), so that it is above #sibling (if #above is %TRUE)
     * or below #sibling (if #above is %FALSE).
     *
     * If #sibling is %NULL, then this either raises (if #above
     * is %TRUE) or lowers the window.
     *
     * If #window is a toplevel, the window manager may choose to
     * deny the request to move the window in the Z-order,
     * {@link Gdk.Window.restack} only requests the restack, does
     * not guarantee it.
     *
     * @param sibling A {@link Window} that is a sibling of
     *   #window, or %NULL
     * @param above A boolean
     */
    restack(sibling: Window | null, above: boolean): void;
    /**
     * Scroll the contents of #window, both pixels and children,
     * by the given amount. #window itself does not move.
     * Portions of the window that the scroll operation brings in
     * from offscreen areas are invalidated. The invalidated
     * region may be bigger than what would strictly be
     * necessary.
     *
     * For X11, a minimum area will be invalidated if the window
     * has no subwindows, or if the edges of the window’s parent
     * do not extend beyond the edges of the window. In other
     * cases, a multi-step process is used to scroll the window
     * which may produce temporary visual artifacts and
     ...
     * environment that the window doesn’t want to receive input
     * focus.
     *
     * On X, it is the responsibility of the window manager to
     * interpret this hint. ICCCM-compliant window manager
     * usually respect it.
     *
     * @param accept_focus %TRUE if the window should receive
     *   input focus
     */
     ...
     *
     *   A background of %NULL means that the window won't have any
     *   background. On the X11 backend it's also possible to
     *   inherit the background from the parent window using
     *   {@link Gdk.x11_get_parent_relative_pattern}.
     *
     *   The windowing system will normally fill a window with its
     *   background when the window is obscured then exposed.
    ...
    /**
     * Sets the input shape mask of #window to the union of input
     * shape masks for all children of #window, ignoring the
     * input shape mask of #window itself. Contrast with
     * {@link Gdk.Window.merge_child_input_shapes} which includes
     * the input shape mask of #window in the masks to be
     * merged.
     */
    set_child_input_shapes(): void;
    /**
     * Sets the shape mask of #window to the union of shape masks
     * for all children of #window, ignoring the shape mask of
     * #window itself. Contrast with
     * {@link Gdk.Window.merge_child_shapes} which includes the
     ...
     *   only ever worked on X11.
     *
     *   Sets a {@link Window} as composited, or unsets it.
     *   Composited windows do not automatically have their
     *   contents drawn to the screen. Drawing is redirected to
     *   an offscreen buffer and an expose event is emitted on
     *   the parent of the composited window. It is the
     *   responsibility of the parent’s expose handler to
     *   manually merge the off-screen content onto the screen in
     *   whatever way it sees fit.
     *
     *   It only makes sense for child windows to be composited;
     *   see {@link Gdk.Window.set_opacity} if you need
     *   translucent toplevel windows.
     *
     ...
     *   only X11 with new enough Xcomposite and Xdamage
     *   extensions). You must call
     *   gdk_display_supports_composite() to check if setting a
     *   window as composited is supported before attempting to
     *   do so.
     ...
     *
     * Use {@link Gdk.Cursor.new_for_display} or
     * gdk_cursor_new_from_pixbuf() to create the cursor. To make
     * the cursor invisible, use %GDK_BLANK_CURSOR. Passing %NULL
     * for the #cursor argument to gdk_window_set_cursor() means
     * that #window will use the cursor of its parent window.
    ...
    /**
     * “Decorations” are the features the window manager adds to
     * a toplevel {@link Window}. This function sets the
     ...
     * included in the mask, the other bits indicate which
     * decorations should be turned off. If #GDK_DECOR_ALL is not
     * included, then the other bits indicate which decorations
     * should be turned on.
     *
     * Most window managers honor a decorations hint of 0 to
     * disable all decorations, but very few honor all possible
    ...
    /**
     * Sets a specific {@link Cursor} for a given device when it
     * gets inside #window. Use {@link Gdk.Cursor.new_for_display}
     * or gdk_cursor_new_from_pixbuf() to create the cursor. To
     * make the cursor invisible, use %GDK_BLANK_CURSOR. Passing
     * %NULL for the #cursor argument to gdk_window_set_cursor()
     * means that #window will use the cursor of its parent
    ...
    /**
     * Sets the event mask for a given device (Normally a
     * floating device, not attached to any visible pointer) to
     * #window. For example, an event mask including
     * #GDK_BUTTON_PRESS_MASK means the window should report
     * button press events. The event mask is the bitwise OR of
     * values from the {@link EventMask} enumeration.
     *
     * See the [input handling overview][event-masks] for
     * details.
     *
    ...
    /**
     * Determines whether or not extra unprocessed motion events
     * in the event queue can be discarded. If %TRUE only the
     * most recent event will be delivered.
     *
     * Some types of applications, e.g. paint programs, need to
     * see all motion events and will benefit from turning off
     * event compression.
     *
     * By default, event compression is enabled.
     *
     * @param event_compression %TRUE if motion events should be
     *   compressed
     */
    set_event_compression(event_compression: boolean): void;
    /**
     * The event mask for a window determines which events will
     * be reported for that window from all master input devices.
     * For example, an event mask including
     * #GDK_BUTTON_PRESS_MASK means the window should report
     * button press events. The event mask is the bitwise OR of
     * values from the {@link EventMask} enumeration.
     *
     ...
     * environment that the window doesn’t want to receive input
     * focus when it is mapped. focus_on_map should be turned off
     * for windows that aren’t triggered interactively (such as
     * popups from network activity).
     *
     * On X, it is the responsibility of the window manager to
     * interpret this hint. Window managers following the
     * freedesktop.org window manager extension specification
     * should respect it.
     *
     * @param focus_on_map %TRUE if the window should receive
     *   input focus when mapped
     */
    set_focus_on_map(focus_on_map: boolean): void;
    /**
     * Specifies whether the #window should span over all
     * monitors (in a multi-head setup) or only the current
     * monitor when in fullscreen mode.
     *
     * The #mode argument is from the {@link FullscreenMode}
     * enumeration. If #GDK_FULLSCREEN_ON_ALL_MONITORS is
     * specified, the fullscreen #window will span over all
     * monitors from the #GdkScreen.
     *
     * On X11, searches through the list of monitors from the
     * #GdkScreen the ones which delimit the 4 edges of the
     * entire #GdkScreen and will ask the window manager to span
     * the #window over these monitors.
     *
     * If the XINERAMA extension is not available or not usable,
     * this function has no effect.
     *
     * Not all window managers support this, so you can’t rely on
     * the fullscreen window to span over the multiple monitors
     * when #GDK_FULLSCREEN_ON_ALL_MONITORS is specified.
     *
     * @param mode Fullscreen mode
     */
    set_fullscreen_mode(mode: FullscreenMode): void;
    /**
     * Sets hints about the window management functions to make
     * available via buttons on the window frame.
     *
     * On the X backend, this function sets the traditional Motif
     * window manager hint for this purpose. However, few window
     * managers do anything reliable or interesting with this
     * hint. Many ignore it entirely.
     *
     * The #functions argument is the logical OR of values from
     * the {@link WMFunction} enumeration. If the bitmask includes
     * #GDK_FUNC_ALL, then the other bits indicate which
     * functions to disable; if it doesn’t include #GDK_FUNC_ALL,
    ...
    /**
     * Sets the geometry hints for #window. Hints flagged in
     * #geom_mask are set, hints not flagged in #geom_mask are
     * unset. To unset all hints, use a #geom_mask of 0 and a
     * #geometry of %NULL.
     *
     * This function provides hints to the windowing system about
     * acceptable sizes for a toplevel window. The purpose of
     * this is to constrain user resizing, but the windowing
     * system will typically (but is not required to) also
     * constrain the current size of the window to the provided
     * values and constrain programatic resizing via
     * {@link Gdk.Window.resize} or gdk_window_move_resize().
     *
     * Note that on X11, this effect has no effect on windows of
     * type %GDK_WINDOW_TEMP or windows where override redirect
     ...
     * @param geom_mask Bitmask indicating fields of #geometry to
     *   pay attention to
     */
    set_geometry_hints(geometry: Geometry, geom_mask: WindowHints): void;
    /**
     * Sets the group leader window for #window. By default, GDK
     * sets the group leader for all toplevel windows to a global
     * window implicitly created by GDK. With this function you
     * can override this default.
     *
     * The group leader window allows the window manager to
     * distinguish all windows that belong to a single
     * application. It may for example allow users to
     * minimize/unminimize all windows belonging to an
     * application at once. You should only set a non-default
     * group window if your application pretends to be multiple
     * applications.
     *
     * @param leader Group leader window, or %NULL to restore the
     *   default group leader window
     */
    set_group(leader: Window | null): void;
    /**
     * Sets a list of icons for the window. One of these will be
     * used to represent the window when it has been iconified.
     * The icon is usually shown in an icon box or some sort of
     * task bar. Which icon size is shown depends on the window
     * manager. The window manager can scale the icon but setting
     * several size icons can give better image quality since the
     * window manager may only need to scale the icon by a small
    ...
    /**
     * Windows may have a name used while minimized, distinct
     * from the name they display in their titlebar. Most of the
     * time this is a bad idea from a user interface standpoint.
     * But you can set such a name with this function, if you
     * like.
     *
     * After calling this with a non-%NULL #name, calls to
     * {@link Gdk.Window.set_title} will not update the icon
     * title.
     *
     * Using %NULL for #name unsets the icon title; further calls
     * to gdk_window_set_title() will again update the icon title
     * as well.
     *
     * Note that some platforms don't support window icons.
     *
    ...
    /**
     * Registers an invalidate handler for a specific window.
     * This will get called whenever a region in the window or
     * its children is invalidated.
     *
     * This can be used to record the invalidated region, which
     * is useful if you are keeping an offscreen copy of some
     * region and want to keep it up to date. You can also modify
     * the invalidated region in case you’re doing some effect
     * where e.g. a child widget appears in multiple places.
     *
     * @param handler A {@link WindowInvalidateHandlerFunc}
     *   callback function
     */
     ...
     * nothing.
     *
     * On X11, asks the window manager to keep #window above, if
     * the window manager supports this operation. Not all window
     * managers support this, and some deliberately ignore it or
     * don’t have a concept of “keep above”; so you can’t rely on
     * the window being kept above. But it will happen with most
     * standard window managers, and GDK makes a best effort to
     * get it to happen.
     *
     ...
     * nothing.
     *
     * On X11, asks the window manager to keep #window below, if
     * the window manager supports this operation. Not all window
     * managers support this, and some deliberately ignore it or
     * don’t have a concept of “keep below”; so you can’t rely on
     * the window being kept below. But it will happen with most
     * standard window managers, and GDK makes a best effort to
     * get it to happen.
     *
    ...
    /**
     * The application can use this hint to tell the window
     * manager that a certain window has modal behaviour. The
     * window manager can use this information to handle modal
     * windows in a special way.
     *
     * You should only use this on windows for which you have
     * previously called {@link Gdk.Window.set_transient_for}
     *
     * @param modal %TRUE if the window is modal, %FALSE
     *   otherwise.
     */
    set_modal_hint(modal: boolean): void;
    /**
     * Set #window to render as partially transparent, with
     * opacity 0 being fully transparent and 1 fully opaque.
     * (Values of the opacity parameter are clamped to the [0,1]
     * range.)
     *
     * For toplevel windows this depends on support from the
     * windowing system that may not always be there. For
     * instance, On X11, this works only on X screens with a
     * compositing manager running. On Wayland, there is no
     * per-window opacity value that the compositor would apply.
     ...
     * like to not draw obscured regions of windows, or turn off
     * blending during for these regions. With RGB windows with
     * no transparency, this is just the shape of the window, but
     * with ARGB32 windows, the compositor does not know what
     * regions of the window are transparent or not.
     *
     * This function only works for toplevel windows.
     *
     * GTK+ will update this property automatically if the
     * #window background is opaque, as we know where the opaque
     * regions are. If your window background is not opaque,
     * please update this property in your
     * #GtkWidget::style-updated handler.
     *
     * @param region A region, or %NULL
     */
    set_opaque_region(region: cairo.Region | null): void;
    /**
     * An override redirect window is not under the control of
     * the window manager. This means it won’t have a titlebar,
     * won’t be minimizable, etc. - it will be entirely under the
     * control of the application. The window manager can’t see
     * the override redirect window at all.
     *
     * Override redirect should only be used for short-lived
     * temporary windows, such as popup menus. #GtkMenu uses an
     * override redirect window in its implementation, for
     * example.
     *
     * @param override_redirect %TRUE if window should be
     *   override redirect
     */
    set_override_redirect(override_redirect: boolean): void;
    /**
     * Sets whether input to the window is passed through to the
     * window below.
     *
     * The default value of this is %FALSE, which means that
     * pointer events that happen inside the window are send
     * first to the window, but if the event is not selected by
     * the event mask then the event is sent to the parent
     * window, and so on up the hierarchy.
     *
     * If #pass_through is %TRUE then such pointer events happen
     * as if the window wasn't there at all, and thus will be
     * sent first to any windows below #window. This is useful if
     * the window is used in a transparent fashion. In the
     * terminology of the web this would be called
     * "pointer-events: none".
     *
     * Note that a window with #pass_through %TRUE can still have
     * a subwindow without pass through, so you can get events on
     * a subset of a window. And in that cases you would get the
     * in-between related events such as the pointer enter/leave
     * events on its way to the destination window.
     *
     ...
     * to distinguish it from other kinds of window in the same
     * application. When an application is restarted after being
     * saved in a previous session, all windows with the same
     * title and role are treated as interchangeable. So if you
     * have two windows with the same title that should be
     * distinguished for session management purposes, you should
     * set the role on those windows. It doesn’t matter what
     * string you use for the role, as long as you have a
     * different role for each non-interchangeable kind of
     * window.
     *
    ...
    /**
     * Newer GTK+ windows using client-side decorations use extra
     * geometry around their frames for effects like shadows and
     * invisible borders. Window managers that want to maximize
     * windows or snap to edges need to know where the extents of
     * the actual frame lie, so that users don’t feel like
     * windows are snapping against random invisible edges.
     *
     * Note that this property is automatically updated by GTK+,
     * so this function should only be used by applications which
     * do not use GTK+ to create toplevel windows.
     *
    ...
    /**
     * Toggles whether a window should appear in a pager
     * (workspace switcher, or other desktop utility program that
     * displays a small thumbnail representation of the windows
     * on the desktop). If a window’s semantic type as specified
     * with {@link Gdk.Window.set_type_hint} already fully
     * describes the window, this function should not be called
     * in addition, instead you should allow the window to be
     * treated according to standard policy for its semantic
     * type.
     *
    ...
    /**
     * Toggles whether a window should appear in a task list or
     * window list. If a window’s semantic type as specified with
     * {@link Gdk.Window.set_type_hint} already fully describes
     * the window, this function should not be called in
     * addition, instead you should allow the window to be
     * treated according to standard policy for its semantic
     * type.
     *
    ...
    /**
     * Sets the event mask for any floating device (i.e. not
     * attached to any visible pointer) that has the source
     * defined as #source. This event mask will be applied both
     * to currently existing, newly added devices after this
    ...
    /**
     * @deprecated Static gravities haven't worked on anything
     *   but X11 for a long time.
     *
     *   Used to set the bit gravity of the given window to static,
     *   and flag it so all children get static subwindow
     *   gravity. This is used if you are implementing scary
     *   features that involve deep knowledge of the windowing
     ...
     *
     * Multidevice aware windows will need to handle properly
     * multiple, per device enter/leave events, device grabs and
     * grab ownerships.
     *
     * @param support_multidevice %TRUE to enable multidevice
     *   support in #window.
     */
    set_support_multidevice(support_multidevice: boolean): void;
    /**
     * Sets the title of a toplevel window, to be displayed in
     * the titlebar. If you haven’t explicitly set the icon name
     * for the window (using {@link Gdk.Window.set_icon_name}),
     * the icon name will be set to #title as well. #title must
     * be in UTF-8 encoding (as with all user-readable strings in
    ...
    /**
     * The application can use this call to provide a hint to the
     * window manager about the functionality of a window. The
     * window manager can use this information when determining
     * the decoration and behaviour of the window.
     *
    ...
    /**
     * Toggles whether a window needs the user's urgent
     * attention.
     *
     ...
     * {@link GObject.set_data}. However, for historical reasons
     * GTK+ stores the #GtkWidget that owns a {@link Window} as
     * user data on the #GdkWindow. So, custom widget
     * implementations should use this function for that. If GTK+
     * receives an event for a #GdkWindow, and the user data for
     * the window is non-%NULL, GTK+ will assume the user data is
     * a #GtkWidget, and forward the event to that widget.
     */
    set_user_data(): void;
    /**
     ...
     *
     * If #shape_region is %NULL, the shape will be unset, so the
     * whole window will be opaque again. #offset_x and #offset_y
     * are ignored if #shape_region is %NULL.
     *
     * On the X11 platform, this uses an X server extension which
     * is widely available on most common platforms, but not
     * available on very old X servers, and occasionally the
     * implementation will be buggy. On servers without the shape
    ...
    /**
     * Like {@link Gdk.Window.show_unraised}, but also raises the
     * window to the top of the window stack (moves the window to
     * the front of the Z-order).
     *
     * This function maps a window so it’s visible onscreen. Its
     * opposite is gdk_window_hide().
     *
     * When implementing a #GtkWidget, you should call this
     * function on the widget's {@link Window} as part of the
     * “map” method.
     */
    show(): void;
    /**
     * Shows a {@link Window} onscreen, but does not modify its
     * stacking order. In contrast, {@link Gdk.Window.show} will
     * raise the window to the top of the window stack.
     *
     * On the X11 platform, in Xlib terms, this function calls
     * XMapWindow() (it also updates some internal GDK state,
     * which means that you can’t really use XMapWindow()
     * directly on a GDK window).
     */
    show_unraised(): void;
    /**
     * Asks the windowing system to show the window menu. The
     * window menu is the menu shown when right-clicking the
     * titlebar on traditional windows managed by the window
     * manager. This is useful for windows using client-side
     * decorations, activating it with a right-click on the
     * window decorations.
     *
     * @param event A {@link Event} to show the menu for
     * @returns %TRUE if the window menu was shown and %FALSE
     *   otherwise.
     */
     ...
     * managers. However, GDK will do the best it can to convince
     * the window manager to stick the window. For window
     * managers that don’t support this operation, there’s
     * nothing you can do to force it to happen.
     */
    stick(): void;
    /**
     * @deprecated This symbol was never meant to be used outside
    ...
    /**
     * Thaws a window frozen with
     * {@link Gdk.Window.freeze_updates}.
     */
    thaw_updates(): void;
    /**
     * Moves the window out of fullscreen mode. If the window was
     ...
     * operation. Not all window managers support this, and some
     * deliberately ignore it or don’t have a concept of
     * “fullscreen”; so you can’t rely on the
     * unfullscreenification actually happening. But it will
     * happen with most standard window managers, and GDK makes a
     * best effort to get it to happen.
     */
    unfullscreen(): void;
    /**
     * Unmaximizes the window. If the window wasn’t maximized,
     * then this function does nothing.
     *
     * On X11, asks the window manager to unmaximize #window, if
     * the window manager supports this operation. Not all window
     * managers support this, and some deliberately ignore it or
     * don’t have a concept of “maximized”; so you can’t rely on
     * the unmaximization actually happening. But it will happen
     * with most standard window managers, and GDK makes a best
     * effort to get it to happen.
     *
    ...
    unmaximize(): void;
    /**
     * Reverse operation for {@link Gdk.Window.stick}; see
     * gdk_window_stick(), and gtk_window_unstick().
     */
    unstick(): void;
    /**
     * Withdraws a window (unmaps it and asks the window manager
     * to forget about it). This function is not really useful as
     * {@link Gdk.Window.hide} automatically withdraws toplevel
     * windows before hiding them.
     */
    withdraw(): void;
    /**
     * The ::create-surface signal is emitted when an offscreen
     * window needs its surface (re)created, which happens either
     * when the window is first drawn to, or when the window is
     * being resized. The first signal handler that returns a
     * non-%NULL surface will stop any further signal emission,
     * and its surface will be used.
     *
     * Note that it is not possible to access the window's
     * previous surface from within any callback of this signal.
     * Calling {@link Gdk.offscreen.window_get_surface} will lead
     * to a crash.
     *
     * @param signal
     * @param callback Callback function
     ...
     *   - Returns the newly created #cairo_surface_t for the
     *       offscreen window
     *
     * @returns Callback ID
     */
    connect(
      ...
      callback: (owner: this, width: number, height: number) => cairo.Surface
    ): number;
    /**
     * The ::from-embedder signal is emitted to translate
     * coordinates in the embedder of an offscreen window to the
     * offscreen window.
     *
     ...
     *   - Returns return location for the x coordinate in the
     *       offscreen window
     *
     *   Return location for the y coordinate in the offscreen
     *   window
     * @returns Callback ID
     */
    connect(
      ...
      callback: (
        ...
        embedder_x: number,
        embedder_y: number
      ) => [offscreen_x: number, offscreen_y: number]
    ): number;
    /**
     * Emitted when the position of #window is finalized after
     * being moved to a destination rectangle.
     *
     * #window might be flipped over the destination rectangle in
     * order to keep it on-screen, in which case #flipped_x and
     * #flipped_y will be set to %TRUE accordingly.
     *
     * #flipped_rect is the ideal position of #window after any
     * possible flipping, but before any possible sliding.
     * #final_rect is #flipped_rect, but possibly translated in
     * the case that flipping is still ineffective in keeping
     * #window on-screen.
     *
     * @param signal
     * @param callback Callback function
    ...
    /**
     * The ::pick-embedded-child signal is emitted to find an
     * embedded child at the given position.
     *
     * @param signal
     * @param callback Callback function
     *
     ...
     *   - Returns the {@link Window} of the embedded child at #x, #y,
     *       or %NULL
     *
     * @returns Callback ID
     */
    connect(
      ...
      callback: (owner: this, x: number, y: number) => Window | null
    ): number;
    /**
     * The ::to-embedder signal is emitted to translate
     * coordinates in an offscreen window to its embedder.
     *
     ...
     *       embedder window
     *
     *   Return location for the y coordinate in the embedder
     *   window
     * @returns Callback ID
     */
    connect(
      ...
      callback: (
        ...
        offscreen_x: number,
        offscreen_y: number
      ) => [embedder_x: number, embedder_y: number]
    ): number;

    connect(
      signal: "notify::cursor",
      callback: (owner: this, ...args: any) => void
    ): number;
  }
  ...
  class Window {
     ...
     * #attributes. See #GdkWindowAttr and
     * #GdkWindowAttributesType for more details. Note: to use
     * this on displays other than the default display, #parent
     * must be specified.
     *
     * @param parent A {@link Window}, or %NULL to create the
     *   window as a child of the default root window for the
     *   default display.
     * @param attributes Attributes of the new window
     * @param attributes_mask Mask indicating which fields in
     ...
     *
     *   Obtains the window underneath the mouse pointer, returning
     *   the location of that window in #win_x, #win_y. Returns
     *   %NULL if the window under the mouse pointer is not known
     *   to GDK (if the window belongs to another application and
     *   a {@link Window} hasn’t been created for it with
     *   {@link Gdk.Window.foreign_new})
     *
    ...
    /**
     * Constrains a desired width and height according to a set
     * of geometry hints (such as minimum and maximum size).
     *
     * @param geometry A {@link Geometry} structure
     * @param flags A mask indicating what portions of #geometry
     *   are set
    ...
    /**
     * Calls {@link Gdk.Window.process_updates} for all windows
     * (see {@link Window}) in the application.
     */
    public static process_all_updates(): void;
    /**
     * With update debugging enabled, calls to
     * {@link Gdk.Window.invalidate_region} clear the invalidated
     * region of the screen to a noticeable color, and GDK pauses
     * for a short time before sending exposes to windows during
     * gdk_window_process_updates(). The net effect is that you
     * can see the invalid region for each window and watch
     * redraws as they occur. This allows you to diagnose
     * inefficiencies in your application.
     *
     * In essence, because the GDK rendering model prevents all
     * flicker, if you are redrawing the same region 400 times
     * you may never notice, aside from noticing a speed problem.
     * Enabling update debugging causes GTK to flicker slowly and
     * noticeably, so you can see exactly what’s being redrawn
     * when, in what order.
     *
     * The --gtk-debug=updates command line option passed to GTK+
     * programs enables this debug option at application startup
     * time. That's usually more useful than calling
     * gdk_window_set_debug_updates() yourself, though you might
     * want to use this function to enable updates sometime after
  ...
  /**
   * An opaque type representing a string as an index into a
   * table of strings on the X server.
   */
  interface Atom {}
  class Atom {
  ...
  /**
   * A {@link Color} is used to describe a color, similar to the
  ...
  class Color {
    ...
    /**
     * For allocated colors, the pixel value used to draw this
     * color on the screen. Not used anymore.
     */
    public pixel: number;
    /**
     * The red component of the color. This is a value between 0
     * and 65535, with 65535 indicating full intensity
     */
    public red: number;
     ...
     * @deprecated Use {@link RGBA}
     *
     *   A hash function suitable for using for a hash table that
     *   stores {@link Colors}.
     ...
     *   are hex digits representing the red, green and blue
   ...
   * {@link EventAny} to access these fields.
   */
  interface EventAny {}
  class EventAny {
   ...
   *
   * Double and triple-clicks result in a sequence of events
   * being received. For double-clicks the order of events will
   * be:
   *
   * - %GDK_BUTTON_PRESS
   * - %GDK_BUTTON_RELEASE
   ...
   * occur within 1/4 of a second of the first. For a triple
   * click to occur, the third button press must also occur
   * within 1/2 second of the first button press.
   */
  ...
  class EventCrossing {
    ...
    /**
     * The type of the event (%GDK_ENTER_NOTIFY or
     * %GDK_LEAVE_NOTIFY).
     */
    public type: EventType;
    /** The window which received the event. */
     ...
     * %GDK_CROSSING_GTK_GRAB, %GDK_CROSSING_GTK_UNGRAB or
     * %GDK_CROSSING_STATE_CHANGED). %GDK_CROSSING_GTK_GRAB,
     * %GDK_CROSSING_GTK_UNGRAB, and %GDK_CROSSING_STATE_CHANGED
     * were added in 2.14 and are always synthesized, never
     * native.
     */
    public mode: CrossingMode;
    /**
     * The kind of crossing that happened (%GDK_NOTIFY_INFERIOR,
     * %GDK_NOTIFY_ANCESTOR, %GDK_NOTIFY_VIRTUAL,
  ...
  class EventDND {
    ...
    /**
     * The x coordinate of the pointer relative to the root of
     * the screen, only set for %GDK_DRAG_MOTION and
     * %GDK_DROP_START.
     */
    public x_root: number;
    /**
  ...
  /**
   * Generated when all or part of a window becomes visible and
   * needs to be redrawn.
   */
  ...
  class EventExpose {
     ...
     * handling all contiguous %GDK_EXPOSE events in one go,
  ...
  class EventFocus {
    ...
    public send_event: number;
    /**
     * %TRUE if the window has gained the keyboard focus, %FALSE
   ...
   * this happens when the grab window becomes unviewable (i.e.
   * it or one of its ancestors is unmapped), or if the same
   * application grabs the pointer or keyboard again. Note that
   * implicit grabs (which are initiated by button presses) can
   * also cause {@link EventGrabBroken} events.
   */
  ...
  class EventGrabBroken {
    ...
    public type: EventType;
    /**
     * The window which received the event, i.e. the window that
     ...
     * application, #grab_window contains the new grab window.
  ...
  /**
   * Generated when the owner of a selection changes. On X11,
   * this information is only available if the X server supports
  ...
  /**
   * Generated during %GDK_SOURCE_TABLET_PAD interaction with
   * tactile sensors.
   */
  ...
  class EventPadAxis {
     ...
     * %GDK_SOURCE_TABLET_PAD device may have one or more groups
     * containing a set of buttons/rings/strips each.
     */
    public group: number;
    /**
     * Number of strip/ring that was interacted. This number is
     * 0-indexed.
     */
    public index: number;
    /**
     * The current mode of #group. Different groups in a
     * %GDK_SOURCE_TABLET_PAD device may have different current
     * modes.
     */
    public mode: number;
  ...
  /**
   * Generated during %GDK_SOURCE_TABLET_PAD button presses and
   * releases.
   */
  ...
  class EventPadButton {
    ...
    /**
     * The type of the event (%GDK_PAD_BUTTON_PRESS or
    ...
    public time: number;
    /**
     * The pad group the button belongs to. A
     * %GDK_SOURCE_TABLET_PAD device may have one or more groups
     * containing a set of buttons/rings/strips each.
     */
    public group: number;
  ...
  /**
   * Generated during %GDK_SOURCE_TABLET_PAD mode switches in a
   * group.
   */
  ...
  class EventPadGroupMode {
    ...
    public group: number;
    /**
     * The new mode of #group. Different groups in a
     * %GDK_SOURCE_TABLET_PAD device may have different current
     * modes.
     */
    public mode: number;
  ...
  class EventProperty {
    ...
    public time: number;
    /**
     * Whether the property was changed (%GDK_PROPERTY_NEW_VALUE)
   ...
   * standard X that allows you to use nonstandard devices such
   * as graphics tablets. A proximity event indicates that the
   * stylus has moved in or out of contact with the tablet, or
   * perhaps that the user’s finger has moved in or out of
   * contact with a touch screen.
   *
   * This event type will be used pretty rarely. It only is
   * important for XInput aware programs that are drawing their
   * own cursor.
   */
  ...
  class EventProximity {
    ...
    /**
     * The type of the event (%GDK_PROXIMITY_IN or
     * %GDK_PROXIMITY_OUT).
     */
    public type: EventType;
    /** The window which received the event. */
  ...
  /**
   * Generated from button presses for the buttons 4 to 7. Wheel
   * mice are usually configured to generate button press events
   * for buttons 4 and 5 when the wheel is turned.
   *
   * Some GDK backends can also generate “smooth” scroll events,
   * which can be recognized by the %GDK_SCROLL_SMOOTH scroll
   * direction. For these, the scroll deltas can be obtained with
  ...
  /**
   * Generated when a selection is requested or ownership of a
  ...
  class EventSelection {
    ...
    public target: Atom;
    /**
     * The property in which to place the result of the
     * conversion.
     */
   ...
   * %GDK_TOUCH_BEGIN, %GDK_TOUCH_UPDATE, %GDK_TOUCH_END or
   * %GDK_TOUCH_CANCEL.
   *
   * Touch events are grouped into sequences by means of the
   * #sequence field, which can also be obtained with
   * {@link Gdk.event.get_event_sequence}. Each sequence begins
   * with a %GDK_TOUCH_BEGIN event, followed by any number of
   * %GDK_TOUCH_UPDATE events, and ends with a %GDK_TOUCH_END (or
   * %GDK_TOUCH_CANCEL) event. With multitouch devices, there may
  ...
  class EventTouch {
    ...
    /**
     * The type of the event (%GDK_TOUCH_BEGIN,
    ...
    /**
     * Whether the event should be used for emulating pointer
     * event
     */
  ...
  class EventTouchpadPinch {
    ...
    public dy: number;
    /**
     * The angle change in radians, negative angles denote
     * counter-clockwise movements
     */
    public angle_delta: number;
    /**
     * The current scale, relative to that at the time of the
  ...
  class EventWindowState {
    ...
    /**
     * The new window state, a combination of {@link WindowState}
     * bits.
     */
  ...
  /**
   * A {@link FrameTimings} object holds timing information for a
   * single frame of the application’s displays. To retrieve
   * #GdkFrameTimings objects, use
   * {@link Gdk.FrameClock.get_timings} or
   * gdk_frame_clock_get_current_timings(). The information in
   * #GdkFrameTimings is useful for precise synchronization of
   * video with the event or audio streams, and for measuring
   * quality metrics for the application’s display, such as
   * latency and jitter.
   */
  ...
  class FrameTimings {
    ...
    /**
     * The timing information in a {@link FrameTimings} is filled
     * in incrementally as the frame as drawn and passed off to
     * the window system for processing and display to the user.
     * The accessor functions for #GdkFrameTimings can return 0
     * to indicate an unavailable value for two reasons: either
     * because the information is not yet available, or because
     * it isn't available at all. Once
     * {@link Gdk.FrameTimings.get_complete} returns %TRUE for a
     * frame, you can be certain that no further values will
     * become available and be stored in the #GdkFrameTimings.
     *
    ...
    /**
     * Gets the frame counter value of the {@link FrameClock} when
     * this this frame was drawn.
     *
     * @returns The frame counter value for this frame
     */
    public get_frame_counter(): number;
    /**
     * Returns the frame time for the frame. This is the time
     * value that is typically used to time animations for the
     * frame. See {@link Gdk.FrameClock.get_frame_time}.
     *
     * @returns The frame time for the frame, in the timescale of
     *   {@link G.get_monotonic_time}
     */
    public get_frame_time(): number;
    /**
     * Gets the predicted time at which this frame will be
     * displayed. Although no predicted time may be available, if
     * one is available, it will be available while the frame is
     * being generated, in contrast to
     * {@link Gdk.FrameTimings.get_presentation_time}, which is
     * only available after the frame has been presented. In
     * general, if you are simply animating, you should use
     * gdk_frame_clock_get_frame_time() rather than this
     * function, but this function is useful for applications
     * that want exact control over latency. For example, a movie
     * player may want this information for Audio/Video
     * synchronization.
     *
     * @returns The predicted time at which the frame will be
     *   presented, in the timescale of
     *   {@link G.get_monotonic_time}, or 0 if no predicted
     *   presentation time is available.
     */
    public get_predicted_presentation_time(): number;
    /**
     * Reurns the presentation time. This is the time at which
     * the frame became visible to the user.
     *
     * @returns The time the frame was displayed to the user, in
     *   the timescale of {@link G.get_monotonic_time}, or 0 if no
     *   presentation time is available. See
     *   gdk_frame_timings_get_complete()
     */
    public get_presentation_time(): number;
    /**
     * Gets the natural interval between presentation times for
     * the display that this frame was displayed on. Frame
     * presentation usually happens during the “vertical blanking
     * interval”.
     *
     * @returns The refresh interval of the display, in
     *   microseconds, or 0 if the refresh interval is not
     *   available. See {@link Gdk.FrameTimings.get_complete}.
     */
    ...
    /**
     * Decreases the reference count of #timings. If #timings is
     * no longer referenced, it will be freed.
     */
    public unref(): void;
  }
   ...
   * fully valid already and simply passes them to the window
   * manager; in contrast, gtk_window_set_geometry_hints()
   * performs some interpretation. For example, #GtkWindow will
   * apply the hints to the geometry widget instead of the
   * toplevel window, if you set a geometry widget. Also, the
   * #min_width/#min_height/#max_width/#max_height fields may be
   * set to -1, and #GtkWindow will substitute the size request
   * of the window or geometry widget. If the minimum size hint
   * is not provided, #GtkWindow will use its requisition as the
   * minimum size. If the minimum size is provided and a geometry
   * widget is set, #GtkWindow will take the minimum size as the
   * minimum size of the geometry widget rather than the entire
   * window. The base size is treated similarly.
   *
   * The canonical use-case for gtk_window_set_geometry_hints()
   * is to get a terminal widget to resize properly. Here, the
   * terminal text area should be the geometry widget; #GtkWindow
   * will then automatically set the base size to the size of
   * other widgets in the terminal window, such as the menubar
   * and scrollbar. Then, the #width_inc and #height_inc fields
   ...
   * GTK_WIDGET (terminal), &hints, GDK_HINT_RESIZE_INC |
   * GDK_HINT_MIN_SIZE | GDK_HINT_BASE_SIZE); ]|
   *
   * The other useful fields are the #min_aspect and #max_aspect
   * fields; these contain a width/height ratio as a floating
   * point number. If a geometry widget is set, the aspect
   * applies to the geometry widget rather than the entire
   * window. The most common use of these hints is probably to
   * set #min_aspect and #max_aspect to the same value, thus
   * forcing the window to keep a constant aspect ratio.
   */
  interface Geometry {}
  class Geometry {
    ...
    /**
     * Allowed window widths are #base_height + #height_inc * N
     * where N is any integer (-1 allowed with #GtkWindow)
     */
  ...
  /**
   * A {@link KeymapKey} is a hardware key that can be mapped to a
   * keyval.
   */
  interface KeymapKey {}
  class KeymapKey {
    ...
    /**
     * Indicates movement in a horizontal direction. Usually
     * groups are used for two different languages. In group 0, a
     * key might have two English characters, and in group 1 it
     * might have two Hebrew characters. The Hebrew characters
     * will be printed on the key next to the English
     * characters.
     */
    public group: number;
    /**
     * Indicates which symbol on the key will be used, in a
     * vertical direction. So on a standard US keyboard, the key
     * with the number “1” on it also has the exclamation point
     * ("!") character on it. The level indicates whether to use
     * the “1” or the “!” symbol. The letter keys are considered
     * to have a lowercase letter at level 0, and an uppercase
     * letter at level 1, though only the uppercase letter is
     * printed.
     */
    public level: number;
  }
   ...
   * color, in a way that is compatible with cairo’s notion of
   * color.
   */
  interface RGBA {}
  class RGBA {
    ...
    public red: number;
    /**
     * The intensity of the green channel from 0.0 to 1.0
     * inclusive
     */
    public green: number;
    /**
     * The intensity of the blue channel from 0.0 to 1.0
     * inclusive
     */
    public blue: number;
    /**
     ...
     *
     * @returns A newly allocated {@link RGBA}, with the same
     *   contents as #rgba
     */
    public copy(): RGBA;
    /**
     ...
     * #red, #green, #blue and #alpha fields of the #rgba
     * {@link RGBA}.
     *
     ...
     * - A RGB color in the form “rgb(r,g,b)” (In this case the
     *   color will have full opacity)
     * - A RGBA color in the form “rgba(r,g,b,a)”
     *
     * Where “r”, “g”, “b” and “a” are respectively the red,
     * green, blue and alpha color values. In the last two cases,
     * “r”, “g”, and “b” are either integers in the range 0 to
     * 255 or percentage values in the range 0% to 100%, and a is
     ...
     * respectively. “r”, “g”, and “b” are represented as
     * integers in the range 0 to 255, and “a” is represented as
     * a floating point value in the range 0 to 1.
     *
     * These string forms are string forms that are supported by
     * the CSS3 colors module, and can be parsed by
     * {@link Gdk.RGBA.parse}.
     *
     * Note that this string representation may lose some
     * precision, since “r”, “g” and “b” are represented as 8-bit
     * integers. If this is a concern, you should use a different
     * representation.
     *
     * @returns A newly allocated text string
     */
  ...
  /**
   * Defines the position and size of a rectangle. It is
   * identical to #cairo_rectangle_int_t.
   */
  interface Rectangle {}
  class Rectangle {
    ...
    /**
     * Calculates the intersection of two rectangles. It is
     * allowed for #dest to be the same as either #src1 or #src2.
     * If the rectangles do not intersect, #dest’s width and
     * height is set to 0 and its x and y values are undefined.
     * If you are only interested in whether the rectangles
     * intersect, but not in the intersecting area itself, pass
     * %NULL for #dest.
     *
    ...
    /**
     * Calculates the union of two rectangles. The union of
     * rectangles #src1 and #src2 is the smallest rectangle which
     * includes both #src1 and #src2 within it. It is allowed for
     * #dest to be the same as either #src1 or #src2.
     *
     * Note that this function does not ignore 'empty' rectangles
     * (ie. with zero width or height).
     *
     * @param src2 A {@link Rectangle}
  ...
  /**
   * A {@link TimeCoord} stores a single event in a motion
   * history.
   */
  interface TimeCoord {}
  class TimeCoord {
  ...
  class WindowAttr {
    ...
    public height: number;
    /**
     * #GDK_INPUT_OUTPUT (normal window) or #GDK_INPUT_ONLY
  ...
  interface IDevicePad {
     ...
     * @param feature_idx The index of the feature to get the
     *   group from
     ...
     *
     * @param group_idx Group to get the number of available
     *   modes from
     ...
     * @returns The amount of elements of type #feature that this
     *   pad has.
     */
    get_n_features(feature: DevicePadFeature): number;
    /**
     * Returns the number of groups this pad device has. Pads
     * have at least one group. A pad group is a subcollection of
     * buttons/strip/rings that is affected collectively by a
     * same current mode.
     *
     * @returns The number of button/ring/strip groups in the
     *   pad.
     */
    get_n_groups(): number;
  }
  ...
  /**
   * {@link DevicePad} is an interface implemented by devices of
   * type %GDK_SOURCE_TABLET_PAD, it allows querying the features
   * provided by the pad device.
   *
   * Tablet pads may contain one or more groups, each containing
   * a subset of the buttons/rings/strips available.
   * {@link Gdk.DevicePad.get_n_groups} can be used to obtain the
   * number of groups, gdk_device_pad_get_n_features() and
   * gdk_device_pad_get_feature_group() can be combined to find
   * out the number of buttons/rings/strips the device has, and
   * how are they grouped.
   *
   * Each of those groups have different modes, which may be used
   * to map each individual pad feature to multiple actions. Only
   * one mode is effective (current) for each given group,
   * different groups may have different current modes. The
   * number of available modes in a group can be found out
   * through gdk_device_pad_get_group_n_modes(), and the current
   * mode for a given group will be notified through the
   * #GdkEventPadGroupMode event.
   */
   ...
   * (valuator) maps onto the predefined valuator types that GTK+
   * understands.
   *
   * Note that the X and Y axes are not really needed; pointer
   * devices report their location via the x/y members of events
   * regardless. Whether X and Y are present as axes depends on
   * the GDK backend.
   */
  enum AxisUse {
    /** The axis is ignored. */
    ...
    DISTANCE = 7,
    /**
     * The axis is used for pen rotation information. (Since:
     * 3.22)
     */
    ROTATION = 8,
  ...
  /**
   * A set of values describing the possible byte-orders for
   * storing pixel values in memory.
   */
  enum ByteOrder {
    /**
     * The values are stored with the least-significant byte
     * first. For instance, the 32-bit value 0xffeecc would be
     * stored in memory as 0xcc, 0xee, 0xff, 0x00.
     */
    LSB_FIRST = 0,
    /**
     * The values are stored with the most-significant byte
     * first. For instance, the 32-bit value 0xffeecc would be
     * stored in memory as 0x00, 0xff, 0xee, 0xcc.
     */
    MSB_FIRST = 1,
  }
  ...
  enum CrossingMode {
    ...
    STATE_CHANGED = 5,
    /**
     * Crossing because a touch sequence has begun, this event is
     * synthetic as the pointer might have not left the window.
     */
    TOUCH_BEGIN = 6,
    /**
     * Crossing because a touch sequence has ended, this event is
     * synthetic as the pointer might have not left the window.
     */
    TOUCH_END = 7,
    /**
     * Crossing because of a device switch (i.e. a mouse taking
     * control of the pointer after a touch device), this event
   ...
   * Predefined cursors.
   *
   * Note that these IDs are directly taken from the X cursor
   * font, and many of these cursors are either not useful, or
   * are not available on other platforms.
   *
   * The recommended way to create cursors is to use
   * {@link Gdk.Cursor.new_from_name}.
   */
  enum CursorType {
    /** [](X_cursor.png) */
    X_CURSOR = 0,
  ...
  /**
   * Indicates the specific type of tool being used being a
  ...
  /**
   * Indicates the device type. See
   * [above][GdkDeviceManager.description] for more information
   * about the meaning of these device types.
   */
  enum DeviceType {
    /**
     * Device is a master (or virtual) device. There will be an
    ...
    SLAVE = 1,
    /**
     * Device is a physical device, currently not attached to any
     * virtual device.
     */
    FLOATING = 2,
  }
  ...
  /**
   * Used in {@link DragContext} to the reason of a cancelled DND
   * operation.
   */
  ...
  /**
   * Used in {@link DragContext} to indicate the protocol
  ...
  enum DragProtocol {
    ...
    XDND = 2,
    /**
     * An extension to the Xdnd protocol for unclaimed root
     * window drops.
     */
    ROOTWIN = 3,
   ...
   * %GDK_3BUTTON_PRESS would translate into something
   * syntactically invalid (eg `Gdk.EventType.2ButtonPress`,
   * where a symbol is not allowed to start with a number). In
   * that case, the aliases %GDK_DOUBLE_BUTTON_PRESS and
  ...
  enum EventType {
    ...
    DESTROY = 1,
    /**
     * All or part of the window has become visible and needs to
     * be redrawn.
     */
    EXPOSE = 2,
    ...
    PROXIMITY_OUT = 21,
    /**
     * The mouse has entered the window while a drag is in
     * progress.
     */
    DRAG_ENTER = 22,
    ...
    SETTING = 33,
    /**
     * The owner of a selection has changed. This event type was
     * added in 2.6
     */
    OWNER_CHANGE = 34,
    /**
     * A pointer or keyboard grab was broken. This event type was
     * added in 2.8.
     */
    GRAB_BROKEN = 35,
    /**
     * The content of the window has been changed. This event
    ...
    TOUCH_UPDATE = 38,
    /**
     * A touch event sequence has finished. This event type was
     * added in 3.4.
     */
    TOUCH_END = 39,
    /**
     ...
     * determined by its phase field. This event type was added
     * in 3.18.
     */
    TOUCHPAD_SWIPE = 41,
    /**
    ...
    TOUCHPAD_PINCH = 42,
    /**
     * A tablet pad button press event. This event type was added
     * in 3.22.
     */
    PAD_BUTTON_PRESS = 43,
    /**
     * A tablet pad button release event. This event type was
     * added in 3.22.
     */
    ...
    PAD_RING = 45,
    /**
     * A tablet pad axis event from a "strip". This event type
     * was added in 3.22.
     */
    PAD_STRIP = 46,
    /**
     * A tablet pad group mode change. This event type was added
     * in 3.22.
     */
    PAD_GROUP_MODE = 47,
    /**
     * Marks the end of the GdkEventType enumeration. Added in
     * 2.18
     */
    EVENT_LAST = 48,
  }
  ...
  /**
   * Specifies the result of applying a {@link FilterFunc} to a
   * native event.
   */
  enum FilterReturn {
  ...
  /**
   * Indicates which monitor (in a multi-head setup) a window
  ...
  enum GrabOwnership {
    ...
    WINDOW = 1,
    /**
     * Other devices’ events are blocked for the whole
     * application.
     */
    APPLICATION = 2,
  }
  ...
  /**
   * Returned by {@link Gdk.Device.grab}, gdk_pointer_grab() and
   * gdk_keyboard_grab() to indicate success or the reason for
   * the failure of the grab attempt.
   */
  enum GrabStatus {
    ...
    ALREADY_GRABBED = 1,
    /**
     * The resource was grabbed more recently than the specified
     * time.
     */
    INVALID_TIME = 2,
    /**
    ...
    NOT_VIEWABLE = 3,
    /**
     * The resource is frozen by an active grab of another
     * client.
     */
    FROZEN = 4,
  ...
  enum InputMode {
    ...
    DISABLED = 0,
    /**
     * The device is enabled. The device’s coordinate space maps
     * to the entire screen.
     */
    SCREEN = 1,
    /**
     * The device is enabled. The device’s coordinate space is
     * mapped to a single window. The manner in which this window
     * is chosen is undefined, but it will typically be the same
     * way in which the focus window for key events is
     * determined.
     */
    WINDOW = 2,
  }
  ...
  /**
   * An enumeration describing the type of an input device in
   * general terms.
   */
  enum InputSource {
    /**
     * The device is a mouse. (This will be reported for the core
     * pointer, even if it is something else, such as a
     * trackball.)
     */
    MOUSE = 0,
    /**
    ...
    KEYBOARD = 4,
    /**
     * The device is a direct-input touch device, such as a
     * touchscreen or tablet. This device type has been added in
     * 3.4.
     */
    TOUCHSCREEN = 5,
    /**
     * The device is an indirect touch device, such as a
    ...
    TRACKPOINT = 7,
    /**
     * The device is a "pad", a collection of buttons, rings and
     * strips found in drawing tablets. This device type has been
     * added in 3.22.
     */
    TABLET_PAD = 8,
  }
   ...
   * in order to determine what modifiers the currently used
   * windowing system backend uses for particular purposes. For
   * example, on X11/Windows, the Control key is used for
   * invoking menu shortcuts (accelerators), whereas on Apple
   * computers it’s the Command key (which correspond to
   * %GDK_CONTROL_MASK and %GDK_MOD2_MASK, respectively).
   */
  enum ModifierIntent {
    /** The primary modifier used to invoke menu accelerators. */
    PRIMARY_ACCELERATOR = 0,
    /**
     * The modifier used to invoke context menus. Note that mouse
     * button 3 always triggers context menus. When this modifier
     * is not 0, it additionally triggers context menus when used
     * with mouse button 1.
     */
    CONTEXT_MENU = 1,
    /**
    ...
    EXTEND_SELECTION = 2,
    /**
     * The modifier used to modify selections, which in most
     * cases means toggling the clicked item into or out of the
     * selection.
     */
    MODIFY_SELECTION = 3,
    /**
     * When any of these modifiers is pressed, the key event
     * cannot produce a symbol directly. This is meant to be used
     * for input methods, and for use cases like typeahead
     * search.
     */
    NO_TEXT_INPUT = 4,
    /**
     * The modifier that switches between keyboard groups (AltGr
     * on X11/Windows and Option/Alt on OS X).
     */
    SHIFT_GROUP = 5,
    /**
     * The set of modifier masks accepted as modifiers in
     * accelerators. Needed because Command is mapped to MOD2 on
     * OSX, which is widely used, but on X11 MOD2 is NumLock and
     * using that for a mod key is problematic at best. Ref:
  ...
  enum NotifyType {
    ...
    ANCESTOR = 0,
    /**
     * The pointer moves between an ancestor and an inferior of
     * the window.
     */
    VIRTUAL = 1,
    /**
    ...
    NONLINEAR = 3,
    /**
     * The pointer moves between two windows which are not
     * ancestors of each other and the window is part of the
     * ancestor chain between one of these windows and their
  ...
  enum ScrollDirection {
     ...
     * {@link EventScroll}. See
     * {@link Gdk.event.get_scroll_deltas}. Since: 3.4
     */
    SMOOTH = 4,
  }
  ...
  /**
   * This enumeration describes how the red, green and blue
   * components of physical pixels on an output device are laid
   * out.
   */
  enum SubpixelLayout {
  ...
  /**
   * Specifies the current state of a touchpad gesture. All
   * gestures are guaranteed to begin with an event with phase
   * %GDK_TOUCHPAD_GESTURE_PHASE_BEGIN, followed by 0 or several
   * events with phase %GDK_TOUCHPAD_GESTURE_PHASE_UPDATE.
   *
   * A finished gesture may have 2 possible outcomes, an event
   * with phase %GDK_TOUCHPAD_GESTURE_PHASE_END will be emitted
   * when the gesture is considered successful, this should be
   * used as the hint to perform any permanent changes.
   *
   * Cancelled gestures may be so for a variety of reasons, due
   * to hardware or the compositor, or due to the gesture
   * recognition layers hinting the gesture did not finish
   * resolutely (eg. a 3rd finger being added during a pinch
   * gesture). In these cases, the last event will report the
   * phase %GDK_TOUCHPAD_GESTURE_PHASE_CANCEL, this should be
   * used as a hint to undo any visible/permanent changes that
   * were done throughout the progress of the gesture.
   *
   * See also {@link EventTouchpadSwipe} and
  ...
  enum TouchpadGesturePhase {
    ...
    UPDATE = 1,
    /**
     * The gesture was finished, changes should be permanently
     * applied.
     */
    END = 2,
   ...
   * values for a visual are converted into RGB values for
   * display.
   */
  enum VisualType {
     ...
     * components. Use {@link Gdk.Visual.get_red_pixel_details},
     * etc, to obtain information about how the components are
     * assembled into a pixel value.
     */
    TRUE_COLOR = 4,
    /**
     * Each pixel value contains red, green, and blue components
     * as for %GDK_VISUAL_TRUE_COLOR, but the components are
     * mapped via a color table into the final output table
     * instead of being converted directly.
     */
    DIRECT_COLOR = 5,
  }
  ...
  enum WindowType {
    ...
    OFFSCREEN = 5,
    /**
     * Subsurface-based window; This window is visually tied to a
     * toplevel, and is moved/stacked with it. Currently this
     * window type is only implemented in Wayland. Since 3.14
     */
    SUBSURFACE = 6,
  }
   ...
   * this when determining decoration and behaviour of the
  ...
  enum WindowTypeHint {
    ...
    DIALOG = 1,
    /**
     * Window used to implement a menu; GTK+ uses this hint only
     * for torn-off menus, see #GtkTearoffMenuItem.
     */
    MENU = 2,
    ...
    SPLASHSCREEN = 4,
    /**
     * Utility windows which are not detached toolbars or
     * dialogs.
     */
    UTILITY = 5,
    ...
    DROPDOWN_MENU = 8,
    /**
     * A menu that does not belong to a menubar, e.g. a context
     * menu.
     */
    POPUP_MENU = 9,
    /** A tooltip. */
    TOOLTIP = 10,
    /**
     * A notification - typically a “bubble” that belongs to a
     * status icon.
     */
    NOTIFICATION = 11,
  ...
  /**
   * #GDK_INPUT_OUTPUT windows are the standard kind of window
   * you might expect. Such windows receive events and are also
   * displayed on screen. #GDK_INPUT_ONLY windows are invisible;
   * they are usually placed above other windows in order to trap
   * or filter the events. You can’t draw on #GDK_INPUT_ONLY
   * windows.
   */
  ...
  /**
   * Positioning hints for aligning a window relative to a
   * rectangle.
   *
   * These hints determine how the window should be positioned in
   * the case that the window would fall off-screen if placed in
   * its ideal position.
   *
   * For example, %GDK_ANCHOR_FLIP_X will replace
   * %GDK_GRAVITY_NORTH_WEST with %GDK_GRAVITY_NORTH_EAST and
   * vice versa if the window extends beyond the left or right
   * edges of the monitor.
   *
   * If %GDK_ANCHOR_SLIDE_X is set, the window can be shifted
   * horizontally to fit on-screen. If %GDK_ANCHOR_RESIZE_X is
   * set, the window can be shrunken horizontally to fit.
   *
   * In general, when multiple flags are set, flipping should
   * take precedence over sliding, which should take precedence
   * over resizing.
   */
  enum AnchorHints {
  ...
  enum DragAction {
    ...
    COPY = 2,
    /**
     * Move the data, i.e. first copy it, then delete it from the
     * source using the DELETE target of the X selection
     * protocol.
     */
    MOVE = 4,
    /**
    ...
    LINK = 8,
    /**
     * Special action which tells the source that the destination
  ...
  /**
   * A set of bit-flags to indicate which events a window is to
   * receive. Most of these masks map onto one or more of the
   * {@link EventType} event types above.
   *
   * See the [input handling overview][chap-input-handling] for
   * details of [event masks][event-masks] and [event
   * propagation][event-propagation].
   *
   * %GDK_POINTER_MOTION_HINT_MASK is deprecated. It is a special
   * mask to reduce the number of %GDK_MOTION_NOTIFY events
   * received. When using %GDK_POINTER_MOTION_HINT_MASK, fewer
   * %GDK_MOTION_NOTIFY events will be sent, some of which are
   * marked as a hint (the is_hint member is %TRUE). To receive
   * more motion events after a motion hint event, the
   * application needs to asks for more, by calling
   * {@link Gdk.event.request_motions}.
   *
   * Since GTK 3.8, motion events are already compressed by
   * default, independent of this mechanism. This compression can
   * be disabled with gdk_window_set_event_compression(). See the
   * documentation of that function for details.
   *
   * If %GDK_TOUCH_MASK is enabled, the window will receive touch
   * events from touch-enabled devices. Those will come as
   * sequences of #GdkEventTouch with type %GDK_TOUCH_UPDATE,
   * enclosed by two events with type %GDK_TOUCH_BEGIN and
   * %GDK_TOUCH_END (or %GDK_TOUCH_CANCEL).
   * gdk_event_get_event_sequence() returns the event sequence
   * for these events, so different sequences may be
   * distinguished.
   */
  enum EventMask {
    ...
    /**
     * Receive events about window configuration changes of child
     * windows
     */
  ...
  /**
   * {@link FrameClockPhase} is used to represent the different
   * paint clock phases that can be requested. The elements of
   * the enumeration correspond to the signals of
   * #GdkFrameClock.
   */
  enum FrameClockPhase {
    ...
    NONE = 0,
    /**
     * Corresponds to GdkFrameClock::flush-events. Should not be
     * handled by applications.
     */
    FLUSH_EVENTS = 1,
    /**
     * Corresponds to GdkFrameClock::before-paint. Should not be
     * handled by applications.
     */
    BEFORE_PAINT = 2,
    ...
    PAINT = 16,
    /**
     * Corresponds to GdkFrameClock::resume-events. Should not be
     * handled by applications.
     */
    RESUME_EVENTS = 32,
    /**
     * Corresponds to GdkFrameClock::after-paint. Should not be
     * handled by applications.
     */
    AFTER_PAINT = 64,
  }
   ...
   * and mouse buttons in various event types. Typical modifier
   * keys are Shift, Control, Meta, Super, Hyper, Alt, Compose,
   * Apple, CapsLock or ShiftLock.
   *
   * Like the X Window System, GDK supports 8 modifier keys and 5
   * mouse buttons.
   *
   * Since 2.10, GDK recognizes which of the Meta, Super or Hyper
   * keys are mapped to Mod2 - Mod5, and indicates this by
   * setting %GDK_SUPER_MASK, %GDK_HYPER_MASK or %GDK_META_MASK
   * in the state field of key events.
   *
   * Note that GDK may add internal values to events which
   * include reserved values such as
   * %GDK_MODIFIER_RESERVED_13_MASK. Your code should preserve
   * and ignore them. You can use %GDK_MODIFIER_MASK to remove
   * all reserved values.
   *
   * Also note that the GDK X backend interprets button press
   * events for button 4-7 as scroll events, so %GDK_BUTTON4_MASK
  ...
  enum ModifierType {
    ...
    SHIFT_MASK = 1,
    /**
     * A Lock key (depending on the modifier mapping of the X
    ...
    MOD1_MASK = 8,
    /**
     * The fifth modifier key (it depends on the modifier mapping
     * of the X server which key is interpreted as this
     * modifier).
     */
    MOD2_MASK = 16,
    /**
     * The sixth modifier key (it depends on the modifier mapping
     * of the X server which key is interpreted as this
     * modifier).
     */
    MOD3_MASK = 32,
     ...
     * between (keyval, modifiers) pairs from key press and
     * release events.
     */
  ...
  /**
   * These are hints originally defined by the Motif toolkit. The
   * window manager can use them when determining the functions
   * to offer for the window. The hint must be set before mapping
   * the window.
   */
  enum WMFunction {
  ...
  /**
   * Used to indicate which fields in the {@link WindowAttr}
   * struct should be honored. For example, if you filled in the
   * “cursor” and “x” fields of #GdkWindowAttr, pass “#GDK_WA_X |
   * #GDK_WA_CURSOR” to {@link Gdk.Window.new}. Fields in
   * #GdkWindowAttr not covered by a bit in this enum are
   * required; for example, the #width/#height, #wclass, and
   * #window_type fields are required, they have no corresponding
  ...
  /**
   * Used to indicate which fields of a {@link Geometry} struct
   * should be paid attention to. Also, the presence/absence of
   * #GDK_HINT_POS, #GDK_HINT_USER_POS, and #GDK_HINT_USER_SIZE
  ...
  enum WindowHints {
    ...
    USER_POS = 128,
    /**
     * Indicates that the window’s size was explicitly set by the
     * user
     */
    USER_SIZE = 256,
  }
  ...
  enum WindowState {
    ...
    BELOW = 64,
    /**
     * The window is presented as focused (with active
     * decorations).
     */
    FOCUSED = 128,
    /**
     * The window is in a tiled state, Since 3.10. Since 3.22.23,
   ...
   * {@link Gdk.event.handler_set} to handle all GDK events.
   */
  interface EventFunc {
    /**
     * Specifies the type of function passed to
     ...
     * @param data User data set when the event handler was
     *   installed with {@link Gdk.event.handler_set}.
     */
    (event: Event, data: any | null): void;
  }

  /**
   * Specifies the type of function used to filter native events
   * before they are converted to GDK events.
   *
   * When a filter is called, #event is unpopulated, except for
   * `event->window`. The filter may translate the native event
   * to a GDK event and store the result in #event, or handle it
  ...
  interface FilterFunc {
    /**
     * Specifies the type of function used to filter native
     * events before they are converted to GDK events.
     *
     * When a filter is called, #event is unpopulated, except for
     * `event->window`. The filter may translate the native event
     * to a GDK event and store the result in #event, or handle
     * it without translation. If the filter translates the event
     * and processing should continue, it should return
     * %GDK_FILTER_TRANSLATE.
     *
     * @param xevent The native event to filter.
     * @param event The GDK event to which the X event will be
     *   translated.
   ...
   * {@link Gdk.Window.invalidate_maybe_recurse}. It gets called
   * for each child of the window to determine whether to
  ...
  /**
   * Whenever some area of the window is invalidated (directly in
   * the window or in a child window) this gets called with
   * #region in the coordinate space of #window. You can use
   * #region to just keep track of the dirty region, or you can
   * actually change #region in case you are doing display tricks
   * like showing a child in multiple places.
   */
  interface WindowInvalidateHandlerFunc {
    /**
     * Whenever some area of the window is invalidated (directly
     * in the window or in a child window) this gets called with
     * #region in the coordinate space of #window. You can use
     * #region to just keep track of the dirty region, or you can
     * actually change #region in case you are doing display
  ...
  interface IEvent {
    ...
    /**
     * If both events contain X/Y information, this function will
     * return %TRUE and return in #angle the relative angle from
     * #event1 to #event2. The rotation direction for positive
     * angles is from the positive X axis towards the positive Y
     * axis.
     *
    ...
    /**
     * If both events contain X/Y information, the center of both
    ...
    /**
     * If both events have X/Y information, the distance between
     * both coordinates (as in a straight line going from #event1
     * to #event2) will be returned.
     *
     * @param event2 Second {@link Event}
    ...
    /**
     * Copies a {@link Event}, copying or incrementing the
     * reference count of the resources associated with it (e.g.
     * #GdkWindow’s and strings).
     *
     * @returns A copy of #event. The returned {@link Event}
     *   should be freed with {@link Gdk.Event.free}.
     */
    copy(): Event;
    /**
     * Frees a {@link Event}, freeing or decrementing any
     * resources associated with it. Note that this function
     * should only be called with events returned from functions
     * such as {@link Gdk.event.peek}, gdk_event_get(),
     * gdk_event_copy() and gdk_event_new().
     */
    free(): void;
    /**
     * Extract the axis value for a particular axis use from an
     * event structure.
     *
     * @param axis_use The axis use to look for
     * @returns %TRUE if the specified axis was found, otherwise
     *   %FALSE
     *
    ...
    /**
     * Extract the event window relative x/y coordinates from an
     * event.
     *
     * @returns %TRUE if the event delivered event window
     *   coordinates
     *
    ...
    /**
     * If the event contains a “device” field, this function will
     * return it, else it will return %NULL.
     *
     ...
     * different tools (eg. a tablet), this function will return
     * a {@link DeviceTool} representing the tool that caused the
     * event. Otherwise, %NULL will be returned.
     *
     * Note: the #GdkDeviceTool<!-- -->s will be constant during
     * the application lifetime, if settings must be stored
     * persistently across runs, see
     ...
     * {@link EventSequence} to which the event belongs.
    ...
    get_keyval(): [boolean, number];
    /**
     * #event: a {@link Event} Returns whether this event is an
     * 'emulated' pointer event (typically from a touch event),
    ...
    /**
     * Extract the root window relative x/y coordinates from an
     * event.
     *
     * @returns %TRUE if the event delivered root window
     *   coordinates
     *
     ...
     *
     * This is usually hardware_keycode. On Windows this is the
     * high word of WM_KEY{DOWN,UP} lParam which contains the
     * scancode and some extended flags.
     *
     * @returns The associated keyboard scancode or 0
     */
    get_scancode(): number;
    /**
     * Returns the screen for the event. The screen is typically
     * the screen for `event->any.window`, but for events such as
     * mouse events, it is the screen where the pointer was when
     * the event occurs - that is, the screen which has the root
     * window to which `event->motion.x_root` and
     ...
     *
     * @returns %TRUE if the event contains smooth scroll
     ...
     *
     * If #event is not of type %GDK_SCROLL, the contents of
     * #direction are undefined.
     *
     * If you wish to handle both discrete and smooth scrolling,
     * you should check the return value of this function, or of
     * {@link Gdk.event.get_scroll_deltas}; for instance:
     *
     * |[<!-- language="C" -->
     *
     * GdkScrollDirection direction; double vscroll_factor = 0.0;
     ...
     *     }
     *
     * Else if (gdk_event_get_scroll_deltas (event, &x_scroll,
     * &y_scroll)) { // Handle smooth scrolling directly
    ...
    get_seat(): Seat;
    /**
     * This function returns the hardware (slave) {@link Device}
     * that has triggered the event, falling back to the virtual
     * (master) device (as in {@link Gdk.event.get_device}) if the
     * event wasn’t caused by interaction with a hardware device.
     * This may happen for example in synthesized crossing events
     * after a #GdkWindow updates its geometry or a grab is
     * acquired/released.
     *
     * If the event does not contain a device field, this
     ...
     * #state. Otherwise stores an empty state (0). Returns %TRUE
     * if there was a state field in the event. #event may be
     * %NULL, in which case it’s treated as if the event had no
     * state field.
     *
    ...
    /**
     * Returns the time stamp from #event, if there is one;
     * otherwise returns #GDK_CURRENT_TIME. If #event is %NULL,
     ...
     * Scroll sequences with smooth scroll information may
     * provide a stop scroll event once the interaction with the
     * device finishes, e.g. by lifting a finger. This stop
     * scroll event is the signal that a widget may trigger
    ...
    /**
     * Appends a copy of the given event onto the front of the
     * event queue for event->any.window’s display, or the
     * default event queue if event->any.window is %NULL. See
     * {@link Gdk.Display.put_event}.
     */
    put(): void;
    /**
     * Sets the device for #event to #device. The event must have
     * been allocated by GTK+, for instance, by
     * {@link Gdk.Event.copy}.
     *
     * @param device A {@link Device}
     */
    set_device(device: Device): void;
    /**
     * Sets the device tool for this event, should be rarely
     * used.
     *
    ...
    /**
     * Sets the screen for #event to #screen. The event must have
     * been allocated by GTK+, for instance, by
     * {@link Gdk.Event.copy}.
     *
     * @param screen A {@link Screen}
     ...
     *
     * The event must have been allocated by GTK+, for instance
     * by {@link Gdk.Event.copy}.
     *
     * @param device A {@link Device}
     */
    set_source_device(device: Device): void;
    /**
     * This function returns whether a {@link EventButton} should
     * trigger a context menu, according to platform conventions.
     * The right mouse button always triggers context menus.
     * Additionally, if {@link Gdk.Keymap.get_modifier_mask}
     * returns a non-0 mask for
     * %GDK_MODIFIER_INTENT_CONTEXT_MENU, then the left mouse
     * button will also trigger a context menu if this modifier
     * is pressed.
     *
     * This function should always be used instead of simply
     * checking for event->button == %GDK_BUTTON_SECONDARY.
     *
    ...
    connect(
      signal: "notify::expose",
      callback: (owner: this, ...args: any) => void
    ): number;
    connect(
      signal: "notify::visibility",
      callback: (owner: this, ...args: any) => void
    ): number;
    connect(
      signal: "notify::motion",
      callback: (owner: this, ...args: any) => void
    ): number;
    connect(
    ...
    connect(
      signal: "notify::crossing",
      callback: (owner: this, ...args: any) => void
    ): number;
    connect(
      signal: "notify::focus_change",
      callback: (owner: this, ...args: any) => void
    ): number;
    connect(
      signal: "notify::configure",
      callback: (owner: this, ...args: any) => void
    ): number;
    connect(
    ...
    connect(
      signal: "notify::selection",
      callback: (owner: this, ...args: any) => void
    ): number;
    connect(
      signal: "notify::owner_change",
      callback: (owner: this, ...args: any) => void
    ): number;
    connect(
      signal: "notify::proximity",
      callback: (owner: this, ...args: any) => void
    ): number;
    connect(
      signal: "notify::dnd",
    ...
    connect(
      signal: "notify::window_state",
      callback: (owner: this, ...args: any) => void
    ): number;
    connect(
      signal: "notify::setting",
      callback: (owner: this, ...args: any) => void
    ): number;
    connect(
      signal: "notify::grab_broken",
      callback: (owner: this, ...args: any) => void
    ): number;
    connect(
      signal: "notify::touchpad_swipe",
      callback: (owner: this, ...args: any) => void
    ): number;
    connect(
      signal: "notify::touchpad_pinch",
      callback: (owner: this, ...args: any) => void
    ): number;
    connect(
      signal: "notify::pad_button",
      callback: (owner: this, ...args: any) => void
    ): number;
    connect(
      signal: "notify::pad_axis",
      callback: (owner: this, ...args: any) => void
    ): number;
    connect(
      signal: "notify::pad_group_mode",
      callback: (owner: this, ...args: any) => void
    ): number;
  }

  type EventInitOptionsMixin = Pick<
    IEvent,
    | "type"
    | "any"
    | "expose"
   ...
   * types, and can always be accessed with the following code,
   * no matter what type of event it is:
   *
   * |[<!-- language="C" -->
   *
   * GdkEvent *event; GdkEventType type;
   *
   * Type = event->type; ]|
   *
   * To access other fields of the event, the pointer to the
   * event can be cast to the appropriate event type, or the
   * union member name can be used. For example if the event type
   * is %GDK_BUTTON_PRESS then the x coordinate of the button
  ...
  class Event {
    ...
    /**
     * Creates a new event of the given type. All fields are set
     * to 0.
     *
     * @param type A {@link EventType}
     * @returns A newly-allocated {@link Event}. The returned
     *   #GdkEvent should be freed with {@link Gdk.Event.free}.
     */
    public static new(type: EventType): Event;
    /**
     * Checks all open displays for a {@link Event} to process,to
     * be processed on, fetching events from the windowing system
     * if necessary. See {@link Gdk.Display.get_event}.
     *
     * @returns The next {@link Event} to be processed, or %NULL
     *   if no events are pending. The returned #GdkEvent should
     *   be freed with {@link Gdk.Event.free}.
     ...
     * @param notify The function to call when the handler
     *   function is removed, i.e. when
     *   {@link Gdk.event.handler_set} is called with another
     *   event handler.
     */
    public static handler_set(
      func: EventFunc,
      data: any | null,
      notify: GLib.DestroyNotify
    ): void;
    /**
     * If there is an event waiting in the event queue of some
     * open display, returns a copy of it. See
     * {@link Gdk.Display.peek_event}.
     *
     * @returns A copy of the first {@link Event} on some event
     *   queue, or %NULL if no events are in any queues. The
     *   returned #GdkEvent should be freed with
     *   {@link Gdk.Event.free}.
     */
     ...
     * {@link Gdk.Window.get_pointer} to request further motion
     * notifies, because it also works for extension events where
     * motion notifies are provided for devices other than the
     * core pointer. Coordinate extraction, processing and
     * requesting more motion events from a %GDK_MOTION_NOTIFY
     * event usually works like this:
     *
     * |[<!-- language="C" -->
     *
     * { // motion_event handler x = motion_event->x; y =
     * motion_event->y; // handle (x,y) motion
     * gdk_event_request_motions (motion_event); // handles
     * is_hint events } ]|
     *
   ...
   * @param only_if_exists If %TRUE, GDK is allowed to not create
   *   a new atom, but just return %GDK_NONE if the requested
   *   atom doesn’t already exists. Currently, the flag is
   *   ignored, since checking the existance of an atom is as
   ...
   * {@link Gdk.atom.intern} except that if a new {@link Atom} is
   * created the string itself is used rather than a copy. This
   * saves memory, but can only be used if the string will always
   * exist. It can be used with statically allocated strings in
   * the main program, but not with statically allocated memory
   * in dynamically loaded modules, if you expect to ever unload
   * the module again (e.g. do not use this function in GTK+
   * theme engines).
   *
   ...
   *
   * Note that calling {@link Cairo.reset_clip} on the resulting
   * #cairo_t will produce undefined results, so avoid it at all
   * costs.
   *
   * Typically, this function is used to draw on a {@link Window}
   * out of the paint cycle of the toolkit; this should be
   * avoided, as it breaks various assumptions and
   * optimizations.
   *
   * If you are drawing on a native #GdkWindow in response to a
   * %GDK_EXPOSE event you should use
   * gdk_window_begin_draw_frame() and
   * gdk_drawing_context_get_cairo_context() instead. GTK will
  ...
  /**
   * This is the main way to draw GL content in GTK+. It takes a
   * render buffer ID (#source_type == #GL_RENDERBUFFER) or a
   * texture id (#source_type == #GL_TEXTURE) and draws it onto
   * #cr with an OVER operation, respecting the current clip. The
   * top left corner of the rectangle specified by #x, #y, #width
   * and #height will be drawn at the current (0,0) position of
   * the cairo_t.
   *
   * This will work for _all_ cairo_t, as long as #window is
   * realized, but the fallback implementation that reads back
   * the pixels from the buffer may be used in the general case.
   * In the case of direct drawing to a window with no special
   * effects applied to #cr it will however use a more efficient
   * approach.
   *
   * For #GL_RENDERBUFFER the code will always fall back to
   * software for buffers with alpha components, so make sure you
   * use #GL_TEXTURE if using alpha.
   *
   * Calling this may change the current GL context.
   *
   * @param cr A cairo context
   * @param window The window we're rendering for (not
   *   necessarily into)
   * @param source The GL ID of the source buffer
   * @param source_type The type of the #source
   * @param buffer_scale The scale-factor that the #source buffer
   *   is allocated for
   * @param x The source x position in #source to start copying
   *   from in GL coordinates
   * @param y The source y position in #source to start copying
   *   from in GL coordinates
   ...
   * {@link Cairo.clip_extents}. It rounds the clip extents to
   * integer coordinates and returns a boolean indicating if a
   * clip area exists.
   *
   * @param cr A cairo context
   * @returns %TRUE if a clip rectangle exists, %FALSE if all of
   *   #cr is clipped and all drawing can be skipped
   *
  ...
  /**
   * Creates region that describes covers the area where the
   * given #surface is more than 50% opaque.
   *
   * This function takes into account device offsets that might
   * be set with {@link Cairo.Surface.set_device_offset}.
   *
   * @param surface A cairo surface
   * @returns A #cairo_region_t; must be freed with
   *   {@link Cairo.Region.destroy}
   */
   ...
   *
   * The pattern has an extend mode of %CAIRO_EXTEND_NONE and is
   * aligned so that the origin of #pixbuf is #pixbuf_x,
   * #pixbuf_y.
   *
   * @param cr A cairo context
   * @param pixbuf A {@link Pixbuf}
   * @param pixbuf_x X coordinate of location to place upper left
   *   corner of #pixbuf
   * @param pixbuf_y Y coordinate of location to place upper left
   *   corner of #pixbuf
   */
   ...
   *
   * The pattern has an extend mode of %CAIRO_EXTEND_NONE and is
   * aligned so that the origin of #window is #x, #y. The window
   * contains all its subwindows when rendering.
   *
   * Note that the contents of #window are undefined outside of
   * the visible part of #window, so use this function with
   * care.
   *
   * @param cr A cairo context
   * @param window A {@link Window}
   * @param x X coordinate of location to place upper left corner
   *   of #window
   * @param y Y coordinate of location to place upper left corner
   *   of #window
   */
  ...
  /**
   * Creates an image surface with the same contents as the
   * pixbuf.
   *
   * @param pixbuf A {@link Pixbuf}
   * @param scale The scale of the new surface, or 0 to use same
   *   as #window
   * @param for_window The window this will be drawn to, or %NULL
   * @returns A new cairo surface, must be freed with
   ...
   * (taken from the X11 `rgb.txt` file), or it can be a
   * hexadecimal value in the form “#rgb” “#rrggbb”, “#rrrgggbbb”
   * or “#rrrrggggbbbb” where “r”, “g” and “b” are hex digits of
  ...
  /**
   * Disables multidevice support in GDK. This call must happen
   * prior to {@link Gdk.Display.open}, gtk_init(),
   * gtk_init_with_args() or gtk_init_check() in order to take
   * effect.
   *
   * Most common GTK+ applications won’t ever need to call this.
   * Only applications that do mixed GDK/Xlib calls could want to
   * disable multidevice support if such Xlib code deals with
   * input devices in any way and doesn’t observe the presence of
   * XInput 2.
   */
   ...
   *
   * This function does not need to be called in managed drag and
   * drop operations. See {@link Gdk.DragContext.manage_dnd} for
   * more information.
   *
  ...
  /**
   * Starts a drag and creates a new drag context for it. This
   * function assumes that the drag is controlled by the client
   * pointer device, use {@link Gdk.drag.begin_for_device} to
   * begin a drag with a different device.
   *
   * This function is called by the drag source.
   *
   * @param window The source window for this drag.
   * @param targets The offered targets, as list of {@link Atoms}
   ...
   * @param device The device that controls this drag
   * @param targets The offered targets, as list of {@link Atoms}
   * @param x_root The x coordinate where the drag nominally
   *   started
   * @param y_root The y coordinate where the drag nominally
   *   started
   * @returns A newly created {@link DragContext}
   */
   ...
   *
   * This function is called by the drag source, and should be
   * the last call before dropping the reference to the
   * #context.
   *
   * The {@link DragContext} will only take the first
   * {@link Gdk.drag.drop_done} call as effective, if this
   * function is called multiple times, all subsequent calls will
   * be ignored.
   *
   * @param context A {@link DragContext}
   ...
   * #dest_window and #protocol parameters for
   * {@link Gdk.drag.motion}.
   *
   * @param context A {@link DragContext}
   * @param drag_window A window which may be at the pointer
   *   position, but should be ignored, since it is put up by the
   *   drag source as an icon
   * @param screen The screen where the destination window is
   *   sought
   * @param x_root The x position of the pointer in root
   *   coordinates
   * @param y_root The y position of the pointer in root
   *   coordinates
  ...
  /**
   * Updates the drag context when the pointer moves or the set
   * of actions changes.
   *
   * This function is called by the drag source.
   *
   ...
   * @param protocol The DND protocol in use, obtained by
   *   {@link Gdk.drag_find_window}
   * @param x_root The x position of the pointer in root
   *   coordinates
   * @param y_root The y position of the pointer in root
   ...
   * to {@link Gdk.drag.motion} called by the drag source.
   *
   * @param context A {@link DragContext}
   * @param action The selected action which will be taken when a
   *   drop happens, or 0 to indicate that a drop will not be
   *   accepted
   * @param time_ The timestamp for this operation
   */
   ...
   * {@link Gdk.error.trap_push}. May block until an error has
   * been definitively received or not received from the X
   * server. gdk_error_trap_pop_ignored() is preferred if you
   * don’t need to know whether an error occurred, because it
   * never has to block. If you don't need the return value of
   * gdk_error_trap_pop(), use gdk_error_trap_pop_ignored().
   *
   * Prior to GDK 3.0, this function would not automatically sync
   * for you, so you had to gdk_flush() if your last call to Xlib
   * was not a blocking round trip.
   *
  ...
  /**
   * Removes an error trap pushed with
   * {@link Gdk.error.trap_push}, but without bothering to wait
   * and see whether an error occurred. If an error arrives later
   * asynchronously that was triggered while the trap was pushed,
   * that error will be ignored.
   */
  function error_trap_pop_ignored(): void;

  /**
   * This function allows X errors to be trapped instead of the
   * normal behavior of exiting the application. It should only
   * be used if it is not possible to avoid the X error in any
   * other way. Errors are ignored on all {@link Display}
   * currently known to the #GdkDisplayManager. If you don’t care
   * which error happens and just want to ignore everything, pop
   * with {@link Gdk.error.trap_pop_ignored}. If you need the
   * error code, use gdk_error_trap_pop() which may have to block
   * and wait for the error to arrive from the X server.
   *
   * This API exists on all platforms but only does anything on
   * X.
   *
   * You can use gdk_x11_display_error_trap_push() to ignore
  ...
  /**
   * Checks all open displays for a {@link Event} to process,to be
   * processed on, fetching events from the windowing system if
   * necessary. See {@link Gdk.Display.get_event}.
   *
   * @returns The next {@link Event} to be processed, or %NULL if
   *   no events are pending. The returned #GdkEvent should be
   ...
   * @param data User data to pass to the function.
   * @param notify The function to call when the handler function
   *   is removed, i.e. when {@link Gdk.event.handler_set} is
   *   called with another event handler.
   */
  function event_handler_set(
    func: EventFunc,
    data: any | null,
    notify: GLib.DestroyNotify
  ): void;
   ...
   * {@link Gdk.Window.get_pointer} to request further motion
   * notifies, because it also works for extension events where
   * motion notifies are provided for devices other than the core
   * pointer. Coordinate extraction, processing and requesting
   * more motion events from a %GDK_MOTION_NOTIFY event usually
   * works like this:
   *
   * |[<!-- language="C" -->
   *
   * { // motion_event handler x = motion_event->x; y =
   * motion_event->y; // handle (x,y) motion
   * gdk_event_request_motions (motion_event); // handles is_hint
   * events } ]|
   *
   * @param event A valid {@link Event}
   */
  ...
  /**
   * Checks if any events are ready to be processed for any
   * display.
   *
  ...
  /**
   * Flushes the output buffers of all display connections and
   * waits until all requests have been processed. This is rarely
   * needed by applications.
   */
  function flush(): void;

  /**
   * Obtains the root window (parent all other windows are
   ...
   * `DISPLAY` environment variable or the `--display` command
   * line option.
   *
  ...
  /**
   * Gets the display name specified in the command line
   * arguments passed to gdk_init() or gdk_parse_args(), if any.
   *
   * @returns The display name, if specified explicitly,
   *   otherwise %NULL this string is owned by GTK+ and must not
   *   be modified or freed.
   */
  function get_display_arg_name(): string | null;

  /**
   * Gets the program class. Unless the program class has
   * explicitly been set with {@link Gdk.set.program_class} or
   * with the `--class` commandline option, the default value is
   * the program name (determined with g_get_prgname()) with the
  ...
  /**
   * Initializes the GDK library and connects to the windowing
   * system. If initialization fails, a warning message is output
   * and the application terminates with a call to `exit(1)`.
   *
   * Any arguments used by GDK are removed from the array and
   * #argc and #argv are updated accordingly.
   *
   * GTK+ initializes GDK in gtk_init() and so this function is
   * not usually needed by GTK+ applications.
   */
  function init(): void;

  /**
   * Initializes the GDK library and connects to the windowing
   * system, returning %TRUE on success.
   *
   * Any arguments used by GDK are removed from the array and
   * #argc and #argv are updated accordingly.
   *
   ...
   * {@link Gdk.keyboard.ungrab}. This overrides any previous
   * keyboard grab by this client.
   *
   * If you set up anything at the time you take the grab that
   * needs to be cleaned up when the grab ends, you should handle
   * the {@link EventGrabBroken} events that are emitted when the
   * grab ends unvoluntarily.
   *
   * @param window The {@link Window} which will own the grab (the
   *   grab window).
   * @param owner_events If %FALSE then all keyboard events are
   ...
   *   are always reported, independant of the event mask set by
   *   the application.
   * @param time_ A timestamp from a {@link Event}, or
  ...
  /**
   * Ungrabs the keyboard on the default display, if it is
   ...
   * #symbol. Examples of keyvals are #GDK_KEY_a, #GDK_KEY_Enter,
   * #GDK_KEY_F1, etc.
   *
   ...
   * @returns %TRUE if #keyval is in lower case, or if #keyval is
   ...
   *
   * @param keyval A key value.
   * @returns %TRUE if #keyval is in upper case, or if #keyval is
   *   not subject to case conversion.
   */
   ...
   *
   * @param keyval A key value
   * @returns A string containing the name of the key, or %NULL
   *   if #keyval is not a valid key. The string should not be
   *   modified.
   */
   ...
   *   if it is already in lower case or it is not subject to
   *   case conversion.
   */
  function keyval_to_lower(keyval: number): number;

  /**
   * Convert from a GDK key symbol to the corresponding ISO10646
   * (Unicode) character.
   *
   ...
   *   if it is already in upper case or it is not subject to
   *   case conversion.
   */
   ...
   * {@link Gdk.Screen.list_visuals}) A visual describes a
   ...
   *
   * Call g_list_free() on the return value when you’re finished
   * with it.
   *
   * @returns A list of visuals; the list must be freed, but not
   *   its contents
   */
  ...
  /**
   * Indicates to the GUI environment that the application has
   * finished loading. If the applications opens windows, this
   * function is normally called after opening the application’s
   ...
   *
   * @param window A {@link Window}
   * @returns The embedding {@link Window}, or %NULL if #window is
   *   not an mbedded offscreen window
   */
  function offscreen_window_get_embedder(window: Window): Window | null;

  /**
   * Gets the offscreen surface that an offscreen window renders
   * into. If you need to keep this around over window resizes,
   ...
   *
   * To fully embed an offscreen window, in addition to calling
   * this function, it is also necessary to handle the
   * {@link Window.pick_embedded_child} signal on the #embedder
   * and the #GdkWindow::to-embedder and
   * #GdkWindow::from-embedder signals on #window.
   *
   * @param window A {@link Window}
   * @param embedder The {@link Window} that #window gets embedded
   *   in
   */
   ...
   * (see #cairo_font_options_t) for the default screen; if these
   * options change it will not be updated. Using
   * gtk_widget_get_pango_context() is more convenient if you
   * want to keep a context around and track changes to the
   ...
   *
   * The newly created context will have the default font options
   * (see #cairo_font_options_t) for the display; if these
   * options change it will not be updated. Using
   * gtk_widget_get_pango_context() is more convenient if you
   * want to keep a context around and track changes to the font
   * rendering settings.
   *
   * @param display The {@link Display} for which the context is
   *   to be created
   ...
   * screen’s font rendering settings.
   *
   * @param screen The {@link Screen} for which the context is to
   *   be created.
  ...
  /**
   * Obtains a clip region which contains the areas where the
   * given ranges of text would be drawn. #x_origin and #y_origin
   * are the top left point to center the layout. #index_ranges
   * should contain ranges of bytes in the layout’s text.
   *
   * Note that the regions returned correspond to logical extents
   * of the text ranges, not ink extents. So the drawn layout may
   * in fact touch areas out of the clip region. The clip region
   * is mainly useful for highlightling parts of text, such as
   * when text is selected.
   *
   * @param layout A #PangoLayout
   * @param x_origin X pixel where you intend to draw the layout
   *   with this clip
   * @param y_origin Y pixel where you intend to draw the layout
   *   with this clip
   * @param index_ranges Array of byte indexes into the layout,
   *   where even members of array are start indexes and odd
   *   elements are end indexes
   * @param n_ranges Number of ranges in #index_ranges, i.e. half
   *   the size of #index_ranges
   ...
   * should contain ranges of bytes in the layout’s text. The
   * clip region will include space to the left or right of the
   * line (to the layout bounding box) if you have indexes above
   * or below the indexes contained inside the line. This is to
   * draw the selection all the way to the side of the layout.
   * However, the clip region is in line coordinates, not layout
   * coordinates.
   *
   * Note that the regions returned correspond to logical extents
   * of the text ranges, not ink extents. So the drawn line may
   * in fact touch areas out of the clip region. The clip region
   * is mainly useful for highlightling parts of text, such as
   * when text is selected.
   *
   ...
   * @param x_origin X pixel where you intend to draw the layout
   *   line with this clip
   * @param y_origin Baseline pixel where you intend to draw the
   ...
   * @returns A clip region containing the given ranges
   */
  function pango_layout_line_get_clip_region(
  ...
  /**
   * Parse command line arguments, and store for future use by
   * calls to {@link Gdk.Display.open}.
   *
   * Any arguments used by GDK are removed from the array and
   * #argc and #argv are updated accordingly.
   *
   * You shouldn’t call this function explicitly if you are using
   * gtk_init(), gtk_init_check(), gdk_init(), or
   * gdk_init_check().
   */
   ...
   * to an RGB(A) representation inside a {@link Pixbuf}. This
   * allows you to efficiently read individual pixels from cairo
   * surfaces. For #GdkWindows, use
   * {@link GdkPixbuf.get_from_window} instead.
   *
   * This function will create an RGB pixbuf with 8 bits per
   * channel. The pixbuf will contain an alpha channel if the
  ...
  /**
   * Transfers image data from a {@link Window} and converts it to
   * an RGB(A) representation inside a #GdkPixbuf. In other
   * words, copies image data from a server-side drawable to a
   * client-side RGB(A) buffer. This allows you to efficiently
   * read individual pixels on the client side.
   *
   * This function will create an RGB pixbuf with 8 bits per
   * channel with the size specified by the #width and #height
   * arguments scaled by the scale factor of #window. The pixbuf
   * will contain an alpha channel if the #window contains one.
   *
   * If the window is off the screen, then there is no image data
   * in the obscured/offscreen regions to be placed in the
   * pixbuf. The contents of portions of the pixbuf corresponding
   * to the offscreen region are undefined.
   *
   * If the window you’re obtaining data from is partially
   ...
   * and if it fails it returns %NULL; so check the return
   * value.)
   *
   ...
   * passed to this application until the pointer is ungrabbed
   * with {@link Gdk.pointer.ungrab}, or the grab window becomes
   * unviewable. This overrides any previous pointer grab by this
   * client.
   *
   * Pointer grabs are used for operations which need complete
   * control over mouse events, even if the mouse leaves the
   * application. For example in GTK+ it is used for Drag and
   * Drop, for dragging the handle in the #GtkHPaned and
   * #GtkVPaned widgets.
   *
   * Note that if the event mask of an X window has selected both
   * button press and button release events, then a button press
   * event will cause an automatic pointer grab until the button
   * is released. X does this automatically since most
   * applications expect to receive button press and release
   * events in pairs. It is equivalent to a pointer grab on the
   ...
   * @param window The {@link Window} which will own the grab (the
   *   grab window).
   * @param owner_events If %FALSE then all pointer events are
   *   reported with respect to #window and are only reported if
   *   selected by #event_mask. If %TRUE then pointer events for
   *   this application are reported as normal, but pointer
   *   events outside this application are reported with respect
   *   to #window and only if selected by #event_mask. In either
   *   mode, unreported events are discarded.
   * @param event_mask Specifies the event mask, which is used in
   *   accordance with #owner_events. Note that only pointer
   *   events (i.e. button and motion events) may be selected.
   * @param confine_to If non-%NULL, the pointer will be confined
   *   to this window during the grab. If the pointer is outside
   *   #confine_to, it will automatically be moved to the closest
   *   edge of #confine_to and enter and leave events will be
   *   generated as necessary.
   * @param cursor The cursor to display while the grab is
   *   active. If this is %NULL then the normal cursors are used
   *   for #window and its descendants, and the cursor for
   *   #window is used for all other windows.
   * @param time_ The timestamp of the event which led to this
   *   pointer grab. This usually comes from a {@link EventButton}
   *   struct, though %GDK_CURRENT_TIME can be used if the time
   *   isn’t known.
   * @returns %GDK_GRAB_SUCCESS if the grab was successful.
   */
   ...
   *
   * Note that this does not take the inmplicit pointer grab on
  ...
  /**
   * Ungrabs the pointer on the default display, if it is grabbed
   * by this application.
   *
   * @param time_ A timestamp from a {@link Event}, or
   *   %GDK_CURRENT_TIME if no timestamp is available.
  ...
  /**
   * Prepare for parsing command line arguments for GDK. This is
   * not public API and should not be used in application code.
   */
   ...
   * @param mode A value describing how the new data is to be
   *   combined with the current data.
   * @param data The data (a `guchar *` `gushort *`, or `gulong
   *   *`, depending on #format), cast to a `guchar *`.
   * @param nelements The number of elements of size determined
   *   by the format, contained in #data.
   */
   ...
   * undefined), and also prints warnings to stderr in cases
   * where it should return a useful error to the program. You
   * are advised to use XGetWindowProperty() directly until a
   * replacement function for gdk_property_get() is provided.
   *
   * @param window A {@link Window}
   * @param property The property to retrieve
   * @param type The desired property type, or %GDK_NONE, if any
   *   type of data is acceptable. If this does not match the
   *   actual type, then #actual_format and #actual_length will
   *   be filled in, a warning will be printed to stderr and no
   *   data will be returned.
   * @param offset The offset into the property at which to begin
   *   retrieving data, in 4 byte units.
   * @param length The length of the data to retrieve in bytes.
   *   Data is considered to be retrieved in 4 byte chunks, so
   *   #length will be rounded up to the next highest 4 byte
   *   boundary (so be careful not to pass a value that might
   *   overflow when rounded up).
   * @param pdelete If %TRUE, delete the property after
   *   retrieving the data.
   ...
   *
   *   Location to store the actual return format of the data;
   *   either 8, 16 or 32 bits.
   *
   *   Location to store the length of the retrieved data, in
   *   bytes. Data returned in the 32 bit format is stored in a
   *   long variable, so the actual number of 32 bit elements
   *   should be be calculated via #actual_length / sizeof(glong)
   *   to ensure portability to 64 bit systems.
   *
   *   Location to store a pointer to the data. The retrieved data
   *   should be freed with {@link G.free} when you are finished
   *   using it.
   */
  function property_get(
    ...
    length: number,
    pdelete: number
  ): [boolean, Atom, number, number, number[]];

  /**
   * This function returns the available bit depths for the
   * default screen. It’s equivalent to listing the visuals
   * {@link (gdk.list_visuals}) and then looking at the depth
  ...
  /**
   * This function returns the available visual types for the
   * default screen. It’s equivalent to listing the visuals
   * {@link (gdk.list_visuals}) and then looking at the type field
   * in each visual, removing duplicates.
   *
   * The array returned by this function should not be freed.
   *
   ...
   * @param selection An atom identifying the selection to get
   *   the contents of.
   * @param target The form in which to retrieve the selection.
   * @param time_ The timestamp to use when retrieving the
   *   selection. The selection owner may refuse the request if
   *   it did not own the selection at the time indicated by the
   *   timestamp.
   */
   ...
   * @returns If there is a selection owner for this window, and
   *   it is a window known to the current process, the
   *   {@link Window} that owns the selection, otherwise %NULL.
   *   Note that the return value may be owned by a different
   *   process if a foreign window was previously created for
   *   that window, but a new foreign window will never be
   ...
   * window, but a new foreign window will never be created by
   * this call.
   *
   * @param display A {@link Display}
   * @param selection An atom indentifying a selection
   ...
   * @param time_ Timestamp to use when setting the selection. If
   *   this is older than the timestamp given last time the owner
   *   was set for the given selection, the request will be
   *   ignored.
   * @param send_event If %TRUE, and the new owner is different
   *   from the current owner, the current owner will be sent a
  ...
  /**
   * Sets the {@link Window} #owner as the current owner of the
   * selection #selection.
   *
   * @param display The {@link Display}
   * @param owner A {@link Window} or %NULL to indicate that the
   ...
   * data in response to a call to {@link Gdk.selection.convert}.
   * This function will not be used by applications, who should
   * use the #GtkClipboard API instead.
   *
   * @param requestor The window on which the data is stored
   * @param data Location to store a pointer to the retrieved
   *   data. If the retrieval failed, %NULL we be stored here,
   *   otherwise, it will be non-%NULL and the returned data
   *   should be freed with {@link G.free} when you are finished
   *   using it. The length of the allocated memory is one more
   *   than the length of the returned data, and the final byte
   *   will always be zero, to ensure nul-termination of strings
   * @param prop_type Location to store the type of the property
   * @param prop_format Location to store the format of the
   *   property
   * @returns The length of the retrieved data.
   */
  function selection_property_get(
    requestor: Window,
    data: number,
    prop_type: Atom,
    prop_format: number
   ...
   * @param property Property in which the selection owner stored
   *   the data, or %GDK_NONE to indicate that the request was
   *   rejected.
   ...
   *
   * @param display The {@link Display} where #requestor is
   *   realized
   * @param requestor Window to which to deliver response
   * @param selection Selection that was requested
   * @param target Target that was selected
   ...
   *
   * This can be be useful if your application does not work with
   * certain GDK backends.
   *
   ...
   * |[<!-- language="C" -->
   *
   * Gdk_set_allowed_backends ("wayland,quartz,*"); ]| instructs
   * GDK to try the Wayland backend first, followed by the Quartz
   * backend, and then all others.
   *
   * If the `GDK_BACKEND` environment variable is set, it
   * determines what backends are tried in what order, while
   * still respecting the set of allowed backends that are
   * specified by this function.
   *
   * The possible backend names are x11, win32, quartz, broadway,
   * wayland. You can also include a * in the list to try all
   * remaining backends.
   *
   * This call must happen prior to {@link Gdk.Display.open},
   * gtk_init(), gtk_init_with_args() or gtk_init_check() in
   ...
   * {@link Gdk.Display.set_double_click_time}. See also
   * gdk_display_set_double_click_distance(). Applications should
  ...
  /**
   * Sets the program class. The X11 backend uses the program
   * class to set the class name part of the `WM_CLASS` property
   * on toplevel windows; see the ICCCM.
   *
   * The program class can still be overridden with the --class
   * command line option.
   *
  ...
  /**
   * Sets whether a trace of received events is output. Note that
   * GTK+ must be compiled with debugging (that is, configured
   * using the `--enable-debug` option) to use this option.
   *
   * @param show_events %TRUE to output event debugging
   *   information.
   */
  function set_show_events(show_events: boolean): void;

  /**
   * Obtains a desktop-wide setting, such as the double-click
   * time, for the default screen. See
   * {@link Gdk.Screen.get_setting}.
   *
   * @param name The name of the setting.
   * @param value Location to store the value of the setting.
  ...
  /**
   * Retrieves a pixel from #window to force the windowing system
   * to carry out any pending rendering commands.
   *
   * This function is intended to be used to synchronize with
   * rendering pipelines, to benchmark windowing system rendering
   * operations.
   *
   ...
   * It will warp the mouse pointer to the given (#x,#y)
   * coordinates within #window and simulate a button press or
   * release event. Because the mouse pointer needs to be warped
   * to the target location, use of this function outside of test
   * programs that run in their own virtual windowing system
   * (e.g. Xvfb) is not recommended.
   *
   * Also, {@link Gdk.test.simulate_button} is a fairly low level
   * function, for most testing purposes, gtk_test_widget_click()
   * is the right function to call which will generate a button
   * press event followed by its accompanying button release
   * event.
   *
   ...
   * @param button Number of the pointer button for the event,
   *   usually 1, 2 or 3
   * @param modifiers Keyboard modifiers the event is setup with
   * @param button_pressrelease Either %GDK_BUTTON_PRESS or
   *   %GDK_BUTTON_RELEASE
   * @returns Whether all actions necessary for a button event
   *   simulation were carried out successfully
   */
   ...
   *
   * Also, {@link Gdk.test.simulate_key} is a fairly low level
   * function, for most testing purposes,
   * gtk_test_widget_send_key() is the right function to call
   * which will generate a key press event followed by its
   * accompanying key release event.
   *
   ...
   * @param modifiers Keyboard modifiers the event is setup with
   * @param key_pressrelease Either %GDK_KEY_PRESS or
   *   %GDK_KEY_RELEASE
   * @returns Whether all actions necessary for a key event
   *   simulation were carried out successfully
   */
  ...
  /**
   * Converts a text property in the given encoding to a list of
   * UTF-8 strings.
   *
   * @param display A {@link Display}
   * @param encoding An atom representing the encoding of the
   *   text
   ...
   *
   *   Location to store the list of strings or %NULL. The list
   *   should be freed with {@link G.strfreev}.
   */
   ...
   * {@link Gdk.threads.add_idle_full} assigning the default
   ...
   * priority events pending. If the function returns %FALSE it
   * is automatically removed from the list of event sources and
   * will not be called again.
   *
   * This variant of {@link G.idle_add_full} calls #function with
   * the GDK lock held. It can be thought of a MT-safe version
   * for GTK+ widgets for the following use case, where you have
   * to worry about idle_callback() running in thread A and
   ...
   * // using g_idle_add() here would require thread protection
   * in the callback }
   *
   * Static void some_widget_finalize (GObject *object) {
   * SomeWidget *self = SOME_WIDGET (object); if (self->idle_id)
   * g_source_remove (self->idle_id); G_OBJECT_CLASS
   * (parent_class)->finalize (object); } ]|
   *
   * @param priority The priority of the idle source. Typically
   *   this will be in the range between #G_PRIORITY_DEFAULT_IDLE
   *   and #G_PRIORITY_HIGH_IDLE
   * @param _function Function to call
   * @param data Data to pass to #function
   * @param notify Function to call when the idle is removed, or
   *   %NULL
   * @returns The ID (greater than 0) of the event source.
   */
  ...
  /**
   * A wrapper for the common usage of
   * {@link Gdk.threads.add_timeout_full} assigning the default
  ...
  /**
   * Sets a function to be called at regular intervals holding
   * the GDK lock, with the given priority. The function is
   * called repeatedly until it returns %FALSE, at which point
   * the timeout is automatically destroyed and the function will
   * not be called again. The #notify function is called when the
   * timeout is destroyed. The first call to the function will be
   ...
   * on the current time and the given interval (it does not try
   * to “catch up” time lost in delays).
   *
   * This variant of {@link G.timeout_add_full} can be thought of
   * a MT-safe version for GTK+ widgets for the following use
   * case:
   *
   * |[<!-- language="C" -->
   *
  ...
  /**
   * A wrapper for the common usage of
   * {@link Gdk.threads.add_timeout_seconds_full} assigning the
  ...
  /**
   * A variant of {@link Gdk.threads.add_timeout_full} with
   * second-granularity. See g_timeout_add_seconds_full() for a
   * discussion of why it is a good idea to use this function if
   * you don’t need finer granularity.
   *
   * @param priority The priority of the timeout source.
   *   Typically this will be in the range between
   *   #G_PRIORITY_DEFAULT_IDLE and #G_PRIORITY_HIGH_IDLE.
  ...
  /**
   * This function marks the beginning of a critical section in
   * which GDK and GTK+ functions can be called safely and
   * without causing race conditions. Only one thread at a time
   * can be in such a critial section.
   */
  function threads_enter(): void;

  /**
   * Initializes GDK so that it can be used from multiple threads
   * in conjunction with {@link Gdk.threads.enter} and
   * gdk_threads_leave().
   *
   ...
   * {@link Gdk.threads.enter}, and released by
   * gdk_threads_leave(); using this function an application
   * provides, instead, a function #enter_fn that is called by
   * gdk_threads_enter() and a function #leave_fn that is called
   * by gdk_threads_leave().
   *
   * The functions must provide at least same locking
   * functionality as the default implementation, but can also do
   * extra application specific processing.
   *
   ...
   * main loop, the application must temporarily release its lock
   * as well.
   *
   * Most threaded GTK+ apps won’t need to use this method.
   *
   * This method must be called before gdk_threads_init(), and
   * cannot be called multiple times.
   *
   * @param enter_fn Function called to guard GDK
   * @param leave_fn Function called to release the guard
   */
  function threads_set_lock_functions(
  ...
  /**
   * Converts an UTF-8 string into the best possible
   * representation as a STRING. The representation of characters
   * not in STRING is not specified; it may be as pseudo-escape
   * sequences \x{ABCD}, or it may be in some other form of
   * approximation.
   *
  ...
  /**
   * The primary button. This is typically the left mouse button,
   * or the right button in a left-handed setup.
   *
   * @returns The primary button. This is typically the left
   *   mouse button, or the right button in a left-handed setup.
   */
  const BUTTON_PRIMARY: number;

  /**
   * The secondary button. This is typically the right mouse
   * button, or the left button in a left-handed setup.
   *
   * @returns The secondary button. This is typically the right
   *   mouse button, or the left button in a left-handed setup.
   */
  const BUTTON_SECONDARY: number;

  /**
   * Represents the current time, and can be used anywhere a time
   * is expected.
   *
   * @returns Represents the current time, and can be used
   *   anywhere a time is expected.
   */
  const CURRENT_TIME: number;

  /**
   * Use this macro as the return value for continuing the
   * propagation of an event handler.
   *
   * @returns Use this macro as the return value for continuing
   *   the propagation of an event handler.
   */
  const EVENT_PROPAGATE: boolean;

  /**
   * Use this macro as the return value for stopping the
   * propagation of an event handler.
   *
   * @returns Use this macro as the return value for stopping the
   *   propagation of an event handler.
   */
  ...
  /**
   * A special value, indicating that the background for a window
   * should be inherited from the parent window.
   *
   * @returns A special value, indicating that the background for
   *   a window should be inherited from the parent window.
   */
  const PARENT_RELATIVE: number;

  /**
   * This is the priority that the idle handler processing window
   * updates is given in the [GLib Main
   * Loop][glib-The-Main-Event-Loop].
   *
   * @returns This is the priority that the idle handler
   *   processing window updates is given in the [GLib Main
   *   Loop][glib-The-Main-Event-Loop].
   */
