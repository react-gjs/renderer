...
declare module "gi://Pango" {
  namespace Pango {
    ...
    interface IContext {
       ...
       * context.
       *
       * @returns The font map for the `PangoContext`. This value is
       ...
       * which [func#Pango.Gravity.get_for_matrix] is used to return
       ...
       * different scripts, a language tag can be provided to indicate
       * that the metrics should be retrieved that correspond to the
       * script(s) used by that language.
       *
       * The `PangoFontDescription` is interpreted in the same way as
       * by [func#itemize], and the family name may be a comma
       * separated list of names. If characters from multiple of these
       * families would be used to render the string, then the
       * returned fonts would be a composite of the metrics for the
       * fonts loaded for the individual families.
       *
       * @param desc A `PangoFontDescription` structure. %NULL means
       *   that the font description from the context will be used.
       * @param language Language tag used to determine which script
       *   to get the metrics for. %NULL means that the language tag
       *   from the context will be used. If no language tag is set on
       *   the context, metrics for the default language (as
       *   determined by [func#Pango.Language.get_default] will be
       *   returned.
      ...
      /**
       * Returns whether font rendering with this context should round
       * glyph positions and widths.
       *
       * @returns
       */
       ...
       * changed. The serial may wrap, but will never have the value
       * 0. Since it can wrap, never compare it with "less than",
       * always use "not equals".
       *
       * This can be used to automatically detect changes to a
       * `PangoContext`, and is only useful when implementing objects
       * that need update when their `PangoContext` changes, like
       * `PangoLayout`.
       *
       * @returns The current serial number of #context.
       */
      ...
      /**
       * Loads the font in one of the fontmaps in the context that is
       * the closest match for #desc.
       *
       * @param desc A `PangoFontDescription` describing the font to
       *   load
       * @returns The newly allocated `PangoFont` that was loaded, or
       ...
       *
       * @param desc A `PangoFontDescription` describing the fonts to
       *   load
       * @param language A `PangoLanguage` the fonts will be used for
       * @returns The newly allocated `PangoFontset` loaded, or %NULL
       *   if no font matched.
       */
       ...
       * %PANGO_DIRECTION_LTR or %PANGO_DIRECTION_RTL, then the value
       * will be used as the paragraph direction in the Unicode
       * bidirectional algorithm. A value of %PANGO_DIRECTION_WEAK_LTR
       * or %PANGO_DIRECTION_WEAK_RTL is used only for paragraphs that
       * do not contain any strong characters themselves.
       *
      ...
      /**
       * Sets the font map to be searched when fonts are looked-up in
       * this context.
       *
       ...
       *
       * The gravity hint is used in laying vertical text out, and is
       * only relevant if gravity of the context as returned by
       ...
       * rendering with this context.
       *
       * Note that reported metrics are in the user space coordinates
       * before the application of the matrix, not device-space
       * coordinates after the application of the matrix. So, they
       * don't scale with the matrix, though they may change slightly
       * for different matrices, depending on how the text is fit to
       * the pixel grid.
       *
       * @param matrix A `PangoMatrix`, or %NULL to unset any existing
       *   matrix. (No matrix set is the same as setting the identity
       *   matrix.)
       */
      set_matrix(matrix: Matrix | null): void;
      /**
       * Sets whether font rendering with this context should round
       * glyph positions and widths to integral positions, in device
       * units.
       *
     ...
     * itemization process.
     *
     * The information stored by `PangoContext` includes the fontmap
    ...
    class Context {
       ...
       * be followed by a [method#Pango.Context.set_font_map] call,
       * and the function [method#Pango.FontMap.create_context] does
       * these two steps together and hence users are recommended to
       * use that.
       *
       * If you are using Pango as part of a higher-level system, that
       * system may have it's own way of create a `PangoContext`. For
       * instance, the GTK toolkit has, among others,
       * {@link `gtk.widget_get_pango_context}`. Use those instead.
       *
       * @returns The newly allocated `PangoContext`, which should be
       *   freed with {@link GObject.unref}.
       */
    ...
    interface ICoverage {
       ...
       *
       * If the result is zero, free the coverage and all associated
       * memory.
       */
      unref(): void;
    }
     ...
     * font can represent a particular character, and also how well it
     * can represent that character. The `PangoCoverage` is a data
    ...
    class Coverage {
       ...
       *
       * @returns The newly allocated `PangoCoverage`, initialized to
       *   %PANGO_COVERAGE_NONE with a reference count of one, which
    ...
    interface IFont {
       ...
       * set in device units.
       *
       * Use [method#Pango.Font.describe] if you want the font size in
       * points.
       *
       * @returns A newly-allocated `PangoFontDescription` object.
       */
       ...
       *
       * Note that the font maintains a _weak_ reference to the font
       * map, so if all references to font map are dropped, the font
       * map will be finalized even if there are fonts created with
       * the font map that are still alive. In that case this function
       * will return %NULL.
       *
       * It is the responsibility of the user to ensure that the font
       * map is kept alive. In most uses this is not an issue as a
       * `PangoContext` holds a reference to the font map.
       *
       ...
       * increasing coordinates extending to the right and down. The
       * macros {@link PANGO.ASCENT}, PANGO_DESCENT(),
       * PANGO_LBEARING(), and PANGO_RBEARING() can be used to convert
       * from the extents rectangle to more traditional font metrics.
       * The units of the rectangles are in 1/PANGO_SCALE of a device
       * unit.
       *
       ...
       * @returns Rectangle used to store the extents of the glyph as
       *   drawn
       *
       ...
       *
       * @param language Language tag used to determine which script
       *   to get the metrics for, or %NULL to indicate to get the
    ...
    class Font {
       ...
       *
       * @param descs A pointer to an array of `PangoFontDescription`,
       *   may be %NULL
    ...
    interface IFontFace {
       ...
       * `PangoFontFace`. The size field of the resulting font
       ...
       *
       * @returns The face name for the face. This string is owned by
       *   the face object and must not be modified or freed.
       */
       ...
       * underlying font rendering engine from another face, perhaps
       * by shearing, emboldening, or lightening it.
       *
       ...
       * stores %NULL at the location pointed to by #sizes and 0 at
       * the location pointed to by #n_sizes. The sizes returned are
       * in Pango units and are sorted in ascending order.
       *
       * @returns Location to store a pointer to an array of int. This
       *   array should be freed with {@link G.free}.
       *
    ...
    interface IFontFamily {
       ...
       *   family's default face (fontconfig calls it "Regular") will
       *   be returned.
       ...
       *
       * The name is unique among all fonts for the font backend and
       * can be used in a `PangoFontDescription` to specify that a
       * face from this family is desired.
       *
       * @returns The name of the family. This string is owned by the
       *   family object and must not be modified or freed.
       */
      get_name(): string;
      /**
       ...
       *
       * For Western languages this would mean that the advance width
       * of all characters are the same, but this categorization also
       * includes Asian fonts which include double-width characters:
       * characters that occupy two grid cells.
       * {@link G.unichar_iswide} returns a result that indicates
       * whether a character is typically double-width in a monospace
       * font.
       *
       * The best way to find out the grid-cell size is to call
       * [method#Pango.FontMetrics.get_approximate_digit_width], since
       * the results of
       * [method#Pango.FontMetrics.get_approximate_char_width] may be
    ...
    interface IFontMap {
       ...
       * whenever the fontmap is changed. It may wrap, but will never
       * have the value 0. Since it can wrap, never compare it with
       * "less than", always use "not equals".
       *
       * The fontmap can only be changed using backend-specific API,
       * like changing fontmap resolution.
       *
       * This can be used to automatically detect changes to a
       ...
       * @param desc A `PangoFontDescription` describing the font to
       *   load
       * @returns The newly allocated `PangoFont` loaded, or %NULL if
       *   no font matched.
       */
    ...
    interface IFontset {
      ...
      /**
       * Returns the font in the fontset that contains the best glyph
     ...
     * `PangoFontDescription` against a particular `PangoContext`. It
     * has operations for finding the component font for a particular
     ...
     *
     * When creating a `PangoFontsetSimple`, you have to provide the
    ...
    interface ILayout {
       ...
       * of the strong and weak cursors if the insertion point is at
       * that index.
       *
       * The position of each cursor is stored as a zero-width
       * rectangle. The strong cursor location is the location where
       * characters of the directionality equal to the base direction
       * of the layout are inserted. The weak cursor location is the
       * location where characters of the directionality opposite to
       ...
       * things. Note that both extents may have non-zero x and y. You
       * may want to use those to offset where you render the layout.
       * Not doing that is a very typical bug that shows up as
       * right-to-left layouts not being correctly positioned in a
       * layout with a set width.
       *
       * The extents are given in layout coordinates and in Pango
       * units; layout coordinates begin at the top left corner of the
       * layout.
       *
       * @returns Rectangle used to store the extents of the layout as
       *   drawn
       *
       ...
       *
       * @returns A pointer to the layout's font description, or %NULL
       *   if the font description from the layout's context is
       *   inherited.
       */
       ...
       *
       * @returns The height, in Pango units if positive, or number of
       *   lines if negative.
       */
      get_height(): number;
      /**
      ...
      get_indent(): number;
      /**
       * Returns an iterator to iterate over the visual extents of the
       * layout.
       *
       ...
       *
       * Use the faster [method#Pango.Layout.get_line_readonly] if you
       * do not plan to modify the contents of the line (glyphs, glyph
       * widths, etc.).
       *
       * @param line The index of a line, which must be between 0 and
       *   `pango_layout_get_line_count(layout) - 1`, inclusive.
       * @returns The requested `PangoLayoutLine`, or %NULL if the
       *   index is out of range. This layout line can be ref'ed and
       *   retained, but will become invalid if changes are made to
       *   the `PangoLayout`.
       */
       ...
       * [method#Pango.Layout.get_line], but the user is not expected
       ...
       *   index is out of range. This layout line can be ref'ed and
       *   retained, but will become invalid if changes are made to
       *   the `PangoLayout`. No changes should be made to the line.
       */
       ...
       * you do not plan to modify the contents of the lines (glyphs,
       * glyph widths, etc.).
       *
       * @returns A `GSList` containing the lines in the layout. This
       *   points to internal data of the `PangoLayout` and must be
       *   used with care. It will become invalid on any change to the
       *   layout's text or properties.
       */
       ...
       *
       * This is a faster alternative to
       * [method#Pango.Layout.get_lines], but the user is not expected
       * to modify the contents of the lines (glyphs, glyph widths,
       * etc.).
       *
       * @returns A `GSList` containing the lines in the layout. This
       *   points to internal data of the `PangoLayout` and must be
       *   used with care. It will become invalid on any change to the
       *   layout's text or properties. No changes should be made to
       *   the lines.
       */
      get_lines_readonly(): GLib.SList;
      /**
       * Retrieves an array of logical attributes for each character
       * in the #layout.
       *
       * @returns Location to store a pointer to an array of logical
       *   attributes. This value must be freed with {@link G.free}.
       *
       *   Location to store the number of the attributes in the array.
       *   (The stored value will be one more than the total number of
       *   characters in the layout, since there need to be attributes
       *   corresponding to both the position before the first
       ...
       * more than the total number of characters in the layout, since
       * there need to be attributes corresponding to both the
       * position before the first character and the position after
       * the last character.
       *
       ...
       * #ink_rect and #logical_rect such that the rounded rectangles
       * fully contain the unrounded one (that is, passes them as
       * first argument to [func#Pango.extents_to_pixels]).
       *
       * @returns Rectangle used to store the extents of the layout as
       *   drawn
       *
      ...
      /**
       * Determines the logical width and height of a `PangoLayout` in
       * device units.
       *
       ...
       * can wrap, never compare it with "less than", always use "not
       * equals".
       *
       * This can be used to automatically detect changes to a
       * `PangoLayout`, and is useful for example to decide whether a
       * layout needs redrawing. To force the serial to be increased,
       * use [method#Pango.Layout.context_changed].
       *
       * @returns The current serial number of #layout.
       */
      get_serial(): number;
       ...
       * in use and %NULL is returned. Default tabs are every 8
       * spaces.
       *
       ...
       * available to render all characters in a certain string, or
       * when used in combination with %PANGO_ATTR_FALLBACK, to check
       * if a certain font supports all the characters in the string.
       *
      ...
      get_wrap(): WrapMode;
      /**
       * Converts from byte #index_ within the #layout to line and X
       * position.
       *
       ...
       *   grapheme to retrieve the position of. If > 0, the trailing
       *   edge of the grapheme, if 0, the leading of the grapheme
       ...
       * `pos->x + pos->width` the trailing edge of the grapheme. If
       ...
       *
       * This returns %TRUE if the ellipsization mode for #layout is
       * not %PANGO_ELLIPSIZE_NONE, a positive width is set on
       * #layout, and there are paragraphs exceeding that width that
       * have to be ellipsized.
       *
       ...
       * %PANGO_ELLIPSIZE_NONE, and there are paragraphs exceeding the
       ...
       * position will be one position to the right of the old cursor
       * position. If #direction is negative, then the new strong
       * cursor position will be one position to the left of the old
       * cursor position.
       *
       * In the presence of bidirectional text, the correspondence
       * between logical and visual order will depend on the direction
       * of the current run, and there may be jumps when the cursor is
       * moved off of the end of a run.
       *
       * Motion here is in cursor positions, not in characters, so a
       * single call to [method#Pango.Layout.move_cursor_visually] may
       * move the cursor over multiple characters when multiple
       * characters combine to form a single grapheme.
       *
       * @param strong Whether the moving cursor is the strong cursor
       *   or the weak cursor. The strong cursor is the cursor
       *   corresponding to text insertion in the base direction for
       *   the layout.
       * @param old_index The byte index of the grapheme for the old
       *   index
       * @param old_trailing If 0, the cursor was at the leading edge
       *   of the grapheme indicated by #old_index, if > 0, the cursor
       *   was at the trailing edge.
       * @param direction Direction to move cursor. A negative value
       *   indicates motion to the left
       * @returns Location to store the new cursor byte index. A value
       *   of -1 indicates that the cursor has been moved off the
       *   beginning of the layout. A value of %G_MAXINT indicates
       ...
       *   cursor should be displayed. This allows distinguishing the
       *   position at the beginning of one line from the position at
       *   the end of the preceding line. #new_index is always on the
       *   line where the cursor should be displayed.
       */
      ...
      /**
       * Sets whether to calculate the base direction for the layout
       * according to its contents.
       *
       * When this flag is on (the default), then paragraphs in
       * #layout that begin with strong right-to-left characters
       * (Arabic and Hebrew principally), will have right-to-left
       * layout, paragraphs with letters from other scripts will have
       * left-to-right layout. Paragraphs with only neutral characters
       * get their direction from the surrounding paragraphs.
       *
       * When %FALSE, the choice between left-to-right and
       * right-to-left layout is done according to the base direction
       * of the layout's `PangoContext`. (See
       * [method#Pango.Context.set_base_dir]).
       *
       * When the auto-computed direction of a paragraph differs from
       * the base direction of the context, the interpretation of
       ...
       *
       * If the layout contains characters such as newlines that force
       * it to be layed out in multiple paragraphs, then whether each
       * paragraph is ellipsized separately or the entire layout is
       * ellipsized as a whole depends on the set height of the
       * layout.
       *
       ...
       * layout. Only lines would be shown that would fit, and if
       * there is any text omitted, an ellipsis added. At least one
       * line is included in each paragraph regardless of how small
       * the height value is. A value of zero will render exactly one
       * line for the entire layout.
       *
       * If #height is negative, it will be the (negative of) maximum
       * number of lines per paragraph. That is, the total number of
       * lines shown may well be more than this value if the layout
       * contains multiple paragraphs of text. The default value of -1
       * means that the first line of each paragraph is ellipsized.
       * This behavior may be changed in the future to act per layout
       ...
       *
       * @param height The desired height of the layout in Pango units
       *   if positive, or desired number of lines if negative.
       */
       ...
       * width, and subsequent lines will be indented by the absolute
       * value of #indent.
       *
       ...
       * some scripts (such as Arabic), the justification may be done
       * in more complex ways, like extending the characters.
       *
       * Note that this setting is not implemented and so is ignored
       * in Pango older than 1.18.
       *
       ...
       *
       * This is the same as
       * [method#Pango.Layout.set_markup_with_accel], but the markup
       ...
       * characters as paragraph separators; instead, keep all text in
       * a single paragraph, and display a glyph for paragraph
       * separator characters. Used when you want to allow editing of
       ...
       *
       * This function validates #text and renders invalid UTF-8 with
       * a placeholder glyph.
       *
       * Note that if you have used [method#Pango.Layout.set_markup]
       * or [method#Pango.Layout.set_markup_with_accel] on #layout
       * before, you may want to call
       * [method#Pango.Layout.set_attributes] to clear the attributes
       * set on the layout from the markup as this function does not
       * clear attributes.
       *
       * @param text The text
       * @param length Maximum length of #text, in bytes. -1 indicates
       *   that the string is nul-terminated and the length should be
       *   calculated. The text will also be truncated on encountering
       *   a nul-termination even when #length is positive.
       */
       ...
       * position is chosen (the position will be clamped inside the
       * layout). If the X position is not within the layout, then the
       * start or the end of the line is chosen as described for
       * [method#Pango.LayoutLine.x_to_index]. If either the X or Y
       * positions were not inside the layout, then the function
       * returns %FALSE; on an exact hit, it returns %TRUE.
       *
       * @param x The X offset (in Pango units) from the left edge of
       *   the layout
       * @param y The Y offset (in Pango units) from the top edge of
       *   the layout
       ...
       *
       *   Location to store a integer indicating where in the grapheme
       *   the user clicked. It will either be zero, or the number of
       *   characters in the grapheme. 0 represents the leading edge
       *   of the grapheme.
       */
     ...
     * fairly large amount of code. `PangoLayout` provides a
     * high-level driver for formatting entire paragraphs of text at
     * once. This includes paragraph-level functionality such as line
    ...
    class Layout {
      ...
      /**
       * Create a new `PangoLayout` object with attributes initialized
       * to default values for a particular `PangoContext`.
       *
    ...
    interface IRenderer {
       ...
       * drawing. Calls such as [method#Pango.Renderer.draw_layout]
       ...
       * spelling error.
       *
       * The width of the underline is rounded to an integer number of
       * up/down segments and the resulting rectangle is centered in
       ...
       * automatically according to those attributes, you need to use
       * {@link Pango.Renderer.draw_layout_line} or
       * pango_renderer_draw_layout().
       *
       * Note that #text is the start of the text for layout, which is
       * then indexed by `glyph_item->item->offset`.
       *
       * If #text is %NULL, this simply calls
       * [method#Pango.Renderer.draw_glyphs].
       *
       * The default implementation of this method simply falls back
       * to [method#Pango.Renderer.draw_glyphs].
       *
       * @param text The UTF-8 text that #glyph_item refers to
       * @param glyph_item A `PangoGlyphItem`
       * @param x X position of left edge of baseline, in user space
       *   coordinates in Pango units
       * @param y Y position of left edge of baseline, in user space
       *   coordinates in Pango units
       */
      draw_glyph_item(
        text: string | null,
       ...
       * shapes, backgrounds and lines that are specified by the
       ...
       * @param x X position at which to draw rectangle, in user space
       *   coordinates in Pango units
       * @param y Y position at which to draw rectangle, in user space
       *   coordinates in Pango units
      ...
      /**
       * Draws a trapezoid with the parallel sides aligned with the X
       * axis using the given `PangoRenderer`; coordinates are in
       * device space.
       *
       ...
       * @returns The alpha for the specified part, or 0 if it hasn't
       ...
       * rendered.
       *
       * @returns The layout, or %NULL if no layout is being rendered
       *   using #renderer at this time.
       */
       ...
       * example.
       *
       * The returned layout line should not be modified while still
       * being rendered.
       *
       ...
       * #part has changed.
       *
       * This should be called if the rendering changes in a way that
       * would prevent multiple pieces being joined together into one
       * drawing call. For instance, if a subclass of `PangoRenderer`
       * was to add a stipple option for drawing underlines, it needs
       * to call
       *
       ...
       * stipples might be joined together. Pango automatically calls
       * this for changes to colors. (See
       ...
       *
       * Note that the alpha may only be used if a color is specified
       * for #part as well.
       *
     ...
     *
     * By subclassing `PangoRenderer` and overriding operations such
     * as #draw_glyphs and #draw_rectangle, renderers for particular
    ...
    /**
     * The `PangoAttrClass` structure stores the type and operations
    ...
    /**
     * The `PangoAttrColor` structure is used to represent attributes
     * that are colors.
     */
    interface AttrColor {}
    class AttrColor {
    ...
    /**
     * The `PangoAttrFloat` structure is used to represent attributes
    ...
    /**
     * The `PangoAttrFontDesc` structure is used to store an attribute
    ...
    class AttrIterator {
       ...
       *   should be freed with [method#Pango.AttrIterator.destroy]
       */
       ...
       * attribute whose range starts closest to the current location
       * is used.
       *
       * @param type The type of attribute to find
       * @returns The current attribute of the given type, or %NULL if
       *   no attribute of that type applies to the current location.
       */
       ...
       * iterator.
       *
       * @returns A list of all attributes for the current range. To
       *   free this value, call [method#Pango.Attribute.destroy] on
       *   each value and {@link G.slist_free} on the list.
       */
       ...
       *   current values. The family name in this structure will be
       *   set using [method#Pango.FontDescription.set_family_static]
       *   using values from an attribute in the `PangoAttrList`
       *   associated with the iterator, so if you plan to keep it
       *   around, you must call: `pango_font_description_set_family
       *   (desc, pango_font_description_get_family (desc))`.
       * @returns Location to store language tag for item, or %NULL if
       *   none is found.
       *
       *   Location in which to store a list of non-font attributes at
       *   the the current position; only the highest priority value
       *   of each attribute will be added to this list. In order to
       *   free this value, you must call
       ...
       * oversight, stored return values that wouldn't fit into a
       * signed integer are clamped to %G_MAXINT.
       *
    ...
    /**
     * A `PangoAttrList` represents a list of attributes that apply to
     * a section of text.
     *
     * The attributes in a `PangoAttrList` are, in general, allowed to
     * overlap in an arbitrary fashion. However, if the attributes are
     * manipulated only through [method#Pango.AttrList.change], the
     * overlap between properties will meet stricter criteria.
     *
     * Since the `PangoAttrList` structure is stored as a linear list,
     * it is not suitable for storing attributes for large amounts of
     * text. In general, you should not use a single `PangoAttrList`
    ...
    class AttrList {
      ...
      /**
       * Create a new empty attribute list with a reference count of
       * one.
       *
       * @returns The newly allocated `PangoAttrList`, which should be
       *   freed with [method#Pango.AttrList.unref]
       */
       ...
       * segment and be merged with any adjoining attributes that are
       * identical.
       *
       * This function is slower than [method#Pango.AttrList.insert]
       * for creating an attribute list in order (potentially much
       * slower for large lists). However,
       ...
       *   reference count of one, which should be freed with
       *   [method#Pango.AttrList.unref]. Returns %NULL if #list was
       *   %NULL.
       */
       ...
       * elements of #list for which #func returns %TRUE and inserts
       * them into a new list.
       *
       * @param func Callback function; returns %TRUE if an attribute
       *   should be filtered out
       * @param data Data to be passed to #func
       * @returns The new `PangoAttrList` or %NULL if no attributes of
       ...
       *   value, call [method#Pango.Attribute.destroy] on each value
       *   and {@link G.slist_free} on the list.
       */
      ...
      /**
       * Increase the reference count of the given attribute list by
       * one.
       *
       ...
       * that applies at position #pos in #list by an amount #len, and
       * then calling [method#Pango.AttrList.change] with a copy of
       * each attribute in #other in sequence (offset in position by
       * #pos).
       *
      ...
      /**
       * Decrease the reference count of the given attribute list by
       * one.
       *
    ...
    /**
     * The `PangoAttrShape` structure is used to represent attributes
    ...
    /**
     * The `PangoAttrSize` structure is used to represent attributes
     * which set font size.
     */
    interface AttrSize {}
    class AttrSize {
    ...
    /**
     * The `PangoAttrString` structure is used to represent attributes
     * with a string value.
     */
    ...
    /**
     * The `PangoAttribute` structure represents the common portions
     * of all attributes.
     *
     * Particular types of attributes include this structure as their
     * initial portion. The common portion of the attribute holds the
     * range to which the value in the type-specific part of the
     * attribute applies and should be initialized using
    ...
    class Attribute {
       ...
       *
       * This compares only the actual value of the two attributes and
       * not the ranges that the attributes apply to.
       *
    ...
    class Color {
       ...
       * otherwise (since colors can just be copied by assignment in
       * C).
       *
       ...
       * hex digits of the red, green, and blue components of the
       * color, respectively. (White in the four forms is `#fff`,
       ...
       * `#rrggbb`, `#rrrgggbbb` or `#rrrrggggbbbb` where `r`, `g` and
       * `b` are hex digits of the red, green, and blue components of
    ...
    class FontDescription {
       ...
       * match for #desc than those of #old_match are, or if
       * #old_match is %NULL, determines if #new_match is a match at
       * all.
       *
       * Approximate matching is done for weight and style; other
       * style attributes must match exactly. Style attributes are all
       * attributes other than family and size-related attributes.
       * Approximate matching for style considers %PANGO_STYLE_OBLIQUE
       * and %PANGO_STYLE_ITALIC as matches, but not as good a match
       ...
       *
       * @returns The newly allocated `PangoFontDescription`, which
       *   should be freed with [method#Pango.FontDescription.free],
       ...
       * allocated fields.
       *
       * This is like [method#Pango.FontDescription.copy], but only a
       * shallow copy is made of the family name and other allocated
       * fields. The result can only be used until #desc is modified
       * or freed. This is meant to be used when the copy is only
       * needed temporarily.
       *
       * @returns The newly allocated `PangoFontDescription`, which
       *   should be freed with [method#Pango.FontDescription.free],
       ...
       *
       * Two font descriptions are considered equal if the fonts they
       * describe are provably identical. This means that their masks
       * do not have to match, as long as other fields are all the
       * same. (Two font descriptions may result in identical fonts
       ...
       *   %NULL if not previously set. This has the same life-time as
       *   the font description itself and should not be freed.
       */
       ...
       *   [method#Pango.FontDescription.get_set_fields] to find out
       ...
       *
       * @returns A bitmask with bits set corresponding to the fields
       *   in #desc that have been set.
       */
       ...
       *   device units. You must call
       *   [method#Pango.FontDescription.get_size_is_absolute] to find
       *   out which is the case. Returns 0 if the size field has not
       *   previously been set or it has been set to 0 explicitly. Use
       *   [method#Pango.FontDescription.get_set_fields] to find out
       *   if the field was explicitly set or not.
       */
       ...
       *   [method#Pango.FontDescription.get_set_fields] to find out
       *   if the size field of the font description was explicitly
       *   set or not.
       */
       ...
       * {@link G.hash_table_new}. The hash value is independent of
       * #desc->mask.
       *
       ...
       * fields in #desc.
       *
       * If #replace_existing is %FALSE, only fields in #desc that are
       * not already set are affected. If %TRUE, then fields that are
       ...
       *   with the corresponding values from #desc_to_merge, even if
       ...
       *
       * This is like [method#Pango.FontDescription.merge], but only a
       * shallow copy is made of the family name and other allocated
       * fields. #desc can only be used until #desc_to_merge is
       * modified or freed. This is meant to be used when the merged
       ...
       * [method#Pango.FontDescription.set_size] which sets the font
       * size in points.
       *
       * @param size The new size, in Pango units. There are
       *   %PANGO_SCALE Pango units in one device unit. For an output
       *   backend where a device unit is a pixel, a #size value of 10
       *   * PANGO_SCALE gives a 10 pixel font.
       */
       ...
       * and will resolve to a particular `PangoFontFamily`. In some
       * uses of `PangoFontDescription`, it is also possible to use a
       ...
       * except that no copy of #family is made. The caller must make
       * sure that the string passed in stays around until #desc has
       * been freed or the name is set again. This function can be
       * used if #family is a static string such as a C string
       ...
       * If #gravity is %PANGO_GRAVITY_AUTO, this actually unsets the
       ...
       *   %PANGO_SCALE. (That is, a #size value of 10 * PANGO_SCALE
       *   is a 10 point font. The conversion factor between points
       *   and device units depends on system configuration and the
       *   output device. For screen display, a logical DPI of 96 is
       *   common, in which case a 10 point font corresponds to a 10 *
       *   (96 / 72) = 13.3 pixel font. Use
       *   [method#Pango.FontDescription.set_absolute_size] if you
       *   need a particular size in device units.
       */
      public set_size(size: number): void;
      /**
       * Sets the stretch field of a font description.
       *
       * The [enum#Pango.Stretch] field specifies how narrow or wide
       * the font should be.
       *
       ...
       *
       * The [enum#Pango.Style] enumeration describes whether the font
       * is slanted and the manner in which it is slanted; it can be
       * either %PANGO_STYLE_NORMAL, %PANGO_STYLE_ITALIC, or
       * %PANGO_STYLE_OBLIQUE.
       *
       * Most fonts will either have a italic style or an oblique
       * style, but not both, and font matching in Pango will match
       * italic specifications with oblique fonts and vice-versa if an
       ...
       * specifying values for a number of axes, such as width or
       * weight.
       *
       ...
       * and each VALUE a floating point number. Unknown axes are
       * ignored, and values are clamped to their allowed range.
       *
       ...
       * Sets the variations field of a font description.
       *
       * This is like [method#Pango.FontDescription.set_variations],
       * except that no copy of #variations is made. The caller must
       * make sure that the string passed in stays around until #desc
       * has been freed or the name is set again. This function can be
       ...
       *
       * The weight field specifies how bold or light the font should
       * be. In addition to the values of the [enum#Pango.Weight]
       ...
       * description of the format of the string representation. The
       * family list in the string description will only have a
     ...
     * documentation of the corresponding getters for documentation of
     * their meaning.
     */
    ...
    class FontMetrics {
       ...
       *
       * This is merely a representative value useful, for example,
       * for determining the initial size for a window. Actual digits
       * in text can be wider or narrower than this, though this value
       ...
       * top of a line of text. (The logical top may be above or below
       * the top of the actual drawn ink. It is necessary to lay out
      ...
      /**
       * Increase the reference count of a font metrics structure by
       * one.
       *
       * @returns #metrics
       */
      ...
      /**
       * Decrease the reference count of a font metrics structure by
       * one. If the result is zero, frees the structure and any
       * associated memory.
       */
      public unref(): void;
    }
     ...
     *
     * As an example of the usage of `PangoGlyphItem`, the results of
     * shaping text with `PangoLayout` is a list of `PangoLayoutLine`,
    ...
    class GlyphItem {
      ...
      /**
       * Splits a shaped item (`PangoGlyphItem`) into multiple items
       ...
       * ([method#Pango.AttrList.filter]), apply the shaping process
       * and then reapply them to the result using this function.
       *
       * All attributes that start or end inside a cluster are applied
       * to that cluster; for instance, if half of a cluster is
       * underlined and the other-half strikethrough, then the cluster
       * will end up with both underline and strikethrough attributes.
       * In these cases, it may happen that #item->extra_attrs for
       * some of the result items can have multiple attributes of the
       * same type.
       *
       ...
       *
       * When multiple characters compose a single cluster, the width
       * of the entire cluster is divided equally among the
       * characters.
       *
      ...
      /**
       * Adds spacing between the graphemes of #glyph_item to give the
       * effect of typographic letter spacing.
       *
       * @param text Text that #glyph_item corresponds to
       *   (glyph_item->item->offset is an offset from the start of
       *   #text)
       * @param log_attrs Logical attributes for the item (the first
       *   logical attribute refers to the position before the first
       ...
       * that used to be in #orig.
       *
       * You can think of #split_index as the length of the returned
       * item. #split_index may not be 0, and it may not be greater
       * than or equal to the length of #orig (that is, there must be
       * at least one byte assigned to each item, you can't create a
       * zero-length item).
       *
     ...
     * #start_char values. If #glyph_item is right-to-left (that is,
     * if `glyph_item->item->analysis.level` is odd), then
     * #start_glyph decreases as the iterator moves forward. Moreover,
     * in right-to-left cases, #start_glyph is greater than
     * #end_glyph.
     *
     ...
     *
     * A common idiom for doing a forward iteration over the clusters
     * is:
     *
     ...
     *
     * Note that #text is the start of the text for layout, which is
     * then indexed by `glyph_item->item->offset` to get to the text
     * of #glyph_item. The #start_index and #end_index values can
     * directly index into #text. The #start_glyph, #end_glyph,
     * #start_char, and #end_char values however are zero-based for
     * the #glyph_item. For each cluster, the item pointed at by the
     * start variables is included in the cluster while the one
    ...
    class GlyphItemIter {
       ...
       * orders.
       *
       * @returns %TRUE if the iterator was moved, %FALSE if we were
       *   already on the first cluster.
       */
    ...
    class GlyphString {
       ...
       *
       * @returns The newly allocated `PangoGlyphString`, which should
       *   be freed with [method#Pango.GlyphString.free].
       */
       ...
       * range (the origin of their coordinate system is at the start
       * of the range, not at the start of the entire glyph string).
       *
      ...
      public free(): void;
      /**
       * Given a `PangoGlyphString` and corresponding text, determine
       ...
       * @param logical_widths An array whose length is the number of
       *   characters in text (equal to `g_utf8_strlen (text, length)`
       *   unless text has `NUL` bytes) to be filled in with the
       *   resulting character widths.
       */
       ...
       * [method#Pango.GlyphString.extents]. However, since this only
       * computes the width, it's much faster. This is in fact only a
       * convenience function that computes the sum of #geometry.width
       ...
       * @param trailing Whether we should compute the result for the
       ...
       *
       * Character positions are computed by dividing up each cluster
       * into equal portions. In scripts where positioning within a
       * cluster is not allowed (such as Thai), the returned value may
       * not be a valid cursor position; the caller must combine the
       * result with the logical attributes for the text to compute
    ...
    /**
     * The `PangoItem` structure stores information about a segment of
     * text.
     *
    ...
    class Item {
       ...
       * using [method#Pango.AttrList.filter], itemize your text, then
       * reapply the attributes to the resulting items using this
       * function.
       *
       * The #iter should be positioned before the range of the item,
       * and will be advanced past it. This function is meant to be
       * called in a loop over the items resulting from itemization,
       * while passing the iter to each call.
       *
       ...
       * than or equal to the length of #orig (that is, there must be
       * at least one byte assigned to each item, you can't create a
       * zero-length item). #split_offset is the length of the first
       * item in chars, and must be provided because the text used to
       * generate the item isn't available, so
       * {@link `pango.item_split}` can't count the char length of the
       * split items itself.
       *
       * @param split_index Byte index of position to split item,
       *   relative to the start of the item
    ...
    class Language {
      ...
      /**
       * Get a string that is representative of the characters needed
       * to render a particular language.
       *
       * The sample text may be a pangram, but is not necessarily. It
       * is chosen to be demonstrative of normal text in the language,
       * as well as exposing font feature requirements unique to the
       * language. It is suitable for use as sample text in a font
       * selection dialog.
       *
       ...
       *
       * If nothing is known about the language tag #language, or if
       * #language is %NULL, then %NULL is returned. The list of
       * scripts returned starts with the script that the language
       * uses most and continues to the one it uses least.
       *
       * The value #num_script points at will be set to the number of
       * scripts in the returned array (or zero if %NULL is
       * returned).
       *
       * Most languages use only one script for writing, but there are
       * some that use two (Latin and Cyrillic for example), and a few
       * use three (Japanese for example). Applications should not
       * make any assumptions on the maximum number of scripts
       * returned though, except that it is positive if the return
       * value is not %NULL, and it is a small number.
       *
       ...
       *
       * @returns An array of `PangoScript` values, with the number of
       *   entries in the array stored in #num_scripts, or %NULL if
       *   Pango does not have any information about this particular
       *   language tag (also the case if #language is %NULL).
       *
       ...
       * far as Pango knows, #script might be used to write
       * #language.
       *
       ...
       * @returns %TRUE if #script is one of the scripts used to write
       *   #language or if nothing is known about #language (including
       *   the case that #language is %NULL), %FALSE otherwise.
       */
       ...
       *
       * A language tag is considered to match a range in the list if
       * the range is '*', the range is exactly the tag, or the range
       * is a prefix of the tag, and the character after it in the tag
       * is '-'.
       *
       * @param range_list A list of language ranges, separated by
       *   ';', ':', ',', or space characters. Each element must
       *   either be '*', or a RFC 3066 language range canonicalized
       *   as by [func#Pango.Language.from_string]
       ...
       * language tag.
       *
       * Returns (transfer none): a string representing the language
       * tag
       *
       * @returns
       */
    ...
    class LayoutIter {
       ...
       * returned by this function will be the same width/height but
       * not at the same x/y as the extents returned from
       ...
       *
       * A line's range includes the line's logical extents. plus half
       * of the spacing above and below the line, if
       ...
       * position with a %NULL run, so this function can return %NULL.
       * The %NULL run at the end of each line ensures that all lines
       * have at least one run, even lines consisting of only a
       * newline.
       *
       * Use the faster [method#Pango.LayoutIter.get_run_readonly] if
       * you do not plan to modify the contents of the run (glyphs,
       * glyph widths, etc.).
       *
    ...
    /**
     * A `PangoLayoutLine` represents one of the lines resulting from
    ...
    class LayoutLine {
       ...
       *
       * See [method#Pango.Font.get_glyph_extents] for details about
       ...
       * #logical_rect such that the rounded rectangles fully contain
       * the unrounded one (that is, passes them as first argument to
       * [func#extents_to_pixels]).
       *
       * @returns Rectangle used to store the extents of the glyph
       *   string as drawn
       *
       ...
       * sorted from left to right. The ranges are with respect to the
       * left edge of the entire layout, not with respect to the
       * line.
       *
       * @param start_index Start byte index of the logical range. If
       *   this value is less than the start index for the line, then
       *   the first range will extend all the way to the leading edge
       *   of the layout. Otherwise, it will start at the leading edge
       *   of the first character.
       * @param end_index Ending byte index of the logical range. If
       *   this value is greater than the end index for the line, then
       *   the last range will extend all the way to the trailing edge
       *   of the layout. Otherwise, it will end at the trailing edge
       *   of the last character.
       * @returns Location to store a pointer to an array of ranges.
       *   The array will be of length `2*n_ranges`, with each range
       *   starting at `(*ranges)[2*n]` and of width `(*ranges)[2*n +
       *   1] - (*ranges)[2*n]`. This array must be freed with
       *   {@link G.free}. The coordinates are relative to the layout
       *   and are in Pango units.
       *
       ...
       * This determination is based on the resolved direction of the
       * paragraph; for example, if the resolved direction is
       ...
       * #index_ pointing to the (logical) last grapheme in the line
       * and #trailing being set to the number of characters in that
       * grapheme. The reverse is true for a left-to-right line.
       *
       * @param x_pos The X offset (in Pango units) from the left edge
       *   of the line.
       * @returns %FALSE if #x_pos was outside the line, %TRUE if
       *   inside
       *
       *   Location to store calculated byte index for the grapheme in
       *   which the user clicked
       *
       *   Location to store an integer indicating where in the grapheme
       *   the user clicked. It will either be zero, or the number of
       *   characters in the grapheme. 0 represents the leading edge
       *   of the grapheme.
       */
    ...
    class LogAttr {
      ...
      /**
       * Is first non-word char after a word Note that in degenerate
       * cases, you could have both #is_word_start and #is_word_end
       ...
       * chars are in some sentence; #is_sentence_boundary denotes the
       ...
       * #is_sentence_end set for some character. (e.g. no space after
       * a period, so the next sentence starts right away)
       */
       ...
       * some languages, the full grapheme (e.g. letter + diacritics)
       * is considered a unit, while in others, each decomposed
       * character in the grapheme is a unit. In the default
       * implementation of [func#break], this bit is set on all
       * grapheme boundaries except those following Latin, Cyrillic or
       ...
       * means that this is not a position in the middle of a word.
       * For example, both sides of a punctuation mark are considered
       * word boundaries. This flag is particularly useful when
       * selecting text word-by-word. This flag implements Unicode's
       * [Word Boundaries](http://www.unicode.org/reports/tr29/)
    ...
    class Matrix {
       ...
       * transformation given by first applying transformation given
       * by #new_matrix then applying the original transformation.
       *
       ...
       * font.
       *
       * That is, the scale factor in the direction perpendicular to
       * the vector that the X coordinate is mapped to. If the scale
       * in the X coordinate is needed as well, use
       ...
       * height of the font.
       *
       * That is, #xscale is the scale factor in the direction of the
       * X coordinate, and #yscale is the scale factor in the
       * direction perpendicular to the vector that the X coordinate
       * is mapped to.
       *
      ...
      /**
       * Changes the transformation represented by #matrix to be the
       * transformation given by first scaling by #sx in the X
       ...
       * are ignored. The calculation of the returned vector is as
       * follows:
       *
       *     dx2 = dx1 * xx + dy1 * xy;
       *     dy2 = dx1 * yx + dy1 * yy;
       ...
       * rotated {@link Layout} to an image buffer, and want to know
       * how large the image should be and how much you should shift
       * the layout when rendering.
       *
       ...
       *
       * If you have the rectangle in Pango units and want to convert
       * to transformed pixel bounding box, it is more accurate to
       * transform it first (using this function) and pass the result
       * to {@link Pango.extents.to_pixels}, first argument, for an
       * inclusive rounded rectangle. However, there are valid reasons
       * that you may want to convert to pixels first and then
       * transform, for example when the transformed coordinates may
       * overflow in Pango units (large matrix translation for
       * example).
       */
      public transform_rectangle(): void;
      /**
       * Changes the transformation represented by #matrix to be the
       * transformation given by first translating by (#tx, #ty) then
     ...
     *
     * `PangoRectangle` is frequently used to represent the logical or
     * ink extents of a single glyph or section of text. (See, for
     * instance, [method#Pango.Font.get_glyph_extents].)
     */
    interface Rectangle {}
    class Rectangle {
    ...
    class ScriptIter {
       ...
       *
       * No copy is made of #text, so the caller needs to make sure it
       * remains valid until the iterator is freed with
       * [method#Pango.ScriptIter.free].
       *
       * @param text A UTF-8 string
       * @param length Length of #text, or -1 if #text is
       *   nul-terminated.
       * @returns The new script iterator, initialized to point at the
       *   first range in the text, which should be freed with
       *   [method#Pango.ScriptIter.free]. If the string is empty, it
       *   will point at an empty range.
       */
       ...
       * points. The range is the set of locations p where *start <= p
       * < *end. (That is, it doesn't include the character stored at
       * *end)
       *
       * Note that while the type of the #script argument is declared
       * as `PangoScript`, as of Pango 1.18, this function simply
       * returns GUnicodeScript values. Callers must be prepared to
       * handle unknown values.
       *
     ...
     * `PangoLayout`. Each tab stop has an alignment and a position.
     */
    interface TabArray {}
    class TabArray {
       ...
       * #positions_in_pixels is %TRUE, otherwise in Pango units. All
       * stops are initially at position 0.
       *
       * @param initial_size Initial number of tab stops to allocate,
       *   can be 0
       * @param positions_in_pixels Whether positions are in pixel
       *   units
       * @returns The newly allocated `PangoTabArray`, which should be
       *   freed with [method#Pango.TabArray.free].
       */
      public static new(
        initial_size: number,
       ...
       *
       * You **must** provide an alignment and position for #size tab
       * stops.
       *
      ...
      public free(): void;
      /**
       * Returns %TRUE if the tab positions are in pixels, %FALSE if
       ...
       * Resizes a tab array.
       *
       * You must subsequently initialize any tabs that were added as
       * a result of growing the array.
       *
    ...
    /**
     * `PangoBidiType` represents the bidirectional character type of
     * a Unicode character as specified by the [Unicode bidirectional
     ...
     * %PANGO_DIRECTION_WEAK_LTR or %PANGO_DIRECTION_WEAK_RTL, since
     * every character is either neutral or has a strong direction; on
     * the other hand %PANGO_DIRECTION_NEUTRAL doesn't make sense to
     * pass to [func#itemize_with_base_dir].
     *
     * The %PANGO_DIRECTION_TTB_LTR, %PANGO_DIRECTION_TTB_RTL values
     * come from an earlier interpretation of this enumeration as the
     * writing direction of a block of text and are no longer used;
     * See `PangoGravity` for how vertical text is handled in Pango.
     *
     * If you are interested in text direction, you should really use
     * fribidi directly. `PangoDirection` is only retained because it
     * is used in some public apis.
     */
    enum Direction {
     ...
     * text in order to make it fit to a given width and replaced with
     * an ellipsis.
     */
    enum EllipsizeMode {
      /** No ellipsization */
     ...
     * [struct#Pango.Matrix], and then glyph orientation is controlled
     * using `PangoGravity`.
     *
     * Not every value in this enumeration makes sense for every usage
     * of `PangoGravity`; for example, %PANGO_GRAVITY_AUTO only can be
     * passed to [method#Pango.Context.set_base_gravity] and can only
     * be returned by [method#Pango.Context.get_base_gravity].
     *
    ...
    /**
     * `PangoGravityHint` defines how horizontal scripts should behave
    ...
    /**
     * An enumeration specifying the various slant styles possible for
     * a font.
     */
    enum Style {
      /** The font is upright. */
    ...
    /**
     * `PangoTabAlign` specifies where a tab stop appears relative to
     * the text.
     */
    enum TabAlign {
    ...
    enum Underline {
       ...
       * accelerators. %PANGO_UNDERLINE_SINGLE should be used for
       * extended portions of text.
       */
      LOW = 3,
      /**
       * An underline indicating an error should be drawn below. The
       * exact style of rendering is up to the `PangoRenderer` in use,
       * but typical styles include wavy or dotted lines. This
       * underline is typically used to indicate an error such as a
       * possible mispelling; in some cases a contrasting color may
       * automatically be used. This type of underlining is available
       * since Pango 1.4.
       */
      ERROR = 4,
      /**
       * Like #PANGO_UNDERLINE_SINGLE, but drawn continuously across
       * multiple runs. This type of underlining is available since
       * Pango 1.46.
       */
      SINGLE_LINE = 5,
      /**
       * Like #PANGO_UNDERLINE_DOUBLE, but drawn continuously across
       * multiple runs. This type of underlining is available since
       * Pango 1.46.
       */
      DOUBLE_LINE = 6,
     ...
     *
     * For #PANGO_WRAP_WORD, Pango uses break opportunities that are
     * determined by the Unicode line breaking algorithm. For
     * #PANGO_WRAP_CHAR, Pango allows breaking at grapheme boundaries
     * that are determined by the Unicode text segmentation
     * algorithm.
     */
    enum WrapMode {
    ...
    /**
     * A `PangoGlyph` represents a single glyph in the output form of
     * a string.
     */
    type Glyph = number;
     ...
     * device unit might be a pixel for screen display, or a point on
     * a printer.) %PANGO_SCALE is currently 1024, and may change in
     * the future (unlikely though), but you should not depend on its
     * exact value. The {@link PANGO.PIXELS} macro can be used to
     * convert from glyph units into device units with correct
     * rounding.
     */
     ...
     *   freed with [method#Pango.Attribute.destroy]
     */
     ...
     * closest matching font on the system. No fallback will be done
     * to other fonts on the system that might contain the characters
     * in the text.
     *
     ...
     * like small-caps, alternative glyphs, ligatures, etc. for fonts
     * that support them.
     *
     * @param features A string with OpenType font features, with the
     *   syntax of the [CSS font-feature-settings
     ...
     *
     * Pango will insert hyphens when breaking lines in the middle of
     * a word. This attribute can be used to suppress the hyphen.
     *
     ...
     *   vertically, in Pango units. Positive values displace the text
     *   upwards.
     * @returns The newly allocated `PangoAttribute`, which should be
     *   freed with [method#Pango.Attribute.destroy]
     */
     ...
     * @param copy_func Function to copy #data when the attribute is
     *   copied. If %NULL, #data is simply copied as a pointer
     * @param destroy_func Function to free #data when the attribute
     *   is freed
     * @returns The newly allocated `PangoAttribute`, which should be
     *   freed with [method#Pango.Attribute.destroy]
     */
    ...
    /**
     * Create a new attribute that influences how invisible characters
     * are rendered.
     *
     ...
     * {@link G.intern_string} for what that means) that should not be
     * modified or freed.
     *
     ...
     *
     * It applies Unicode rules without language-specific tailoring,
     * therefore the #analyis argument is unused and can be %NULL.
     *
     ...
     *
     * The conversion is done by dividing by the %PANGO_SCALE factor
     * and performing rounding.
     *
     ...
     * #next_paragraph_start are filled with the length of #text (an
     ...
     * one of style, variant, weight, stretch, or gravity, and SIZE is
     * a decimal number (size in points) or optionally followed by the
     * unit modifier "px" for absolute size. VARIATIONS is a
     ...
     *
     * Any one of the options may be absent. If FAMILY-LIST is absent,
     * then the family_name field of the resulting font description
     * will be initialized to %NULL. If STYLE-OPTIONS is missing, then
     * all style options will be set to the default values. If SIZE is
     * missing, the size in the resulting font description will be set
     * to 0.
     *
     * A typical example:
     *
     ...
     * position in #text; if #text contains N characters, it has N+1
     * positions, including the last position at the end of the text.
     * #text should be an entire paragraph; logical attributes can't
     * be computed without context (for example you need to see spaces
     * on either side of a word to know the word is a word).
     *
     ...
     * @param language Language tag
     * @param log_attrs Array with one `PangoLogAttr` per character in
     *   #text, plus one extra, to be filled in
     ...
     * @returns %TRUE if #ch has a mirrored character and #mirrored_ch
     *   is filled in, %FALSE otherwise
     */
     ...
     * and hint.
     *
     * If #base_gravity is %PANGO_GRAVITY_AUTO, it is first replaced
     * with the preferred gravity of #script. To get the preferred
     * gravity of a script, pass %PANGO_GRAVITY_AUTO and
     ...
     *
     * This function is similar to [func#Pango.Gravity.get_for_script]
     * except that this function makes a distinction between
     * narrow/half-width and wide/full-width characters also.
     * Wide/full-width characters always stand _upright_, that is,
     * they always take the base gravity, whereas narrow/full-width
     ...
     * radians.
     *
     * Note that [method#Pango.Matrix.rotate] takes angle in degrees,
     * not radians. So, to call [method#Pango.Matrix,rotate] with the
     * output of this function you should multiply it by (180. /
     * G_PI).
     *
     ...
     *
     * This includes all Unicode characters with "ZERO WIDTH" in their
     * name, as well as _bidi_ formatting characters, and a few other
     * ones. This is totally different from
     ...
     * items in the returned list; the generated list of items will be
     * in logical order (the start offsets of the items are
     * ascending).
     *
     ...
     * position just after #start_index + #length. (i.e. if itemizing
     * in a loop, just keep passing in the same #cached_iter).
     *
     * @param context A structure holding information that affects the
     *   itemization process.
     ...
     * @returns A `GList` of [struct#Pango.Item] structures. The items
     *   should be freed using [method#Pango.Item.free] probably in
     *   combination with {@link G.list_free_full}.
     */
    function itemize(
      context: Context,
      text: string,
    ...
    /**
     * Like {@link `pango.itemize}`, but with an explicitly specified
     * base direction.
     *
     * The base direction is used when computing bidirectional levels.
     * (see [method#Pango.Context.set_base_dir]). [func#itemize] gets
     * the base direction from the `PangoContext`.
     *
     * @param context A structure holding information that affects the
     *   itemization process.
     ...
     * pointers can be efficiently copied (copy the pointer) and
     * compared with other language tags (compare the pointer.)
     *
     * This function first canonicalizes the string by converting it
     * to lowercase, mapping '_' to '-', and stripping all characters
     ...
     * (checked in that order). The locale string typically is in the
     * form lang_COUNTRY, where lang is an ISO-639 language code, and
     * COUNTRY is an ISO-3166 country code. For instance, sv_FI for
     * Swedish as written in Finland or pt_BR for Portuguese as
     * written in Brazil.
     *
     ...
     * setlocale() function) does not use country and language codes,
     * but country and language names spelled out in English. However,
     * this function does check the above environment variables, and
     ...
     * text returned by [method#Pango.Language.get_sample_string], you
     * should first try the default language, followed by the
     ...
     *
     * If the input base direction is a weak direction, the direction
     * of the characters in the text will determine the final resolved
     * direction.
     *
     * @param text The text to itemize.
     * @param length The number of bytes (not characters) to process,
     *   or -1 if #text is nul-terminated and the length should be
     *   calculated.
     * @param pbase_dir Input base direction, and output resolved
     *   direction.
     * @returns A newly allocated array of embedding levels, one item
     *   per character (not byte), that should be freed using
     *   {@link G.free}.
     */
     ...
     * {@link G.markup_parse_context_parse}, use this function to get
     * the list of attributes and text out of the markup. This
     ...
     * parser, use [func#markup_parser_finish] to get the data out of
     * it, and then use g_markup_parse_context_free() to free it.
     *
     * This function is designed for applications that read Pango
     * markup from streams. To simply parse a string containing Pango
     * markup, the [func#parse_markup] API is recommended instead.
     *
     * @param accel_marker Character that precedes an accelerator, or
     *   0 for none
     * @returns A `GMarkupParseContext` that should be destroyed with
     *   {@link G.markup_parse_context_free}.
     */
     ...
     * values for the enum and is not an integer, %FALSE is returned,
     * a warning is issued if #warn is %TRUE, and a string
     * representing the list of possible values is stored in
     ...
     * variations are ignored and the '_' characters may be omitted.
     *
     ...
     * "light", "ultraleight" and integers. Case variations are
     * ignored.
     *
     * @param str A string to parse.
     * @param warn If %TRUE, issue a {@link G.warning} on bad input.
    ...
    /**
     * Quantizes the thickness and position of a line to whole device
     * pixels.
     *
     ...
     * when this function returns, but returned #position may become
     ...
     * delimiter is not written into the buffer. Text after a '#'
     * character is treated as a comment and skipped. '' can be used
     * to escape a # character. '' proceeding a line delimiter
     * combines adjacent lines. A '' proceeding any other character is
     * ignored and written into the output buffer unmodified.
     *
     ...
     *   maintaining a line number counter which doesn't combine lines
     *   with '')
     */
     ...
     *
     * The script of a character is defined by Unicode Standard Annex
     * #24. No check is made for #ch being a valid Unicode character;
     ...
     * the return value of {@link G.unichar_get_script}. Callers must
     * be prepared to handle unknown values.
     *
     * @param ch A Unicode character
     ...
     * language for %PANGO_SCRIPT_CYRILLIC is ru (Russian), the sample
     * language for %PANGO_SCRIPT_ARABIC is ar.
     *
     ...
     * different variants of written Chinese, Japanese, and Korean all
     * use significantly different sets of Han characters and forms of
     * shared characters. No sample language can be provided for many
     * historical scripts as well.
     *
     * As of 1.18, this function checks the environment variables
     * `PANGO_LANGUAGE` and `LANGUAGE` (checked in that order) first.
     * If one of them is set, it is parsed as a list of language tags
     * separated by colons or other separators. This function will
     * return the first language in the parsed list that Pango
     * believes may use #script for writing. This last predicate is
     * tested using [method#Pango.Language.includes_script]. This can
     * be used to control Pango's font selection for non-primary
     * languages. For example, a `PANGO_LANGUAGE` enviroment variable
     * set to "en:fa" makes Pango choose fonts suitable for Persian
     * (fa) instead of Arabic (ar) when a segment of Arabic text is
     ...
     * structure returned from [func#itemize], convert the characters
     ...
     * that API allows for shaping interaction happening across text
     * item boundaries.
     *
     * Note that the extra attributes in the #analyis that is returned
     * from [func#itemize] have indices that are relative to the
     * entire paragraph, so you need to subtract the item offset from
     * their indices before calling [func#shape].
     *
     ...
     * from [func#itemize].
     *
     * This is similar to [func#shape], except it also can optionally
     * take the full paragraph text as input, which will then be used
     * to perform certain cross-item shaping interactions. If you have
     * access to the broader text of which #item_text is part of,
     * provide the broader text as #paragraph_text. If #paragraph_text
     * is %NULL, item text is used instead.
     *
     * Note that the extra attributes in the #analyis that is returned
     * from [func#itemize] have indices that are relative to the
     * entire paragraph, so you do not pass the full paragraph text as
     * #paragraph_text, you need to subtract the item offset from
     * their indices before calling [func#shape_full].
     *
     * @param item_text Valid UTF-8 text to shape.
     * @param item_length The length (in bytes) of #item_text. -1
     *   means nul-terminated text.
     * @param paragraph_text Text of the paragraph (see details). May
     *   be %NULL.
     ...
     * entire paragraph, so you do not pass the full paragraph text as
     * #paragraph_text, you need to subtract the item offset from
     * their indices before calling [func#shape_with_flags].
     *
     * @param item_text Valid UTF-8 text to shape
     * @param item_length The length (in bytes) of #item_text. -1
     *   means nul-terminated text.
     ...
     *
     * @returns %FALSE if skipping the white space leaves the position
     *   at a '\0' character.
     */
     ...
     *   filenames
     * @returns A list of strings to be freed with {@link G.strfreev}
     */
     ...
     * needed, [func#Pango.BidiType.for_unichar] can be used instead.
     *
     * @param ch A Unicode character
     ...
     * macro returns the encoded version available at compile-time. A
     * version number can be encoded into an integer using
     * {@link PANGO.VERSION_ENCODE}.
     *
     * @returns The encoded version of Pango library available at run
     *   time.
     */
     ...
     * %PANGO_VERSION_MINOR, %PANGO_VERSION_MICRO as the three
     * arguments to this function; that produces a check that the
     * library in use at run-time is compatible with the version of
     * Pango the application or module was compiled against.
     *
     ...
     *   The returned string is owned by Pango and should not be
     *   modified or freed.
     */
     ...
     * the macro returns the version available at compile-time.
     *
     * @returns A string containing the version of Pango library
     *   available at run time. The returned string is owned by Pango
     ...
     *
     * @returns Whether the segment should be shifted to center around
     *   the baseline.
     *
     *   This is mainly used in vertical writing directions.
     */
     ...
     *
     * @returns Whether to add a hyphen at the end of the run during
     *   shaping.
     */
    const ANALYSIS_FLAG_NEED_HYPHEN: number;

    /**
     * Value for #start_index in `PangoAttribute` that indicates the
    ...
    /**
     * Value for #end_index in `PangoAttribute` that indicates the end
     * of the text.
     *
     ...
     * glyph for various zero-width Unicode characters (those passing
     * [func#is_zero_width]).
     *
     * @returns A `PangoGlyph` value that indicates a zero-width empty
     *   glpyh.
     *
     *   This is useful for example in shaper modules, to use as the
     ...
     *
     * `PangoLayout` produces one such glyph per invalid input UTF-8
     ...
     * Unicode character into an unknown-character glyph for that
     * `gunichar`.
     *
     * Such unknown-character glyphs may be rendered as a 'hex box'.
     *
     * @returns Flag used in `PangoGlyph` to turn a `gunichar` value
     *   of a valid Unicode character into an unknown-character glyph
     *   for that `gunichar`.
     *
     *   Such unknown-character glyphs may be rendered as a 'hex box'.
     */
     ...
     * for a printer. %PANGO_SCALE is currently 1024, but this may be
     * changed in the future.
     *
     * When setting font sizes, device units are always considered to
     * be points (as in "12 point font"), rather than pixels.
     *
     * @returns The scale between dimensions used for Pango distances
     *   and device units.
     *
     *   The definition of device units is dependent on the output
     *   device; it will typically be pixels for a screen, and points
     ...
     * compile-time.
     *
     * @returns The major component of the version of Pango available
     *   at compile-time.
     */
     ...
     * compile-time.
     *
     * @returns The micro component of the version of Pango available
     *   at compile-time.
     */
     ...
     * compile-time.
     *
     * @returns The minor component of the version of Pango available
     *   at compile-time.
     */
